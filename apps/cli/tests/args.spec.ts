...
describe('parseDshArgs', () => {
  it('routes ordinary named profile boots, handing the rest to the app', () => {
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['--profile', 'tui', '-h']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: [], args: ['-h'] })
  ...
  it('routes named profile config dumps', () => {
    expect(parse(['--profile', 'tui', '--dump-config']))
      .toEqual({ mode: 'dump-config', profile: 'tui', defaultOnly: false, patches: [] })
    expect(parse(['--profile', 'tui', '--dump-default-config']))
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['web'])).toBe(1) // retired alias: a profile must be explicit
    expect(exitCode(['web', '--patch', 'web.yml'])).toBe(1)
