...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('dsh --profile headless')
    expect(help.stdout).toContain('dsh plugin --profile')
    expect(help.stdout).not.toMatch(/^\s+(?:tui|meta|upgrade)\b/mu)
    for (const removed of [['web'], ['tui'], ['--config', 'x.yml'], ['-p', 'task'], ['run', 'task']]) {
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'headless', '--patch', invalidProvider, 'task'], {
        DSH_HOME: home,
  ...
  describe('config dump', () => {
    ...
    it('prints the headless profile without Host transport layers', async () => {
    ...
    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the headless profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'headless', '--dump-default-config'], { DSH_HOME: home })
