...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  // Node's fetch ignores the proxy environment on its own, so install the policy
  // before the first plugin mounts; it resolves from the launcher's snapshot, so a
  // proxy declared in a .env layer works the same as one in the process environment.
