...
struct NativeAssistantMarkdownSourceKey: Hashable, Sendable {
  let messageID: Int
  let sourceSlot: Int
  let source: String
...
private enum NativeAssistantMarkdownProjectionPolicy {
  static func sources(
  ...
  ) -> [NativeAssistantMarkdownSourceKey] {
    ...
    if message.blocks.isEmpty {
      ...
      return [NativeAssistantMarkdownSourceKey(
...
struct NativeAssistantMarkdownProjectionRequest: Equatable, Sendable {
  let sessionID: String
  let epoch: UInt64
  let source: NativeAssistantMarkdownSourceKey
  let token: UUID
}

struct NativeAssistantMarkdownProjectionReconcile: Equatable, Sendable {
  let sessionChanged: Bool
  let removedSources: Set<NativeAssistantMarkdownSourceKey>
}

struct NativeAssistantMarkdownProjectionState: Sendable {
  private(set) var sessionID: String?
  private(set) var epoch: UInt64 = 0
  private(set) var requestedSources = Set<NativeAssistantMarkdownSourceKey>()
  private(set) var installedSources = Set<NativeAssistantMarkdownSourceKey>()
  private var requestTokens: [NativeAssistantMarkdownSourceKey: UUID] = [:]

  mutating func reconcile(
    sessionID: String?,
    requestedSources nextSources: Set<NativeAssistantMarkdownSourceKey>
  ) -> NativeAssistantMarkdownProjectionReconcile {
    let sessionChanged = self.sessionID != sessionID
    var removedSources = requestedSources.subtracting(nextSources)
    if sessionChanged {
      epoch &+= 1
      self.sessionID = sessionID
      removedSources.formUnion(requestedSources)
      requestTokens.removeAll()
      installedSources.removeAll()
    } else {
      for source in removedSources { requestTokens.removeValue(forKey: source) }
      installedSources.subtract(removedSources)
    }
    requestedSources = nextSources
    installedSources.formIntersection(nextSources)
  ...
  mutating func beginRequest(
    for source: NativeAssistantMarkdownSourceKey
  ) -> NativeAssistantMarkdownProjectionRequest? {
    guard let sessionID,
          requestedSources.contains(source),
          !installedSources.contains(source),
  ...
  mutating func accept(_ request: NativeAssistantMarkdownProjectionRequest) -> Bool {
    guard sessionID == request.sessionID,
          epoch == request.epoch,
          requestedSources.contains(request.source),
          requestTokens[request.source] == request.token,
          installedSources.insert(request.source).inserted
    else { return false }
    requestTokens.removeValue(forKey: request.source)
    return true
  }

  @discardableResult
  mutating func cancel(_ request: NativeAssistantMarkdownProjectionRequest) -> Bool {
    guard requestTokens[request.source] == request.token else { return false }
    requestTokens.removeValue(forKey: request.source)
    return true
  }
}
...
private struct NativeChatSnapshot {
  ...
  let markdownBlocksBySource: [NativeAssistantMarkdownSourceKey: [NativeGFMBlock]]

  init(
    model: ArkAppModel,
    entries: [NativeChatEntry],
    contentRevision: UInt64,
    markdownBlocksBySource: [NativeAssistantMarkdownSourceKey: [NativeGFMBlock]] = [:]
  ) {
    self.entries = entries
    context = NativeChatContext(model: model)
    self.contentRevision = contentRevision
    self.markdownBlocksBySource = markdownBlocksBySource
  }
  ...
  func installing(
    ...
    for key: NativeAssistantMarkdownSourceKey
  ) -> NativeChatSnapshot {
    var projected = markdownBlocksBySource
    projected[key] = blocks
    return NativeChatSnapshot(
      entries: entries,
      context: context,
      contentRevision: contentRevision &+ 1,
      markdownBlocksBySource: projected
    )
  }
...
private final class NativeChatTranscriptFeed: ObservableObject {
  ...
  private var markdownTasks: [NativeAssistantMarkdownSourceKey: Task<Void, Never>] = [:]
  ...
  init(model: ArkAppModel) {
    ...
    .sink { [weak self, weak model] _ in
      ...
      let retainedMarkdown = reconciliation.sessionChanged
        ? [:]
        : snapshot.markdownBlocksBySource.filter {
          self.markdownProjectionState.requestedSources.contains($0.key)
        }
      let projectionRemoved = retainedMarkdown.count != snapshot.markdownBlocksBySource.count
      ...
      let next = NativeChatSnapshot(
        ...
        markdownBlocksBySource: retainedMarkdown
      )
  ...
  private func reconcileMarkdownSources(
    ...
    let nextSources = Set(model.messages.flatMap { message in
    ...
    if reconciliation.sessionChanged {
      ...
      for key in reconciliation.removedSources {
        markdownTasks.removeValue(forKey: key)?.cancel()
      }
    }
  ...
  private func scheduleMissingMarkdownSources() {
    for key in markdownProjectionState.requestedSources
    where snapshot.markdownBlocksBySource[key] == nil && markdownTasks[key] == nil {
      guard let request = markdownProjectionState.beginRequest(for: key) else { continue }
      markdownTasks[key] = Task { [weak self] in
        guard !Task.isCancelled else {
          self?.cancelMarkdownRequest(request)
          return
        }
        let blocks = await NativeGFMParseWorker.shared.blocks(for: key.source)
        guard !Task.isCancelled, let blocks, let self else {
          self?.cancelMarkdownRequest(request)
          return
        }
        guard markdownProjectionState.accept(request) else { return }
        markdownTasks.removeValue(forKey: key)
        var transaction = Transaction(animation: nil)
        transaction.disablesAnimations = true
        withTransaction(transaction) {
          self.snapshot = self.snapshot.installing(blocks, for: key)
        }
      }
    }
  }
  ...
  private func cancelMarkdownRequest(_ request: NativeAssistantMarkdownProjectionRequest) {
    ...
    markdownTasks.removeValue(forKey: request.source)
  }
}
...
struct NativeAssistantMarkdownBlockRow: Identifiable, Equatable {
  ...
  var id: ID { ID(source: source, blockIndex: blockIndex) }
  var renderPath: String { "\(source.renderPath).block.\(blockIndex)" }
}

enum NativeAssistantProjectedBodyRow: Identifiable, Equatable {
  enum ID: Hashable {
    case pending(NativeAssistantMarkdownSourceKey)
  ...
  var id: ID {
    switch self {
    case .pending(let source): return .pending(source)
...
enum NativeAssistantMarkdownRowProjection {
  static func rows(
    message: ArkMessage,
    sources: [NativeAssistantMarkdownSourceKey],
    blocksBySource: [NativeAssistantMarkdownSourceKey: [NativeGFMBlock]]
  ) -> [NativeAssistantProjectedBodyRow] {
    var sourceBySlot: [Int: NativeAssistantMarkdownSourceKey] = [:]
    for source in sources { sourceBySlot[source.sourceSlot] = source }
    var rows: [NativeAssistantProjectedBodyRow] = []
    func appendMarkdownRows(for source: NativeAssistantMarkdownSourceKey) {
      guard let blocks = blocksBySource[source] else {
...
private struct NativeAssistantMarkdownSection: Identifiable, Equatable {
  let message: ArkMessage
  let producedFiles: [ArkProducedFile]
  let presentation: NativeMessagePresentation
  let hideReasoning: Bool
  let bodyRows: [NativeAssistantProjectedBodyRow]

  var id: String { "assistant-section-\(message.id)" }
...
private enum NativeChatDisplayEntry: Identifiable, Equatable {
  ...
  case assistantSection(NativeAssistantMarkdownSection)
  ...
  var id: String {
    ...
    case .assistantSection(let section): return section.id
  ...
  var turn: Int? {
    ...
    case .assistantSection(let section): return section.message.turn
...
private struct NativeChatView: View {
  ...
  private var bodyProjection: NativeChatBodyProjection {
    ...
    for entry in entries {
      ...
      appendDisplayEntry(projectedDisplayEntry(
        entry,
  ...
  private func projectedDisplayEntry(
    ...
    let rows = NativeAssistantMarkdownRowProjection.rows(
      message: message,
      sources: sources,
      blocksBySource: transcriptFeed.snapshot.markdownBlocksBySource
    )

    return .assistantSection(NativeAssistantMarkdownSection(
      message: message,
      producedFiles: producedFiles,
      presentation: presentation,
      hideReasoning: hidesFinalReasoning(
        entry,
        finalAnswerByTurn: finalAnswerByTurn
      ),
      bodyRows: rows
  ...
  private func transcriptRow(
    ...
    switch item {
    case .assistantSection(let section):
      if section.hasPrefixContent {
        Section {
          assistantProjectedSectionBody(section, fontSize: fontSize)
        } header: {
          assistantProjectedPrefix(section, fontSize: fontSize)
        } footer: {
          assistantProjectedSuffix(section, fontSize: fontSize)
        }
        .id(scrollID)
      } else {
        Section {
  ...
  @ViewBuilder
  private func assistantProjectedSectionBody(
    _ section: NativeAssistantMarkdownSection,
    fontSize: CGFloat
  ) -> some View {
    ForEach(section.bodyRows) { row in
      assistantProjectedBodyRow(row, section: section, fontSize: fontSize)
    }
  }
    ...
    VStack(alignment: .leading, spacing: 10) {
      if !message.documentReferences.isEmpty {
        NativeMessageDocuments(
          ...
          language: section.presentation.language
        )
      }
  ...
  private func assistantProjectedBodyRow(
    _ row: NativeAssistantProjectedBodyRow,
    section: NativeAssistantMarkdownSection,
    fontSize: CGFloat
  ) -> some View {
    switch row {
    case .pending(let source):
      HStack(spacing: 8) {
        ProgressView().controlSize(.small)
        Text(ArkL10n.text(.chatSyncingHistory, section.presentation.language))
    ...
    case .markdown(let projected):
      ...
      .nativeMarkdownRoutes(
        producedFilePaths: section.producedFiles.map(\.path)
      )
      .frame(maxWidth: .infinity, alignment: .leading)
      .fixedSize(horizontal: false, vertical: true)
      .accessibilityIdentifier(
        "ark.chat.message.\(projected.source.messageID).markdown."
          + "\(projected.source.sourceSlot).\(projected.blockIndex)"
      )
    case .companion(_, _, let block):
      NativeMessageBlockView(
        ...
        streaming: false,
        fontSize: fontSize,
        hideReasoning: section.hideReasoning,
        language: section.presentation.language
      )
      .frame(maxWidth: .infinity, alignment: .leading)
    }
  ...
  private func assistantProjectedSuffix(
    ...
    VStack(alignment: .leading, spacing: 6) {
      if section.message.interrupted {
        Text(ArkL10n.text(.executionCancelled, section.presentation.language))
          .font(.system(size: 10, weight: .medium))
          .foregroundStyle(ArkPalette.secondary)
          .padding(.horizontal, 7)
          .padding(.vertical, 2)
      ...
      if !section.producedFiles.isEmpty {
        NativeProducedFilesRow(
          files: section.producedFiles,
          language: section.presentation.language
        )
      }
    ...
    .frame(maxWidth: .infinity, alignment: .leading)
    .accessibilityIdentifier(
      "ark.chat.message.\(section.message.messageID ?? String(section.message.id))"
    )
  }
      ...
      if let interaction = model.selectedPendingInteraction {
        ...
        if !subagentState.canCompose {
          ...
          if model.composerSuggestions.isOpen {
            NativeComposerSuggestionPanel(model: model)
              .padding(.horizontal, 8)
...
private struct NativeComposerSuggestionPanel: View {
  ...
  private var listHeight: CGFloat {
    ...
    return min(
      320,
      max(46, CGFloat(model.composerSuggestions.candidates.count) * 44
