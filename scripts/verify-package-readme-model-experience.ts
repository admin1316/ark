...
const NO_MODEL_EXPERIENCE_SECTION: Readonly<Record<string, string>> = {
  'packages/util/values': 'Stateless JSON validation, snapshots, freezing and equality helpers; callers own any model-bound values.',
...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/bundle/native-api-app': { kind: 'indirect', reason: 'The API-only patch assembles Host services; Agent presets own the model-facing plugins.' },
  'packages/examples/acp-demo': { kind: 'indirect', reason: 'The app mounts agent-spine-demo and ACP; those composed plugins own model context and ordinary user messages.' },
  'packages/examples/jsonrpc-demo': { kind: 'indirect', reason: 'The bin boots an external cordis.yml; its composed plugins own every model-bound message and schema.' },
  'packages/util/http-proxy': { kind: 'none', reason: 'The policy dispatcher routes outbound HTTP bytes and preserves request content; it registers no model context.' },
