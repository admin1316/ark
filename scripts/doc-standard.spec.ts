...
const PACKAGE_LIBRARIES: Readonly<Record<string, string>> = {
  'packages/boot/native-api-runner': 'Dedicated Native sidecar executable and fixed-profile boot helpers; no plugin entry.',
  'packages/boot/profile-runner': 'Shared profile lifecycle library imported by application executables.',
  'packages/examples/jsonrpc-demo': 'Bin-only JSON-RPC demo application; plain module entry, not an installable plugin.',
  'packages/session/session-format': 'Session format catalog, migration and validation library; no plugin entry.',
  'packages/session/session-format-catalog': 'Released session format catalog assembled for persistence consumers.',
  'packages/session/session-format-v0-to-v1': 'Released session codec and migration library for the v0 to v1 boundary.',
  'packages/session/session-format-v2-to-v3': 'Released session codec and migration library for the v2 to v3 boundary.',
  'packages/util/values': 'Plain JSON value validation, snapshot and equality utilities.',
