...
- **生成物入库**：`lib/**` 构建产物与 208 个 `*.tsbuildinfo` 已移出版本控制；`.gitignore` 忽略这些类别，索引中为零。`pnpm run verify-generated-tracking` 已接入 `ci-static` 车道与 pre-commit 钩子，被暂存或强制加入（`git add -f`）的生成产物会直接失败；移出前提见 [docs/maintenance.md](docs/maintenance.zh.md)。
