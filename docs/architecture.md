...
A **profile** is a named composition stored in the Harness home. It lists the bundles it stacks, holds any out-of-tree plugins it installs, and keeps the user's own `cordis.patch.yml`. `headless` ships as the generic CLI template; managed native products use their own fixed profile and runner.
...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server, while [`dsh-native-api-app`](../packages/bundle/native-api-app/README.md) adds the authenticated API-only Host rows used by a managed native desktop.
...
The session log is the source of the context the model sees. `deriveMessages()` projects model history from it, and raw `assistant/chunk` events preserve replay and client-projection fidelity. Fork, resume, transcripts, telemetry, and persistence all derive from this stream.
...
[Agent Teams](subsystems/agent-team.md) is a scoped coordination seam on `ctx.agentTeams`, with a durable roster, task board, and mailbox layered over continuable subagents. Its domain and model-facing tool packages belong to the Subagent product role and ship with the native standard and code presets.
...
The [extension cookbook](cookbook/extension-cookbook.md) maps features to capabilities and indexes the step-by-step guides for [packages](cookbook/adding-a-package.md), [tools](cookbook/adding-a-tool.md), and [LLM adapters](cookbook/adding-an-llm-adapter.md).
