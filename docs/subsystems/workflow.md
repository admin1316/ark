...
Integrations may fold these records into one workflow item by `runId`; they must preserve exact phase strings, including the distinction between an omitted phase and `''`, and treat a missing terminal record at a closed log location as interruption evidence.
