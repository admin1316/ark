...
The service stores whole Session rows in the `message_feedback` storage domain through `ctx.storageDomain`. Before `put` commits a row that references a target message, a matching live target passes through the canonical `ctx.sessions.flush` checkpoint; both live and cold paths are then physically read from sequence zero through `SessionPersistence.readFrom`. The resulting observation is revalidated before the sidecar write, so the durable target log always precedes its sidecar commit. `maxNoteBytes` is required and bounds note text by UTF-8 bytes; the Native Host composition sets `8192`. The package publishes the Host `messageFeedback.list`, `messageFeedback.put`, and `messageFeedback.delete` unary Remote contract through `TypertRemoteService` and `@Remote`; the generated Cordis API below is the method-level authority.

Plugin disposal closes mutation admission, drains accepted per-Session queue work, and then closes the storage domain.

## Native surface

Ark's SwiftUI shell owns the feedback controls and calls the Host `messageFeedback` Remote methods through `ArkFeedbackAPI`. The shell loads one immutable feedback map per Session, updates it optimistically only after a version-checked response, and refreshes it on a reconnect baseline.

The control is shown only for finalized assistant messages that carry an append-origin `messageId`; interrupted partials remain unrated. Note editing and rating are native views, while the Host service remains the durability and conflict authority.
