...
会话投影 seam 是一项[能力 seam](../capability-seams.zh.md)：领域 host 插件经由它向原生载体供给按会话的日志派生状态当前全量值；三方分别是 Service Definition 与注册表（[dsh-session-projection](../../packages/session/session-projection)，`ctx.sessionProjections`）、领域贡献方（每个领域注册一个纯单元）与消费方（[dsh-host-native-events](../../packages/host/native-events) 推送 `session/projection`，[dsh-host-session-remote-operations](../../packages/host/session-remote-operations) 提供当前基线）。它是一项可选能力，不属于 agent loop（智能体循环）主干。框架负责驱动，领域负责计算：注册表只订阅一次 `session/event`，并把每个已提交事件折叠进每个单元；领域不持有任何订阅，客户端也从不折叠领域事件——它们收到的是成品值。设计权威：[session-projection RFC](../../.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.zh.md)；驱动、缓存与变更流约定：[包 README](../../packages/session/session-projection/README.zh.md)。
