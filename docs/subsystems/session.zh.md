...
如果某项集成要把同一个插件事件族中的多条事件投影成一个客户端条目，该事件族中的每条 start、update、result、resource 或 interruption 事件都必须携带或独立推导出同一个稳定业务 id。此要求只约束需要关联的事件族；客户端因此无须根据相邻关系猜测归属，也无须扫描历史。
