 ...
 * its session from the store, and finally unwinds its scoped world. An owning
 * product surface may preserve durable pending input during lifecycle shutdown;
 * explicit user cancellation keeps the default clear-on-dispose behavior.
 *
...
interface Agent {
   ...
   * A concrete driver may reject with an explicit residual-operation error
   * after its cancellation grace expires; that rejection means the Agent is
   * still registered and running, not detached or falsely quiescent.
...
Cancellation that exceeds its finite grace reports the exact process-local work still active; it does not claim the Agent reached idle:

Source: [`packages/core/agent-loop/src/agent.ts`](../../packages/core/agent-loop/src/agent.ts)

```ts type-equiv
/** Exact process-local residual reported without pretending the agent is quiescent. */
interface AgentQuiescenceResidual {
...
<a id="agentquiescence-timeout--emit"></a>

#### `agent/quiescence-timeout` — emit

Cancellation reached its finite grace boundary while same-process code was still executing. The Agent remains registered and non-quiescent. Scope-filtered dispatch keys the carrier by `payload.agent`, preserving its base filter and admitting unscoped listeners or listeners in that agent's scope or an enclosing scope. This is a process-local notification.
 ...
 * @mode emit
 */
'agent/quiescence-timeout'(this: Scoped<Agent>, payload: { agent: Agent turn: number step: number residual: AgentQuiescenceResidual }): void
```
