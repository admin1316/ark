...
服务通过 `ctx.storageDomain` 在 `message_feedback` 存储域中保存完整 Session 行。`put` 提交引用目标消息的伴随记录前，身份匹配的 live 目标先经过权威 `ctx.sessions.flush` checkpoint；随后 live 与 cold 路径都会通过 `SessionPersistence.readFrom` 从序列零做物理复读。写入伴随记录前会再次校验所得观测，因此目标日志的持久提交始终先于其伴随记录。`maxNoteBytes` 为必填项，按 UTF-8 字节限制备注文本；Native Host 组合将其设为 `8192`。该包通过 `TypertRemoteService` 与 `@Remote` 发布 Host `messageFeedback.list`、`messageFeedback.put` 和 `messageFeedback.delete` 一元 Remote 约定；下方生成的 Cordis API 是方法级权威。

Plugin disposal 会先关闭变更接纳，排空已进入各 Session 队列的工作，然后才关闭 storage domain。

## Native 界面

Ark 的 SwiftUI shell 持有反馈控件，并通过 `ArkFeedbackAPI` 调用 Host `messageFeedback` Remote 方法。shell 为每个 Session 加载一张不可变反馈映射，只有版本校验响应成功后才更新；重连基线会触发刷新。

控件只出现在带有 append-origin `messageId` 的已完成助手消息上；中断的部分输出不参与评分。备注编辑与评分都是原生视图，Host 服务仍是持久化和冲突权威。
