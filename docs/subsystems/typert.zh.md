...
以下类型由生成的 Remote 产物、严格 Host Gateway 与 Native API assembly 共用。[Typert Gateway Agent Note](../../.agents/notes/implemented/architecture/2026-08-02-typert-remote-method-calls.zh.md) 负责架构与传输决策；本页记录 [`dsh-typert-protocol`](../../packages/typert/protocol/src/types.ts) 和 [`dsh-api-gateway`](../../packages/api/gateway/src/index.ts) 中公共约定的字面定义。
...
Host Connection 先鉴权并解码 carrier envelope，随后由严格 Gateway 解析实时斜杠 Remote descriptor。请求将精确的具名 wire 字段与载体取消 signal 分开携带；descriptor 参数和结果校验会封闭失败，普通异常则由 RPC 载体归并为传输层的 `internal` 错误码。
