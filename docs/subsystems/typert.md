...
Types shared by generated Remote artifacts, the strict Host Gateway, and Native API assemblies. The [Typert Gateway Agent Note](../../.agents/notes/implemented/architecture/2026-08-02-typert-remote-method-calls.md) owns the architecture and transport decisions; this page records the literal public contracts from [`dsh-typert-protocol`](../../packages/typert/protocol/src/types.ts) and [`dsh-api-gateway`](../../packages/api/gateway/src/index.ts).
...
Host Connection authenticates and decodes the carrier envelope before the strict Gateway resolves a live slash Remote descriptor. The request carries exact named wire fields and the carrier cancellation signal separately; descriptor argument and result validation fail closed, while ordinary exceptions are folded by the RPC carrier into the transport `internal` error code.
