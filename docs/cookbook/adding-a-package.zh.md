...
| `tsconfig.host.json` | 在 `references` 中添加 `{ "path": "./packages/<group>/<pkg>" }`——每个包属于 Host aggregate（[布局](../development.zh.md#typescript-project-layout)） |
| `knip.json` | 仅当包有仓库发现机制尚未覆盖的入口时需要 |

已退役的浏览器包树不再有注册路径。新包 extends `tsconfig.base.json`，声明自身的 Host/runtime 依赖，并只暴露实际拥有的入口。
