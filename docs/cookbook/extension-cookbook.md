...
## A UI or editor integration

A UI or editor integration renders from the `session/event` feed (the assistant token stream as `assistant/chunk`, plus turn/step boundaries and tool activity) and drives input back in through `agent.followup()` / `agent.steer()`. An out-of-process integration adapts the same lifecycle through its protocol driver.
