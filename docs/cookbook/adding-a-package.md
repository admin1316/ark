...
| `tsconfig.host.json` | add `{ "path": "./packages/<group>/<pkg>" }` to `references` — every package belongs to the Host aggregate ([layout](../development.md#typescript-project-layout)) |
| `knip.json` | only if the package has entrypoints that repository discovery does not already cover |

The retired browser package tree has no registration path here. New packages extend `tsconfig.base.json`, declare their Host/runtime dependencies, and expose only the entrypoints they own.
