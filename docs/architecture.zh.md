...
**profile** 是存放在 Harness home 中的具名组装。它列出自己叠放的组合包，存放自己安装的树外插件，并保存用户自己的 `cordis.patch.yml`。`headless` 是随通用 CLI 交付的模板；受管理的原生产品使用各自固定的 profile 与 runner。
...
[`dsh-base`](../packages/bundle/base/README.zh.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-headless`](../packages/bundle/headless/README.zh.md) 增加不带服务器的一次性 runner，[`dsh-native-api-app`](../packages/bundle/native-api-app/README.zh.md)则增加受管理原生桌面使用的鉴权 API-only Host 配置行。
...
会话日志是模型所见上下文的来源。`deriveMessages()` 从中投影出模型历史，原始 `assistant/chunk` 事件则保证回放与客户端投影保真。fork、恢复、transcript（文本记录）、遥测和持久化都派生自该事件流。

**模型可见即已记录。** 抵达模型请求的一切都必须能从日志重建，并由一项运行时不变量断言这一点。因此，新增一项模型可见输入就需要新增一个会话事件：扩展 `SessionEventMap` 并从日志渲染。

首个请求优先采用创建 Agent 时显式指定的 reasoning effort，否则只恢复同路线的历史显式值，再执行普通请求策略与适配器校验（[循环约定](../packages/core/agent-loop/README.zh.md)）。
...
[Agent Teams](subsystems/agent-team.zh.md) 是 `ctx.agentTeams` 上按作用域启用的协作 seam，在可继续 subagent 之上提供持久 roster、任务板和 mailbox。其领域包和面向模型的工具包归入 Subagent 产品角色，并随原生 standard 与 code preset 发布。
...
[扩展实操手册](cookbook/extension-cookbook.zh.md)将功能映射到能力，并索引[包](cookbook/adding-a-package.zh.md)、[工具](cookbook/adding-a-tool.zh.md)与 [LLM（大语言模型）适配器](cookbook/adding-an-llm-adapter.zh.md)的分步指南。
