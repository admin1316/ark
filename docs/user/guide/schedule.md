...
This overlay enables Schedule in an installed [custom CLI profile](../../../apps/cli/README.md#profiles) with persistent Sessions and live root Agents. The profile must resolve `@deepseek-ai/dsh-schedule` and `@deepseek-ai/dsh-time-context` from its declared dependencies. `my-agent` below names that configured profile, not a shipped profile:
