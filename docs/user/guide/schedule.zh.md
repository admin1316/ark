...
此 overlay 在已安装的[自定义 CLI profile](../../../apps/cli/README.zh.md#profiles) 中启用 Schedule，该 profile 必须提供持久 Session 与 live 根 Agent，并能从其声明的依赖解析 `@deepseek-ai/dsh-schedule` 和 `@deepseek-ai/dsh-time-context`。下文的 `my-agent` 指已配置的 profile，并非随附 profile：
...
调用方可以为提示词附加 IANA 时区。存在该时区时，Time-context 会告诉模型，把未明确限定时区的日期和时间解释为该请求的时区。此假设仅用于自然语言解释：`schedule_create.at` 必须是带 `Z` 或数值偏移量且严格符合 RFC 3339 的日期时间，或是带显式 `UTC` 或 IANA Area/Location 时区的 `{ date, time, time_zone }`。Schedule 不保留或推断 Session 默认时区。夏令时缺口会被拒绝，重叠时段选择第一个时刻；成功创建的记录只保留所得的 UTC 目标。
