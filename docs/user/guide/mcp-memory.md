...
Use an installed [custom CLI profile](../../../apps/cli/README.md#profiles) that keeps an Agent available for interactive requests and resolves `@deepseek-ai/dsh-mcp-client` from its declared dependencies. In the examples below, `my-agent` names that profile; it is not a shipped profile. Pass one overlay when starting it:

```sh
dsh --profile my-agent --patch "$PWD/apps/cli/config/examples/mcp-memory/memorix.cordis.yml"
```
...
dsh --profile my-agent --patch "$PWD/apps/cli/config/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
dsh --profile my-agent --patch "$PWD/apps/cli/config/examples/mcp-memory/engram.cordis.yml"
```
