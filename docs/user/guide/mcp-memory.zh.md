...
使用已安装的[自定义 CLI profile](../../../apps/cli/README.zh.md#profiles)：它必须保持可接受交互请求的 Agent，并能从其声明的依赖解析 `@deepseek-ai/dsh-mcp-client`。以下示例中的 `my-agent` 指该 profile，并非随附 profile。启动它时传入一个 overlay：
