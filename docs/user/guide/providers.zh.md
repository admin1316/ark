...
headless profile 会从启动环境或已配置的凭据提供方读取凭据，并从 `$DSH_HOME/settings.yaml` 读取持久适配器设置。新的一次性调用会读取最新值。

## 配置 DeepSeek

导出随发行版交付的 DeepSeek 适配器使用的凭据引用：

```sh
export DEEPSEEK_API_KEY=sk-your-key-here
```

不要把明文 secret 写入 `settings.yaml`；适配器配置通过 `apiKeyEnv` 指向环境变量或受管理的凭据引用。

## 添加目录提供方

在 `llm-pi-ai.providers` 下声明一个目录提供方，并导出它引用的凭据。已安装目录会提供端点、协议和模型列表：
...
对于公司网关、自建服务器或已安装目录中不存在的提供方，请添加小写 provider id、基础 URL、API 协议、凭据引用和至少一个模型：
...
settings 文档是权威记录。已安装的目录路由可以省略 `models`；手工声明的路由必须列出自己服务的每个模型。

### 图片输入

手动输入的模型在自己声明之前一律按纯文本对待，因为没有任何环节能去询问端点接受哪些模态。给这类模型附加图片，会在发送前就被拒绝，并点名该模型。

因此自定义提供方下的视觉模型需要在 `$DSH_HOME/settings.yaml` 中声明 `input`：
...
其中两样占了绝大多数。声明了推理能力的模型，其系统提示词会以 `role: "developer"` 发出，很多网关直接拒绝；输出上限则写作 `max_completion_tokens`，只认 `max_tokens` 的服务端会拒绝。请在 `$DSH_HOME/settings.yaml` 的路由上更正：
...
在 `agent-default-model` 分节中设置新会话使用的默认值：
...
已经发送过请求的会话会保留自身日志中记录的模型。指向已删除提供方的已保存默认值会保持不可用，直到该分节改为存活路由。

## 排错

- **`MISSING_CREDENTIAL`**：提供被引用的环境变量或受管理凭据。
