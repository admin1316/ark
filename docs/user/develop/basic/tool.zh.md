...
本教程会向 headless profile 添加一个面向模型的 `greet` 工具。请先完成[第一个插件](./index.zh.md)，并保留其中的 `scratch-plugin` 目录。
...
模型可以调用 `greet`，收到 `Hello, Ada!` 作为工具结果，命令随后打印其最终回复。

## 下一步

- [插件配置](./config.zh.md) — 让问候语可配置。
- [工具编写参考](../../../cookbook/adding-a-tool.zh.md) — 查阅嵌套 schema、规范值、后台工作、策略钩子与 Code Mode。
