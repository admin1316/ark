...
本教程会创建一个最小的 Harness 插件，并将其加载到 headless profile 中。请从已完成[开发环境搭建](../../../development.zh.md#setup-tutorial)的仓库检出开始。
...
在仓库根目录运行 `pwd`，然后创建 `scratch-plugin/cordis.yml`，作为插入本地插件的 profile overlay。请将下文的 `/absolute/path/to/deepseek-harness` 替换为命令打印的路径：
...
使用该覆盖层运行一次 headless 任务：

```sh
pnpm dsh --profile headless --patch ./scratch-plugin/cordis.yml "Reply with OK."
```

启动期间，终端会打印 `[hello-plugin] plugin loaded!`，随后打印任务的最终回复。该命令不会打开浏览器，也不会监听任何网络端口。
