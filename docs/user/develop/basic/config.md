...
A long-lived profile runner can watch configuration and replace the plugin; a headless invocation reads the latest file at its next start. Because registrations are effects and clean themselves up, replacement or shutdown does not retain the old instance's registrations.
