...
如果改用全新的源码 checkout，请先按照[开发环境搭建](../../../development.zh.md#setup-tutorial)完成准备，将本教程的 `hello-plugin` 目录放在仓库根目录，并从该目录把下文的 `dsh ...` 命令改为 `pnpm dsh ...`。构建与启动器行为见[源码执行](../../../../apps/cli/reference/README.zh.md#source-execution)。
...
- 你的 patch 可以按 `id` 覆盖前面各层的行——就像 [`dsh-headless` 组合包](../../../../packages/bundle/headless/cordis.patch.yml)覆盖 `dsh-base` 的行那样——但必须重述该行需要的每一个键，而不是只写改动的那个。
