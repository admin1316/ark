...
Run `pwd` from the repository root, then create `scratch-plugin/cordis.yml` as a profile overlay that inserts the local plugin. Replace `/absolute/path/to/deepseek-harness` below with the printed path:
...
Run one headless task with that overlay:

```sh
pnpm dsh --profile headless --patch ./scratch-plugin/cordis.yml "Reply with OK."
```

The terminal prints `[hello-plugin] plugin loaded!` during startup, followed by the task's final response. The command opens no browser and listens on no network port.
