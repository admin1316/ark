...
| `ctx.deepseekLlmApiExtensions` | `seam` | [`deepseek-llm-api-extensions`](../packages/llm/deepseek-llm-api-extensions) | - | [`llm-deepseek`](../packages/llm/llm-deepseek) | [`plugin-package-inventory-deepseek`](../packages/llm/plugin-package-inventory-deepseek), [`session-log-deepseek`](../packages/session/session-log-deepseek) | Host plugins register independently prepared request fields; the DeepSeek adapter validates and merges the enabled fields immediately before one wire request. |
...
| `ctx.typert` | `core` | [`typert-registry`](../packages/typert/registry) | - | [`typert-loader`](../packages/typert/loader), [`api-gateway`](../packages/api/gateway) | - | Plugins register live zod contributions directly or through dsh-typert-loader; the strict Host Gateway consumes invocation descriptors and providers, while other runtime consumers query schemas and reflection metadata at their own edges. |
| `ctx.typertGateway` | `core` | [`api-gateway`](../packages/api/gateway) | - | - | - | Owns strict slash Remote dispatch on Connection's shared `/api` channel. It validates descriptor arguments and results after Connection authenticates a request; Connection retains bearer checks and physical transport ownership. |
| `ctx.connection` | `core` | `connection` | - | [`api-gateway`](../packages/api/gateway), [`native-api-app`](../packages/bundle/native-api-app), `native-events`, `session-remote-operations` | - | Owns the authenticated loopback HTTP bridge, strict RPC carrier, exact response/download registrations, and the host/multiplexed WebSocket downlinks consumed by the native shell. |
| `ctx.nativeEvents` | `core` | `native-events` | - | [`native-api-app`](../packages/bundle/native-api-app) | - | Projects authoritative Session, Agent, job, Workspace, approval, and question state onto Connection event sources while retaining one stable response-correlation table. |
...
| `ctx.settings` | `seam` | [`settings`](../packages/settings/settings) | [`settings-file`](../packages/settings/settings-file) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-pi-ai`](../packages/llm/llm-pi-ai) | - | Plugins register namespace schemas and resolve layered values; providers store the raw document. The service owns its redacted strict Remote methods, including opening only the provider-owned document path. |
| `ctx.credentials` | `seam` | [`credentials`](../packages/credentials/credentials) | [`credentials-local`](../packages/credentials/credentials-local) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-pi-ai`](../packages/llm/llm-pi-ai) | - | Configuration carries references to secrets; providers own the values. Consumers resolve per operation, so a rotated credential reaches the very next request; the service exposes only value-free strict Remote views and write-only mutations. |
...
| `ctx.workspaceRegistry` | `core` | [`workspace`](../packages/workspace/workspace) | - | `native-events`, `session-remote-operations` | - | Owns WorkspaceId-branded records and its strict workspace/* Remote methods; stable sessionIds drive Native event projections and session admission. |
...
| `ctx.sessionProjections` | `core` | [`session-projection`](../packages/session/session-projection) | - | [`tool-todo`](../packages/todo/tool-todo), [`session-title`](../packages/session/session-title), `native-events`, `session-remote-operations` | - | Domains register state-driven fold units; the eager drive keeps per-session watermark states, NativeEvents pushes changes, and SessionRemoteOperations serves current baselines. |
| `ctx.sessionProjectionCache` | `core` | [`session-projection-cache`](../packages/session/session-projection-cache) | - | `session-remote-operations` | - | Durably checkpoints projection unit states per session (throttled + turn/end/detach mandatory points) and serves the cold-read ladder: cache row + persistence tail replay, so listings never load full logs. |
...
| `ctx.agentDefaultModel` | `core` | [`agent-default-model`](../packages/core/agent-default-model) | - | [`headless`](../packages/bundle/headless), `session-remote-operations` | - | Layers the default ModelSelection through settings so direct and Host-backed Agent entry points share one state owner. |
...
| `ctx.subagentModelSelection` | `core` | [`tool-subagent`](../packages/subagent/tool-subagent) | - | [`tool-subagent`](../packages/subagent/tool-subagent) | - | Snapshots whether model selection is enabled and the exact provider/model allowlist used before a child route, reasoning effort, or output limit is accepted. |
...
| `ctx.directoryPicker` | `seam` | `directory-picker` | `directory-picker-native` | - | - | The native backend opens one OS chooser on the host display for compositions that explicitly consume this capability; Ark itself uses its AppKit chooser. |
