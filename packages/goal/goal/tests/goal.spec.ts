...
describe('GoalService mutations', () => {
  it('reuses business methods for later mutations', async () => {
    const { ctx, agent } = await harness()
    const created = ctx.goals.create(agent, { objective: 'remote lifecycle' })
    const edited = ctx.goals.edit(agent, { id: created.id, revision: created.revision }, { objective: 'edited remotely' })
    ...
    expect(cleared).toEqual({ id: created.id, revision: 6 })
  })
