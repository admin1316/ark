...
import { TypertLookupFailure, TypertRemoteService, RemoteScope } from '@deepseek-ai/dsh-typert-protocol';
...
let GoalService = (() => {
    ...
    let _remoteExportEdit_decorators;
    let _remoteExportPause_decorators;
    let _remoteExportResume_decorators;
    let _remoteExportComplete_decorators;
    let _remoteExportClear_decorators;
    return class GoalService extends _classSuper {
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            _remoteExportCreate_decorators = [RemoteScope('agent', 'create')];
            _remoteExportEdit_decorators = [RemoteScope('agent', 'edit')];
            _remoteExportPause_decorators = [RemoteScope('agent', 'pause')];
            _remoteExportResume_decorators = [RemoteScope('agent', 'resume')];
            _remoteExportComplete_decorators = [RemoteScope('agent', 'complete')];
            _remoteExportClear_decorators = [RemoteScope('agent', 'clear')];
            ...
            __esDecorate(this, null, _remoteExportEdit_decorators, { kind: "method", name: "remoteExportEdit", static: false, private: false, access: { has: obj => "remoteExportEdit" in obj, get: obj => obj.remoteExportEdit }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _remoteExportPause_decorators, { kind: "method", name: "remoteExportPause", static: false, private: false, access: { has: obj => "remoteExportPause" in obj, get: obj => obj.remoteExportPause }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _remoteExportResume_decorators, { kind: "method", name: "remoteExportResume", static: false, private: false, access: { has: obj => "remoteExportResume" in obj, get: obj => obj.remoteExportResume }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _remoteExportComplete_decorators, { kind: "method", name: "remoteExportComplete", static: false, private: false, access: { has: obj => "remoteExportComplete" in obj, get: obj => obj.remoteExportComplete }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _remoteExportClear_decorators, { kind: "method", name: "remoteExportClear", static: false, private: false, access: { has: obj => "remoteExportClear" in obj, get: obj => obj.remoteExportClear }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        constructor(ctx, config = {}) {
            super(ctx, 'goals', { namespace: 'goal' });
            this.resolved = {
        ...
        /**
         * Create one Goal through the receiver Agent's scoped Remote boundary.
         * @param request - goal creation request.
         * @returns the created Goal reference and revision.
         */
        remoteExportCreate(request) {
            return this.remoteGoal(() => {
                const view = this.create(this.remoteAgent(), request);
                return { ref: { id: view.id, revision: view.revision } };
            });
        }
        /**
         * Edit one Goal through the receiver Agent's scoped Remote boundary.
         * @param request - Goal reference and replacement fields.
         * @returns the edited Goal reference and revision.
         */
        remoteExportEdit(request) {
            return this.remoteGoal(() => {
                const view = this.edit(this.remoteAgent(), request.ref, request);
                return { ref: { id: view.id, revision: view.revision } };
            });
        }
        /**
         * Pause one Goal through the receiver Agent's scoped Remote boundary.
         * @param request - Goal reference to pause.
         * @returns the paused Goal reference and revision.
         */
        remoteExportPause(request) {
            return this.remoteGoal(() => {
                const view = this.pause(this.remoteAgent(), request.ref);
                return { ref: { id: view.id, revision: view.revision } };
            });
        }
        /**
         * Resume one Goal through the receiver Agent's scoped Remote boundary.
         * @param request - Goal reference to resume.
         * @returns the resumed Goal reference and revision.
         */
        remoteExportResume(request) {
            return this.remoteGoal(() => {
                const view = this.resume(this.remoteAgent(), request.ref);
                return { ref: { id: view.id, revision: view.revision } };
            });
        }
        /**
         * Complete one Goal through the receiver Agent's scoped Remote boundary.
         * @param request - Goal reference to complete.
         * @returns the completed Goal reference and revision.
         */
        remoteExportComplete(request) {
            return this.remoteGoal(() => {
                const view = this.complete(this.remoteAgent(), request.ref);
                return { ref: { id: view.id, revision: view.revision } };
            });
        }
        /**
         * Clear one Goal through the receiver Agent's scoped Remote boundary.
         * @param request - Goal reference to clear.
         * @returns confirmation that the Goal was cleared.
         */
        remoteExportClear(request) {
            return this.remoteGoal(() => {
                this.clear(this.remoteAgent(), request.ref);
                return { cleared: true };
            });
        }
        /** Resolve the exact Agent supplied by the generated `agent` Remote context. */
        remoteAgent() {
            const agent = this.ctx.agent;
            if (agent === undefined)
                throw remoteGoalFailure('goal Remote receiver has no active Agent context', 'GOAL_AGENT_NOT_LIVE');
            this.assertLive(agent);
            return agent;
        }
        /** Preserve a stable Goal error at the Remote boundary rather than relabeling it as internal. */
        remoteGoal(operation) {
            try {
                return operation();
            }
            catch (error) {
                if (error instanceof GoalError)
                    throw remoteGoalFailure(error.message, error.code);
                throw error;
            }
        }
    };
...
export { GoalService };
/** Shape a Goal-domain failure exactly as the Typert carrier preserves it. */
function remoteGoalFailure(message, goalCode) {
