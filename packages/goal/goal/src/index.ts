...
import type {
  ...
  GoalProjection,
  GoalRef,
  GoalRemoteClearResult,
  GoalRemoteEditRequest,
  GoalRemoteRefRequest,
  GoalSnapshot,
  GoalView,
} from './types.ts'
import type {
...
export class GoalService extends TypertRemoteService {
  ...
  remoteExportCreate(request: CreateGoalRequest): CreateGoalResult {
   ...
   */
  @RemoteScope('agent', 'edit')
  remoteExportEdit(request: GoalRemoteEditRequest): CreateGoalResult {
    return this.remoteGoal(() => {
   ...
   */
  @RemoteScope('agent', 'pause')
  remoteExportPause(request: GoalRemoteRefRequest): CreateGoalResult {
    return this.remoteGoal(() => {
   ...
   */
  @RemoteScope('agent', 'resume')
  remoteExportResume(request: GoalRemoteRefRequest): CreateGoalResult {
    return this.remoteGoal(() => {
   ...
   */
  @RemoteScope('agent', 'complete')
  remoteExportComplete(request: GoalRemoteRefRequest): CreateGoalResult {
    return this.remoteGoal(() => {
   ...
   */
  @RemoteScope('agent', 'clear')
  remoteExportClear(request: GoalRemoteRefRequest): GoalRemoteClearResult {
    return this.remoteGoal(() => {
  ...
  private remoteAgent(): Agent {
    ...
    if (agent === undefined) throw remoteGoalFailure('goal Remote receiver has no active Agent context', 'GOAL_AGENT_NOT_LIVE')
    this.assertLive(agent)
    return agent
  }

  /** Preserve a stable Goal error at the Remote boundary rather than relabeling it as internal. */
  private remoteGoal<Result>(operation: () => Result): Result {
    try {
      return operation()
    } catch (error: unknown) {
      if (error instanceof GoalError) throw remoteGoalFailure(error.message, error.code as GoalErrorCode)
      throw error
    }
  }
}
...
function remoteGoalFailure(
  message: string,
  goalCode: import('./domain.ts').GoalErrorCode,
): TypertLookupFailure<RemoteFailure> {
