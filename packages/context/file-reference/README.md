---
description: "File-reference discovery seam and @file grammar shared by native and other host-backed user interfaces. ctx.fileReferences.list(agent, query, signal) returns path-only file or directory candidates for the addressed agent; concrete providers own namespace access, ranking, caching, and invalidation."
...
`fileReferences/list` is declared solely by this Service Definition. Session Controller no longer mounts a duplicate adapter; concrete discovery providers remain composed as before. Agent lookup, query, trailing cancellation signal, and candidate-array contracts are unchanged, and providers retain root access boundaries and caching. Retained browser assembly source selects this package's generated Remote contribution directly; this change does not restore browser delivery.
