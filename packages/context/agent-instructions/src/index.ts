...
import { Config, resolveAgentInstructionsConfig, workspaceBaselineIdentity, type ResolvedConfig } from './config.ts'
...
export function apply(ctx: Context, config: Config): void {
  const resolved: ResolvedConfig = resolveAgentInstructionsConfig(config)
