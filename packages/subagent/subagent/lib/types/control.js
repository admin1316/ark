 ...
 */
export function rejectControl(code, message, details) {
...
export function rejectCatalogRead(error, signal) {
    ...
    if (error instanceof SubagentError && error.code === 'SUBAGENT_CONTROL_PROJECTIONS_UNAVAILABLE') {
        return rejectControl('subagent-projections-unavailable', 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @deepseek-ai/dsh-session-projection)', {});
    }
    ...
    if (error instanceof SubagentError) {
        switch (error.code) {
            ...
            case 'PERSISTENCE_UNAVAILABLE':
                return rejectControl('subagent-delivery-unavailable', 'subagent follow-up is temporarily unavailable', { childSessionId });
