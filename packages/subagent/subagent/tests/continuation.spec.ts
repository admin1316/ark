...
describe('SubagentRuntime.followup residency routing', () => {
  ...
  it('refuses a follow-up without a persistence owner before accepting content', async () => {
    const { ctx, parent } = await setupWith(new MockAdapter([]), { persistence: false })
    await expect(followup(ctx, parent, SessionId('no-durability-owner'), message('continue')))
