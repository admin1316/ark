...
import type { SpawnTeammateRequest, SpawnTeammateResult, TeamMemberSnapshot, TeamMemberView } from './types.ts'
 ...
 */
export function resolveActiveMember(root: Agent, state: TeamFoldState, rawName: string): { id: SessionId; name: string } {
  ...
  if (member === undefined || member.phase !== 'active') throw new TeamError(`active teammate "${name}" not found`, 'TEAM_MEMBER_NOT_FOUND')
  return { id: member.id, name }
}

/** Owns roster identities and their continuable children. */
export class TeamRoster {
  private readonly inFlightCreations = new Set<Promise<SpawnTeammateResult>>()
  constructor(
  ...
  membership(agent: Agent): TeamMembership {
    ...
    if (membership === undefined) throw new TeamError(`agent "${agent.id}" is not a member of an active Agent Team`, 'TEAM_NOT_MEMBER')
    return membership
  }
  ...
  async spawn(caller: Agent, request: SpawnTeammateRequest): Promise<SpawnTeammateResult> {
    ...
    finally { this.inFlightCreations.delete(operation) }
  }
   ...
   */
  pendingCreations(): readonly Promise<unknown>[] { return [...this.inFlightCreations] }
  ...
  private async spawnAdmitted(caller: Agent, request: SpawnTeammateRequest): Promise<SpawnTeammateResult> {
    const membership = this.membership(caller)
    ...
    try {
      ...
      const failed: TeamMemberSnapshot = { ...member, phase: 'failed', error: errorMessage(error) }
      try {
    ...
    const active: TeamMemberSnapshot = { ...member, phase: 'active' }
    if (await this.settleProvisioning(root, active) === 'failed') {
      ...
      try { await this.stopTeammates(root, [childId]) }
      catch (cleanupError) { throw new AggregateError([conflict, cleanupError], 'provisioning conflict cleanup failed') }
      throw conflict
    }
    return { member: this.memberView(active) }
  }

  private async checkpointInitialPrompt(childId: SessionId, messageId: MessageId, signal: AbortSignal): Promise<void> {
    for (;;) {
      signal.throwIfAborted()
      ...
      const stopEvent = this.ctx.on('session/event', (candidate) => { if (candidate === session) progress.resolve(undefined) })
      const stopDisposed = this.ctx.on('session/disposed', (candidate) => { if (candidate === session) progress.resolve(undefined) })
      const onAbort = (): void => {
  ...
  private async reconcileProvisioning(root: Agent, signal: AbortSignal): Promise<void> {
    ...
    for (const member of provisioning) {
      ...
      } catch (error) { failure = `child Session recovery failed: ${errorMessage(error)}` }
      signal.throwIfAborted()
      await this.journal.transact(root.id, async () => {
        signal.throwIfAborted()
        const current = this.journal.state(root).members.get(member.id)
        if (current?.phase !== 'provisioning') return
        const settled: TeamMemberSnapshot = { ...current, phase, ...(phase === 'failed' ? { error: failure } : {}) }
  ...
  private settleProvisioning(root: Agent, terminal: TeamMemberSnapshot): Promise<'active' | 'failed'> {
    return this.journal.transact(root.id, async () => {
      const current = this.journal.state(root).members.get(terminal.id)
      if (current === undefined) throw new TeamError(`provisioned teammate "${terminal.id}" disappeared`, 'TEAM_PROVISIONING_CONFLICT')
