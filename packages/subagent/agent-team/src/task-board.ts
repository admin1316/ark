...
import { TeamId, TeamTaskId } from './brand.ts'
import { TeamError } from './error.ts'
import type { TeamFoldState } from './fold.ts'
import type { TeamJournal } from './journal.ts'
import { resolveActiveMember, type TeamMembership } from './roster.ts'
import { assertTaskGraphCandidate, TeamTaskGraphError } from './task-graph.ts'
import type { CreateTeamTaskRequest, TeamTaskSnapshot, TeamTaskView, UpdateTeamTaskRequest } from './types.ts'
...
export class TeamTaskBoard {
  constructor(private readonly journal: TeamJournal, private readonly maxTasks: number) {}
  ...
  update(caller: Agent, membership: TeamMembership, request: UpdateTeamTaskRequest): Promise<TeamTaskView> {
    ...
    return this.journal.transact(root.id, async () => {
      ...
      if (current.revision !== request.expectedRevision) throw new TeamError(
      ...
      switch (request.action) {
        case 'claim':
          if (current.ownerId !== undefined && current.ownerId !== caller.id) throw new TeamError(`team task "${current.id}" is owned by another member`, 'TEAM_TASK_ALREADY_CLAIMED')
        ...
        case 'delete': {
          ...
          if (dependent !== undefined) throw new TeamError(`team task "${current.id}" still blocks "${dependent.id}"`, 'TEAM_TASK_HAS_DEPENDENTS')
  ...
  private dependencies(values: readonly TeamTaskId[], state: TeamFoldState, self?: TeamTaskId): TeamTaskId[] {
    ...
    for (const id of values) {
      ...
      const task = state.tasks.get(id)
      if (task === undefined || task.status === 'deleted') throw new TeamError(`blocker task "${id}" not found`, 'TEAM_TASK_NOT_FOUND')
      seen.add(id)
      result.push(id)
    }
    return result
