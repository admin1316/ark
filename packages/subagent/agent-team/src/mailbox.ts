...
import { TeamId, TeamMessageId } from './brand.ts'
import { TeamError, errorMessage } from './error.ts'
import type { TeamJournal } from './journal.ts'
import type { TeamRuntimeLifecycle } from './lifecycle.ts'
import { resolveActiveMember, type TeamRoster } from './roster.ts'
import { messageAccepted } from './session-message.ts'
import type { SendTeamMessageRequest, SendTeamMessageResult, TeamMessageSnapshot, TeamMessageSource } from './types.ts'
...
export class TeamMailbox {
  ...
  async recoverFor(agent: Agent, signal: AbortSignal): Promise<void> {
    ...
    for (const message of messages) {
      ...
      if (membership.role === 'lead' && message.delivery === 'quiet' && message.targetId !== membership.root.id
        && this.ctx.agents.get(message.targetId) === undefined) continue
   ...
   */
  pendingDispatches(): readonly Promise<unknown>[] { return [...this.inFlightDispatches] }
  ...
  private tryDispatch(root: Agent, message: TeamMessageSnapshot, signal: AbortSignal): Promise<boolean> {
    ...
    const forget = (): void => { this.inFlightMessages.delete(message.id) }
  ...
  private trackDispatch<T>(operation: Promise<T>): Promise<T> {
    ...
    const forget = (): void => { this.inFlightDispatches.delete(operation) }
    void operation.then(forget, forget)
    return operation
  }

  private async tryDispatchAdmitted(root: Agent, message: TeamMessageSnapshot, signal: AbortSignal): Promise<boolean> {
    ...
    if (active !== undefined && live !== undefined && message.delivery === 'quiet'
      && this.messagePrecedes(root, message.id, active.id)) return await this.dispatchOnce(root, message, signal)
    return await this.serializeDispatch(message, () => this.dispatchOnce(root, message, signal))
  }

  private async serializeDispatch(message: TeamMessageSnapshot, operation: () => Promise<boolean>): Promise<boolean> {
    ...
    const dispatch = async (): Promise<boolean> => {
      ...
      finally { this.activeDispatches.delete(targetId) }
    }
    const run = prior.then(dispatch, dispatch)
    const tail = run.then(() => undefined, () => undefined)
    this.dispatchTails.set(targetId, tail)
    try { return await run }
    finally { if (this.dispatchTails.get(targetId) === tail) this.dispatchTails.delete(targetId) }
  }
  ...
  private async dispatchOnce(root: Agent, message: TeamMessageSnapshot, signal: AbortSignal): Promise<boolean> {
    try {
      ...
      const source: TeamMessageSource = {
  ...
  private async checkpointDelivered(root: Agent, target: Session, messageId: TeamMessageId): Promise<boolean> {
  ...
  private markDelivered(root: Agent, messageId: TeamMessageId, targetId: SessionId): Promise<void> {
  ...
  private targetRecorded(session: Session, messageId: TeamMessageId): boolean {
    return messageAccepted(session.events.slice(session.header.seedLength ?? 0),
      message => message.source.kind === 'team-message' && message.source.messageId === messageId)
  }
  ...
  private async persistedTargetRecorded(targetId: SessionId, messageId: TeamMessageId, signal: AbortSignal): Promise<boolean | undefined> {
    try {
      const stored = await this.ctx.sessionPersistence.inspect(targetId, signal)
      return messageAccepted(stored.events.slice(stored.meta.seedLength ?? 0),
        message => message.source.kind === 'team-message' && message.source.messageId === messageId)
    } catch (error) {
