...
function parsePersisted<T>(type: string, schema: z.ZodType<T>, value: unknown): T {
  try { return schema.parse(value) }
  catch (error) { throw new Error(`persisted Agent Teams ${type} payload is invalid`, { cause: error }) }
}
...
export function applyTeamEvent(state: TeamFoldState, event: SessionEvent): void {
  ...
  parsePersisted<unknown>(event.type, teamEventSchemas[event.type], event.data)
  ...
  switch (decoded.type) {
    case 'team/member': {
      ...
      if (named !== undefined && named !== member.id) throw new Error(`teammate name "${member.name}" is reused by another member`)
      if (prior === undefined) {
    ...
    case 'team/task': {
      ...
      if (prior === undefined && task.revision !== 1) throw new Error(`team task "${task.id}" must begin at revision 1`)
      if (prior !== undefined && task.revision !== prior.revision + 1) throw new Error(`team task "${task.id}" revision is not contiguous`)
