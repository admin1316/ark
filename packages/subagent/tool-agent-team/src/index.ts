...
import { TeamTaskId, type TeamMembership } from '@deepseek-ai/dsh-agent-team'
...
function jsonOutput<const O extends ValueSchemaSpec>(schema: O) {
  return {
    schema,
    render: (_args: unknown, value: InferValue<O>) => [{ type: 'text' as const, text: JSON.stringify(value) }],
  }
}
...
function install(
  scoped: Context, ctx: Context, config: Required<Config>, membershipForPrompt: () => TeamMembership | undefined,
): () => void {
  const disposers: (() => void)[] = []
  const register = (dispose: () => void): void => { disposers.push(dispose) }
  try {
    register(scoped.systemPrompt.section({
      name: 'team:policy', order: 60,
      text: () => {
        const membership = membershipForPrompt()
        return membership === undefined ? POLICY
    ...
    register(scoped.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return await ctx.agentTeams.spawnTeammate(agent, {
          name: args.name, description: args.description, prompt: [{ type: 'text', text: args.prompt }], context,
          provider: context === 'fork' ? config.forkProvider : config.freshProvider, signal: exec.signal,
        })
      },
    })))
    const messageTool = (toolName: string, delivery: 'quiet' | 'wakeup'): void => {
      register(scoped.tools.register(defineTool({
        name: toolName,
        ...
        execute(args, exec) {
          return ctx.agentTeams.sendMessage(callingAgent(exec.agent, toolName), {
            target: args.target, content: [{ type: 'text', text: args.message }], delivery, signal: exec.signal,
          })
        },
      })))
    }
    ...
    register(scoped.tools.register(defineTool({
      name: 'list_agents', description: 'List the Lead and every durable teammate with current runtime status.', parameters: {},
      output: jsonOutput(MEMBER_LIST_VALUE_SCHEMA),
      execute(_args, exec) { return Promise.resolve(ctx.agentTeams.listMembers(callingAgent(exec.agent, 'list_agents'))) },
    })))
    register(scoped.tools.register(defineTool({
      name: 'wait_agent',
      ...
      parameters: { timeout_ms: { type: 'integer', description: 'Wait duration in milliseconds, from 10000 through 3600000. Defaults to 30000.' } },
      ...
      async execute(args, exec) {
        ...
        if (!ctx.agentTeams.listMembers(caller).some(member => member.id !== caller.id && ACTIVE_WAIT_STATUSES.has(member.status))) {
          return { timedOut: false, noProgress: { reason: 'no-active-peer' as const, message: NO_ACTIVE_PEER_MESSAGE } }
        }
        return await ctx.agentTeams.waitForChange(caller, timeoutMs, exec.signal)
      },
    })))
    register(scoped.tools.register(defineTool({
      ...
      execute(args, exec) { return Promise.resolve(ctx.agentTeams.interrupt(callingAgent(exec.agent, 'interrupt_agent'), args.target)) },
    })))
    register(scoped.tools.register(defineTool({
    ...
    register(scoped.tools.register(defineTool({
      ...
      output: jsonOutput(TASK_VIEW_SCHEMA),
      execute(args, exec) { return Promise.resolve(ctx.agentTeams.getTask(callingAgent(exec.agent, 'team_task_get'), TeamTaskId(args.task_id))) },
    })))
    register(scoped.tools.register(defineTool({
      ...
      parameters: {
        ...
        action: { type: 'string', required: true, enum: ['claim', 'release', 'edit', 'set_dependencies', 'complete', 'reopen', 'reassign', 'delete'], description: 'Task transition to apply.' },
      ...
      async execute(args, exec) {
        return await ctx.agentTeams.updateTask(callingAgent(exec.agent, 'team_task_update'), {
          taskId: TeamTaskId(args.task_id), expectedRevision: args.expected_revision, action: args.action,
...
export function apply(ctx: Context, config: Config = {}): void {
  const resolved: Required<Config> = { freshProvider: config.freshProvider ?? 'spawn', forkProvider: config.forkProvider ?? 'fork' }
  const compositionScope = scopeOf(ctx)
  const installed = new Map<Agent, () => void>()
  const belongsToComposition = (agent: Agent): boolean =>
    compositionScope === undefined || scopeChainOf(scopeOf(agent.ctx)).includes(compositionScope)
  const uninstall = (agent: Agent): void => {
    installed.get(agent)?.()
    installed.delete(agent)
  }
  const maybeInstall = (agent: Agent): void => {
    if (!belongsToComposition(agent) || installed.has(agent) || ctx.agentTeams.tryMembership(agent) === undefined) return
