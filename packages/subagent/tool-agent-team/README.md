...
The plugin listens to Agent publication and installs registrations through each matching Agent scope. In a scoped composition it claims only Agents whose scope parent chain contains that composition, so sibling presets never register into one another's Agents. A blank Agent recompose removes the leaving composition's rows before the entering composition installs its rows. Fresh creation and cold resume therefore receive the selected preset's tool/prompt set before the first model request. Agent disposal and plugin HMR remove every owned registration; reloading the plugin reinstalls still-live matching members without changing continuation Activations.
