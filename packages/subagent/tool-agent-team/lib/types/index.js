...
function install(scoped, ctx, config, membershipForPrompt) {
    const disposers = [];
    const register = (dispose) => { disposers.push(dispose); };
    try {
        ...
        register(scoped.tools.register(defineTool({
            ...
            execute(args, exec) {
                const filtered = ctx.agentTeams.listTasks(callingAgent(exec.agent, 'team_task_list')).filter(task => (args.status === undefined || task.status === args.status)
...
export function apply(ctx, config = {}) {
    const resolved = { freshProvider: config.freshProvider ?? 'spawn', forkProvider: config.forkProvider ?? 'fork' };
    const compositionScope = scopeOf(ctx);
    const installed = new Map();
    const belongsToComposition = (agent) => compositionScope === undefined || scopeChainOf(scopeOf(agent.ctx)).includes(compositionScope);
    const uninstall = (agent) => {
        installed.get(agent)?.();
        installed.delete(agent);
    };
    const maybeInstall = (agent) => {
        if (!belongsToComposition(agent) || installed.has(agent) || ctx.agentTeams.tryMembership(agent) === undefined)
            return;
