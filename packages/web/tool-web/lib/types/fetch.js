...
function computeFetchOutput(result, maxOutputChars) {
    ...
    if (full.length <= maxOutputChars)
        return { text: full, markdown, truncated, markdownTruncated };
    if (maxOutputChars < TRUNCATION_FOOTER.length)
        return { text: full.slice(0, maxOutputChars), markdown, truncated, markdownTruncated };
 ...
 */
export function formatFetchOutputState(result, maxOutputChars) {
    if (!Number.isSafeInteger(maxOutputChars) || maxOutputChars < 0)
        throw new RangeError('maxOutputChars must be a non-negative safe integer');
