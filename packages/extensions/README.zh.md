---
description: "agent 修改自身运行时：检查已加载的插件与服务接口、定义并运行模型编写的动态包（dynamic package）并再次撤下，外加受限 repository Plugin 运行时。"
kind: "package-group"
---
...
[运行时扩展参考](../../docs/subsystems/extensions.zh.md)定义动态包执行、检查与生命周期拆卸。
