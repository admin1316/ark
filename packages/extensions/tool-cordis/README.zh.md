---
description: "自引用 Cordis 工具集：五个面向模型的工具，操作当前 DSH 进程中的实时运行时。"
...
- [信任立场](#trust-stance)
- [配置](#config)
- [API 报告从哪里来](#where-the-api-report-comes-from)
- [渲染](#rendering)
- [导出形式](#export-shape)
...
<a id="where-the-api-report-comes-from"></a>
## API 报告从哪里来
...
每个工具都渲染 `generic` 卡片（`read`／`execute`／delete`）；`cordis_define` 以 `rawInput` 携带提交的 Host 源码，并用标签与用途作为卡片标题。presenter 是 args 的纯函数，结果保留默认文本渲染。Native shell 使用 generic 卡片，从调用参数与结果元数据里读取标签、用途和铸出的标识。
