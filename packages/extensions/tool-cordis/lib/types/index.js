...
export function apply(ctx) {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'List every Host Cordis Inspect Provider. Each entry includes its platform, purpose, read-only methods, and '
...
function inspectSelfPackage(ctx, agent, pluginId, packageId) {
    ...
    if (hostStatus === undefined) {
        hostStatus = 'stopped';
        if (active !== undefined)
            hostStatus = hostWaiting.length === 0 ? 'running' : 'waiting';
    }
    return {
        mode: 'package',
