...
export function apply(ctx: Context): void {
  ctx.systemPrompt.section({ name: 'tool:cordis', order: FIRST_PARTY_SECTION_ORDER.TOOL_CORDIS, text: CORDIS_SYSTEM_PROMPT })
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'List every Host Cordis Inspect Provider. Each entry includes its platform, purpose, read-only methods, and '
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'to read exact Service methods, Event modes, Builtin signatures, or Tool schemas. Queries run locally. '
      ...
      + 'referenced types.',
    parameters: {
      platform: { type: 'string', required: true, const: 'host', description: 'Host runtime platform.' },
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'summary. Only pluginId plus packageId returns that immutable Package\'s Host source and runtime '
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'code.host is a plain JavaScript function body that returns a Cordis Plugin; no TypeScript or import '
      + 'transformation occurs. Query Inspect before depending on a Service, Event, Builtin, or Tool. Define only '
      + 'validates parameters and syntax and records source; it does not execute apply or change currentPackageId. '
      + 'On success, call cordis_run with the '
      + 'returned IDs.',
    parameters: {
      plugin: {
        required: true,
      ...
      code: {
        ...
        required: true,
        properties: {
          host: { type: 'string', required: true, description: 'Plain JavaScript function body that returns the Host Cordis Plugin.' },
        },
      },
    },
    output: {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'even if the Plugin is currently stopped. The Host activation completes before this Tool returns. '
      + 'currentPackageId changes only after success; after failure inspect the exact Package, correct the same '
      + 'Plugin, and retry autonomously.',
    parameters: {
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const status = requireJsonString(result, 'status')
        return [{
          type: 'text',
          text: `${pluginId}/${packageId} is ${status} (${pluginRunId}).`,
        }]
      },
...
type SelfState = 'defined' | 'stopped' | 'running' | 'waiting' | 'failed'
...
function selfState(reference: DynamicCordisReference): SelfState {
  ...
  if (status === 'starting-host') return 'defined'
...
function inspectSelfPackage(
  ...
  let hostStatus = latest?.host.status
  if (hostStatus === undefined) {
    hostStatus = 'stopped'
    if (active !== undefined) hostStatus = hostWaiting.length === 0 ? 'running' : 'waiting'
  }
  return {
    mode: 'package',
...
function referencedPluginIds(messages: readonly UserMessage[]): string[] {
  ...
  for (const message of messages) {
    ...
    for (const match of text.matchAll(pattern)) found.add(match[0].slice(1))
  }
  return [...found]
}
