 ...
 * with its metadata and Host lifecycle state. Session-scoped like every runner verb.
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  return rows.flatMap((row) => {
    ...
    const packages = row.packages.map((pkg) => {
      ...
      if (active === undefined) return `    - ${pkg.packageId}: ${pkg.name} — ${pkg.purpose}`
      ...
      return `    - ${pkg.packageId}: ${pkg.name} [${state}, ${active.pluginRunId}] — ${pkg.purpose}`
