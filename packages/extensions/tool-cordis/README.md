---
description: "The self-referential Cordis toolset: five model-facing tools over the live runtime in the current DSH process."
...
- [Config](#config)
- [Where the API report comes from](#where-the-api-report-comes-from)
- [Rendering](#rendering)
...
Every tool renders a `generic` card (`read` / `execute` / `delete`); `cordis_define` carries the submitted Host source as `rawInput` and titles the card with the label and purpose. Presenters are pure functions of the args, and results keep the default text rendering. The Native shell uses the generic card and reads the label, purpose, and minted id from the call arguments and result metadata.
