...
function syntaxErrorContextOr(error: unknown, fallback: Error): string {
 ...
 */
export function parseErrorMessage(half: 'code.host', context: string): string {
 ...
 */
export function precheckCode(code: string, half: 'code.host'): void {
  const wrapped = `(async () => {\n${code}\n})()`
  try {
 ...
 */
function prettyParseContext(wrapped: string, half: 'code.host', refusal: Error): string {
  let vmError: unknown
  try {
