...
| [`api/`](api/README.zh.md) | 原生 API 的 Host 侧身份与事件转发策略 | 产品：稳定 API |
...
| [`host/`](host/README.zh.md) | 原生/API Host 层：API 网关、鉴权传输与 loopback 路由服务器 | 产品：稳定 API |
...
**扩展插件依赖 Service Definition，绝不依赖具体提供方。** `dsh-agent-loop` 可替换；集成、钩子和工具插件使用 `dsh-agent`。包括 `dsh-agent-spine-demo` 在内的组合包可以依赖主干插件。能力会将需要独立演进的 Service Definition／Service Provider／Consumer 角色分离；详见[能力 seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.zh.md)。
