...
if (snapshotMode !== 'replay') {
    // Layered env with bootstrap-name rejection: a .env must never redirect the
    // harness home or inject process-launch variables (DSH_*, NODE_OPTIONS,
    // LD_PRELOAD, ...). Mirrors the product CLI track.
    loadLayeredEnv(NAME);
}
