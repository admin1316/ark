...
export class TeamMailbox {
  ...
  private async dispatchOnce(root: Agent, message: TeamMessageSnapshot, signal: AbortSignal): Promise<boolean> {
    try {
      ...
      return target === undefined
        ? true
        : await this.checkpointRecorded(root, target.session, message.id)
    } catch (error: unknown) {
  ...
  private async checkpointDelivered(
    ...
    return await this.checkpointRecorded(root, target, messageId)
  }

  /**
   * Record the delivered edge for a receipt whose durability barrier the caller
   * already crossed. A continuable-subagent delivery returns only after its own
   * Session flush, so flushing again here would repeat the barrier for the same
   * message and blur the "accepted means durable" boundary.
   */
  private async checkpointRecorded(
    root: Agent,
    target: Session,
    messageId: TeamMessageId,
  ): Promise<boolean> {
