...
export class JsonlSessionPersistence extends SessionPersistence {
    ...
    async withMutation(id, operation) {
        ...
        if (!(await lstat(locks)).isDirectory())
            throw new Error(`invalid session writer lock directory: "${locks}"`);
    ...
    async deleteStored(id) {
        return this.withMutation(id, async () => {
            ...
            for (const project of await this.listProjectDirs()) {
                ...
                try {
                    if (!(await lstat(root)).isDirectory())
                        throw new Error(`invalid session deletion directory: "${root}"`);
                    const pending = join(root, encodeSegment(id));
                    if (!(await lstat(pending)).isDirectory())
                        throw new Error(`invalid session deletion tombstone: "${pending}"`);
            ...
            if (tombstones.length > 1)
                throw new Error(`duplicate session deletion tombstones for "${id}"`);
            for (const pending of tombstones)
                await this.removeTombstone(pending);
            ...
            if (process.platform === 'win32')
                await ensureDurableDirectoryWin32(tombstoneRoot);
            else
                await mkdir(tombstoneRoot, { recursive: true, mode: 0o700 });
            if (!(await lstat(tombstoneRoot)).isDirectory())
                throw new Error(`invalid session deletion directory: "${tombstoneRoot}"`);
            const tombstone = join(tombstoneRoot, encodeSegment(id));
            if (process.platform === 'win32')
                await publishNewFileWin32(directory, tombstone);
            else {
