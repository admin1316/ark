...
物理修改共享 `~locks` 下按 ID 隔离的跨进程锁。删除先把普通会话目录及所属附件移动到项目内隐藏的 `~delete` 暂存目录，再移除它。重复删除会完成遗留目录的清理；符号链接形式的删除根目录会被拒绝。共享写锁不会猜测已有锁是否过期：遗留锁需要检查所有权并由运维明确恢复，之后才能继续修改。
