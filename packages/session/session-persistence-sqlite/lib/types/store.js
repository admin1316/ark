...
export class SqliteStore {
        ...
        try {
            ...
            if (closers.length > 0) {
                ...
                for (const closer of closers)
                    this.insertRecord(insert, sessionKey, bindRecord(closer));
            }
            this.incrementRevision(meta.id);
            this.db.exec(sql('commit'));
        }
    ...
    writeRow(meta) {
        const inserted = this.db.prepare(sql('upsert-session')).get(meta.id, meta.version, meta.createdAt, meta.cwd ?? null, meta.parentSession ?? null, meta.seedLength ?? null, meta.origin ?? null, meta.delegationDepth ?? null, meta.agentPreset ?? null, randomUUID());
        return inserted.id;
    }
}
