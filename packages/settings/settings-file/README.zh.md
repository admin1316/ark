...
`dsh-settings-file` 把所有 namespace 的用户设置保存在一个 YAML 或 JSON 文档中，默认是 harness home 下的 `settings.yaml`：用户可以直接编辑文档——变更实时生效——也可以经服务写入，后者保留同级变更并拒绝命名空间内的冲突编辑。YAML 写入保留每个未触碰节点上的注释、锚点与排版，未加载插件所拥有的分节也绝不会被丢弃。启动时非法文档直接报错；运行中失败的热重载保留最后可用分节并告警，而不是拖垮进程。
...
经 `ctx.settings` 的写入会在写锁内重新读取文档。未观察到的同级 namespace 变更会被保留；若目标 namespace 不同于写入方捕获的分节，则拒绝写入，调用方须刷新后重试。YAML 编辑是叶子级 diff：只设置变化的值、只删除被移除的键，因此每个未触碰节点以及每个被改键值对的键上的注释、锚点与排版都得以保留；被改的数组或其他非 map 值整体替换。JSON 文档重新序列化，无注释。若磁盘上的文档已变为非法，写入会明确报错，而不是覆盖用户的手工编辑。
...
- 文档缺失即空存储；删除文件即回到该状态。
- 磁盘编辑变为非法后，运行中的读取保留最后可用分节；写入拒绝覆盖该文档。
...
- **冲突的 namespace 编辑要求刷新**——提供方在文件锁内比较目标分节，而不合并冲突值。服务的进程内修订号与此磁盘比较各有职责；两者都不会静默覆盖未观察到的编辑。
