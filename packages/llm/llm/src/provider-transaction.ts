...
import { isObject, symbols, type Context } from '@deepseek-ai/cordis'
import { CredentialConflictError, credentialCondition, credentialKey, credentialRef } from '@deepseek-ai/dsh-credentials'
import type { CredentialCondition, CredentialProvider, CredentialRecord, CredentialRef } from '@deepseek-ai/dsh-credentials'
...
import type { RemoteSettingsJsonValue, RemoteSettingsPathOp } from '@deepseek-ai/dsh-settings/types'
import { TypertRemoteFailure } from '@deepseek-ai/dsh-typert-protocol'
import type { LlmRuntime } from './index.ts'
import type {
  LlmConfigurableProvider, RemoteLlmCredentialMutation, RemoteLlmProviderMutationRequest, RemoteLlmProviderMutationResult,
  RemoteLlmProviderResumeRequest, RemoteLlmProviderTransactionRequest, RemoteLlmProviderTransactionResult,
} from './types.ts'

type Json = RemoteSettingsJsonValue
type Op = RemoteSettingsPathOp
type ProviderOwnership = Pick<LlmConfigurableProvider, 'provider' | 'settingsNs' | 'settingsPath'>
type Failure = { code: string; message: string; details: Record<string, Json> }
type Phase = 'prepared' | 'credential-staged' | 'settings-applied' | 'credential-applied' | 'done'
type Outcome = 'committed' | 'rolled-back' | 'committed-not-live'
type CredentialPlan = ({ op: 'set'; ref: string; valueDigest: string } | { op: 'unset'; ref: string }) & {
  /** Absent only in retained plans that cannot prove ownership of an earlier value. */
  before?: CredentialCondition
}
interface Plan {
  settingsPath: string[]
  ops: Op[]
  expectedRevision: number
  /** Captured raw user section, for restart-safe CAS without inventing a revision. */
  expectedUserDigest?: string
  /** Original input binding retained by endpoint-normalized legacy plans. */
...
} & ({ outcome: 'committed'; error?: never } | { outcome: Exclude<Outcome, 'committed'>; error: Failure })
type Journal = {
  version: 1
  receiptVersion: 1
  ...
  outcome?: Outcome
  error?: Failure
  completed: Record<string, Receipt>
} & ({ phase: Exclude<Phase, 'done'> } | { phase: 'done'; outcome: Outcome })
interface JournalSnapshot {
  stored: CredentialRecord | undefined
  journal: Journal | undefined
}
interface RecoverySnapshot extends JournalSnapshot {
  journal: Journal
}
...
const resourceTails = new WeakMap<object, Map<string, Promise<void>>>()
const execution = new AsyncLocalStorage<{ active: boolean }>()

/** Reserve each resource tier atomically: provider journal, namespace, then credential references. */
async function withResources<T>(owner: object, keys: readonly string[], operation: () => Promise<T>): Promise<T> {
  const original: unknown = Reflect.get(owner, symbols.original)
  const identity = isObject(original) ? original : owner
  let tails = resourceTails.get(identity)
  if (tails === undefined) { tails = new Map(); resourceTails.set(identity, tails) }
  ...
  try { await Promise.all(previous); return await operation() }
  finally {
    lease.resolve()
...
function fail(code: string, message: string, details: Record<string, Json> = {}): never {
...
function hashJson(value: object): string {
  return hash(JSON.stringify(value, (_key, item: unknown) => record(item)
    ? Object.fromEntries(Object.keys(item).sort().map(key => [key, item[key]])) : item))
}
function strings(value: unknown): value is string[] {
  return Array.isArray(value) && value.every(part => typeof part === 'string')
}
function pathOp(value: unknown): value is Op {
  return record(value) && strings(value.path)
    && (value.op === 'unset' || (value.op === 'set' && Object.hasOwn(value, 'value')))
}
function pathOps(value: unknown): value is Op[] {
  return Array.isArray(value) && value.every(pathOp)
}
function canonicalOps(ops: readonly Op[]): Op[] {
  return ops.map(op => op.op === 'unset' ? { op: 'unset', path: [...op.path] }
    : { op: 'set', path: [...op.path], value: op.value })
}
function prefix(path: readonly string[], base: readonly string[]): boolean {
  return base.length <= path.length && base.every((part, index) => path[index] === part)
}
function ref(value: string): CredentialRef {
  try { return credentialRef(value) }
  catch { return fail('input-invalid', 'credential reference must be a valid environment name', { field: 'credential.ref' }) }
}
function snapshot(value: unknown): Json {
  try { return snapshotSettingsJson(value) }
  catch { return fail('input-invalid', 'provider mutation must contain only lossless JSON data') }
}
function requestSnapshot(input: RemoteLlmProviderMutationRequest): RemoteLlmProviderMutationRequest {
  ...
  try { settingsNamespace(value.settingsNs) }
  catch { return fail('input-invalid', 'provider mutation namespace is invalid', { field: 'settingsNs' }) }
  ...
  let credential: RemoteLlmCredentialMutation | undefined
  if (value.credential !== undefined) {
    ...
    ref(item.ref)
    if (item.op === 'unset') credential = { op: 'unset', ref: item.ref }
  ...
  if (value.ops.length === 0 && credential === undefined) return fail('settings-rejected', 'provider mutation must change settings, a credential, or both')
...
function transactionIdentity(input: RemoteLlmProviderTransactionRequest): RemoteLlmProviderTransactionRequest {
  const value = snapshot(input)
...
function pathValue(value: unknown, path: readonly string[]): { present: boolean; value?: unknown } {
  let current = value
...
function edit(value: unknown, op: Op, path = op.path): unknown {
  const [head, ...rest] = path
  if (head === undefined) return op.op === 'unset' ? {} : structuredClone(op.value)
...
function apply(value: unknown, ops: readonly Op[]): unknown {
  return ops.reduce<unknown>((current, op) => edit(current, op), structuredClone(value ?? {}))
}
function satisfied(value: unknown, ops: readonly Op[]): boolean {
  return ops.every((op) => {
    if (op.op === 'unset' && op.path.length === 0) return value === undefined || deepEqualJson(value, {})
...
function references(value: unknown, path: readonly string[]): Map<string, string[][]> {
  const profile = pathValue(value, path).value
  const refs = new Map<string, string[][]>()
  if (!record(profile)) return refs
  const add = (value: unknown, tail: string[]) => {
...
function fingerprint(provider: ProviderOwnership, value: unknown): string {
...
function makePlan(provider: ProviderOwnership, value: unknown, request: RemoteLlmProviderMutationRequest, configured: boolean): Plan {
  ...
  if (credential?.op === 'set' && (configured || references(value, provider.settingsPath).has(credential.ref))) {
    ...
    for (const path of references(after, provider.settingsPath).get(credential.ref) ?? []) {
      const owner = ops.find((op): op is Extract<Op, { op: 'set' }> => op.op === 'set' && prefix(path, op.path))
      const replacement: Op = { op: 'set', path, value: versionRef }
      if (owner === undefined) ops.push(replacement)
      else ops[ops.indexOf(owner)] = { ...owner, value: snapshot(edit(owner.value, replacement, path.slice(owner.path.length))) }
    }
...
function inputDigest(request: RemoteLlmProviderMutationRequest): string {
...
function plannedInputDigest(provider: string, settingsNs: string, plan: Plan): string {
  return hashJson({ provider, settingsNs, ops: plan.ops, credential: plan.credential })
}
function legacyPlanDigest(provider: string, settingsNs: string, plan: Plan): string {
...
function planDigest(provider: string, namespace: string, plan: Plan): string {
...
function storedFailure(value: unknown): Failure | undefined {
...
function parseReceipt(value: unknown): Receipt {
  ...
  if (value.outcome === 'committed') {
    if (error !== undefined) return fail('provider-transaction-in-doubt', 'provider receipt outcome is inconsistent')
    return { ...identity, outcome: value.outcome }
  }
  if (error === undefined) return fail('provider-transaction-in-doubt', 'provider receipt outcome is inconsistent')
  return { ...identity, outcome: value.outcome, error }
}
function parseLegacyInput(value: unknown): LegacyInput | undefined {
  if (value === undefined) return undefined
...
function parseJournal(input: CredentialRecord | undefined): Journal | undefined {
  if (input === undefined) return undefined
  if (input.kind !== 'grant') return fail('provider-transaction-in-doubt', 'provider journal has an unsupported record kind')
  let value: Json
  try { value = snapshotSettingsJson(input.payload) }
  catch { return fail('provider-transaction-in-doubt', 'provider journal is not valid JSON data') }
  ...
  if (legacy && plan.expectedUserDigest !== undefined) return fail('provider-transaction-in-doubt', 'legacy provider plan cannot assert a current before-image')
  try { settingsNamespace(value.settingsNs) }
  catch { return fail('provider-transaction-in-doubt', 'provider journal namespace is invalid') }
  ...
  let credential: CredentialPlan | undefined
  if (plan.credential !== undefined) {
    ...
    try { credentialRef(data.ref) }
    catch { return fail('provider-transaction-in-doubt', 'provider credential reference is invalid') }
    if (data.op === 'unset') credential = { op: 'unset', ref: data.ref }
  ...
  const parsedPlan: Plan = { settingsPath: plan.settingsPath, ops: plan.ops, expectedRevision: plan.expectedRevision,
  ...
  if (ops.length === 0 && credential === undefined) return fail('provider-transaction-in-doubt', 'provider plan contains no operation')
  if (ops.length > 64 || ops.some((op, index) => ops.some((other, otherIndex) =>
    index !== otherIndex && prefix(op.path, other.path)))) return fail('provider-transaction-in-doubt', 'provider plan operations overlap or exceed the limit')
  ...
  if (expectedDigest !== value.digest) return fail('provider-transaction-in-doubt', 'provider journal digest does not match its plan')
  ...
  const completed: Record<string, Receipt> = {}
  if (!legacy) {
  ...
  const identity = { version: 1 as const, receiptVersion: 1 as const, transactionId: value.transactionId, provider: value.provider,
...
export class ProviderTransactions {
  ...
  constructor(private readonly ctx: Context, private readonly runtime: LlmRuntime) {
   ...
   */
  async mutate(input: RemoteLlmProviderMutationRequest, signal: AbortSignal): Promise<RemoteLlmProviderMutationResult> {
    const request = requestSnapshot(input)
    return this.run(request.provider, signal, (settings, credentials) =>
      withResources(settings, [request.settingsNs], () => this.execute(settings, credentials, request, signal)))
  }
   ...
   */
  async status(input: RemoteLlmProviderTransactionRequest): Promise<RemoteLlmProviderTransactionResult> {
    const request = transactionIdentity(input)
    return this.track(() => this.query(request))
  }

  private async query(request: RemoteLlmProviderTransactionRequest): Promise<RemoteLlmProviderTransactionResult> {
    const credentials = this.ctx.get('credentials')
    if (credentials === undefined) return fail('service-unavailable', 'provider transaction journal is unavailable')
    const { journal } = await this.read(credentials, request.provider)
    if (journal === undefined) return { state: 'absent', needsCredential: false }
    const current = journal.transactionId === request.transactionId
    const receipt = journal.completed[request.transactionId]
    let state: Exclude<Phase, 'done'> | Outcome
    if (current) state = journal.phase === 'done' ? journal.outcome : journal.phase
    else {
      if (receipt === undefined) return { state: 'absent', needsCredential: false }
   ...
   */
  async resume(input: RemoteLlmProviderResumeRequest, signal: AbortSignal): Promise<RemoteLlmProviderMutationResult> {
    const request = transactionIdentity(input)
    const value = snapshot(input)
    ...
    return this.run(request.provider, signal, async (settings, credentials) => {
      ...
      return withResources(settings, [journal.settingsNs], async () => {
        ...
        if (journal.phase !== 'done' && journal.transactionId === request.transactionId && !unverifiableWrite) {
          ...
          if (planned?.op === 'unset') credential = { op: 'unset', ref: planned.ref }
  ...
  private async run<T>(
    ...
    action: (settings: SettingsProvider, credentials: CredentialProvider) => Promise<T>,
  ): Promise<T> {
    ...
    const credentials = this.ctx.get('credentials')
    if (settings === undefined || credentials === undefined) return fail('service-unavailable', 'provider mutation requires settings and credentials owners')
    return this.track(() => withResources(credentials, [`journal:${provider}`], async () => {
      if (this.stopped) return fail('service-unavailable', 'provider transaction owner is stopped')
      if (signal.aborted) return fail('cancelled', 'provider transaction was cancelled before durable claim')
      const scope = { active: true }
      try { return await execution.run(scope, () => action(settings, credentials)) }
  ...
  private async execute(
    settings: SettingsProvider, credentials: CredentialProvider,
    request: RemoteLlmProviderMutationRequest, signal: AbortSignal, restoring?: RecoverySnapshot,
  ): Promise<RemoteLlmProviderMutationResult> {
    ...
    let declaration: ProviderOwnership | undefined = this.runtime.listConfigurableProviders()
    ...
    if (declaration === undefined) return fail('settings-rejected', 'provider does not own the requested settings namespace')
    ...
    if (previous !== undefined) {
      ...
      if (receipt !== undefined) {
        if (restoring === undefined && !this.matches(receipt, request)) return fail('provider-transaction-in-doubt', 'transaction id was reused with different input')
    ...
    if (before === undefined) return fail('settings-rejected', 'provider settings namespace is not registered')
    ...
    return withResources(credentials, referenceKeys, async () => {
      ...
      this.preflight(settings, declaration, before.value, plan, request,
        replay !== undefined && satisfied(before.user, plan.ops))
      if (plan.credential !== undefined && !(await credentials.describe(ref(plan.credential.ref))).writable) return fail('credential-rejected', 'provider credential is read-only')
      if (signal.aborted) return fail('cancelled', 'provider transaction was cancelled before durable claim')
      let journal = replay ?? { version: 1 as const, receiptVersion: 1 as const, transactionId: request.transactionId,
        provider: request.provider, settingsNs: request.settingsNs, requestDigest: digest,
        digest: planDigest(request.provider, request.settingsNs, plan), plan, phase: 'prepared' as const,
        completed: { ...previous?.completed } }
      await this.claim(credentials, captured, journal, signal)
      const advance = async (phase: Exclude<Phase, 'done'>) => {
        const next = { ...journal, phase }
        await this.write(credentials, journal, next)
        journal = next
      }
      const finish = async (...result: ['committed'] | [Exclude<Outcome, 'committed'>, Failure]) => {
        ...
        const receipt: Receipt = outcome === 'committed' ? { ...identity, outcome } : { ...identity, outcome, error }
        const next: Journal = { ...journal, ...receipt, phase: 'done',
      ...
      const rollback = async (failure: Failure) => {
        if (plan.credential?.op === 'set' && plan.credential.before?.valueDigest === null) {
          try { await credentials.unset(ref(plan.credential.ref), { valueDigest: plan.credential.valueDigest }) }
          catch (error) {
      ...
      const credentialConflict = async () => {
        const failure: Failure = { code: 'credential-rejected', message: 'provider credential changed before commit', details: { provider: request.provider } }
      ...
      if (journal.phase === 'credential-staged') {
        ...
        if (current === undefined) return fail('provider-transaction-in-doubt', 'provider settings owner disappeared')
        if (!satisfied(current.user, plan.ops)) {
          ...
          try { await settings.mutate(namespace, plan.ops, revision) }
          catch (error) {
      ...
      if (!accepted || this.runtime.listProviders().some(provider => provider.id === request.provider) !== expectedLive) {
        const failure: Failure = { code: 'provider-registration-rejected', message: 'provider settings were stored but did not activate', details: { provider: request.provider } }
        await finish('committed-not-live', failure)
        throw new TypertRemoteFailure(failure)
      }
      ...
      if (journal.phase === 'settings-applied') {
        if (plan.credential?.op === 'unset') {
          try { await this.applyCredential(credentials, plan.credential, request.credential) }
          catch (error) {
  ...
  private checkReceipt(receipt: Receipt): void {
    if (receipt.outcome !== 'committed') throw new TypertRemoteFailure(receipt.error)
  }

  private matches(receipt: Pick<Receipt, 'requestDigest' | 'legacyInput'>, request: RemoteLlmProviderMutationRequest): boolean {
    if (receipt.requestDigest === inputDigest(request)) return true
    if (receipt.legacyInput === undefined) return false
    const credential = request.credential === undefined ? undefined : request.credential.op === 'unset'
      ? { op: 'unset' as const, ref: request.credential.ref }
      : { op: 'set' as const, ref: request.credential.ref, valueDigest: hash(request.credential.value) }
  ...
  private async read(credentials: CredentialProvider, provider: string): Promise<JournalSnapshot> {
    try {
      ...
      if (journal !== undefined && journal.provider !== provider) return fail('provider-transaction-in-doubt', 'provider journal ownership is invalid')
  ...
  private async claim(credentials: CredentialProvider, captured: JournalSnapshot, next: Journal, signal: AbortSignal): Promise<void> {
    try {
      await credentials.modifyRecord(credentialKey('llm-remote', next.provider), (current) => {
        ...
        const proposed: CredentialRecord = { kind: 'grant', payload: next }
  ...
  private async resolveCredential(credentials: CredentialProvider, reference: string) {
    try { return await credentials.resolve(ref(reference)) }
    catch { return fail('service-unavailable', 'provider credential is unavailable') }
  }

  private preflight(
    settings: SettingsProvider, declaration: ProviderOwnership, before: unknown,
    plan: Plan, request: RemoteLlmProviderMutationRequest, settingsAlreadyApplied: boolean,
  ): void {
    ...
    const uses = this.runtime.listConfigurableProviders().flatMap(entry =>
      [...references(values.get(entry.settingsNs), entry.settingsPath).keys()]
    ...
    for (const ref of afterRefs.keys()) {
      ...
      if (!unchanged && !(plan.credential?.op === 'set' && plan.credential.ref === ref)) return fail('credential-ownership-required', 'a new endpoint reference requires an explicit credential value')
    }
  ...
  private async write(
    credentials: CredentialProvider, expected: Journal, next: Journal,
    condition?: { ref: CredentialRef; expected: CredentialCondition },
  ): Promise<void> {
    try {
      await credentials.modifyRecord(credentialKey('llm-remote', next.provider), (current) => {
        const previous = parseJournal(current)
        if (!deepEqualJson(previous, expected)) return fail('provider-transaction-in-doubt', 'provider journal ownership changed during commit')
        return Promise.resolve({ kind: 'grant', payload: next })
      }, condition === undefined ? [] : [condition])
    } catch (error) { if (error instanceof TypertRemoteFailure || error instanceof CredentialConflictError) throw error; return fail('provider-transaction-in-doubt', 'provider transaction progress could not be persisted') }
  }

  private async applyCredential(
    credentials: CredentialProvider, plan: CredentialPlan, supplied: RemoteLlmCredentialMutation | undefined,
  ): Promise<void> {
    const reference = ref(plan.ref)
    const current = await credentials.resolve(reference)
    if (plan.op === 'unset') {
      if (current === undefined) return
      if (plan.before === undefined) return fail('provider-transaction-in-doubt', 'credential removal has no durable before-image')
      try { await credentials.unset(reference, plan.before) }
      catch (error) {
        if (error instanceof CredentialConflictError) throw error
        if (await credentials.resolve(reference) !== undefined) return fail('provider-transaction-in-doubt', 'credential removal did not complete')
      }
      return
    }
    if (current !== undefined && hash(current.value) === plan.valueDigest) return
    if (current !== undefined) return fail('provider-transaction-in-doubt', 'the staged credential reference now holds a different value')
    ...
    try { await credentials.set(reference, supplied.value, { valueDigest: null }) }
    catch {
      const after = await credentials.resolve(reference)
      if (after === undefined || hash(after.value) !== plan.valueDigest) return fail('provider-transaction-in-doubt', 'credential staging did not complete')
    }
  }

  private conflict(ns: string, expected: number, actual: number): never {
    return fail('settings-conflict', 'provider settings revision changed', { ns, expected, actual })
  }
  private settingsFailure(ns: string, error: unknown): Failure {
  ...
  private async result(
    settings: SettingsProvider, credentials: CredentialProvider,
    request: RemoteLlmProviderMutationRequest, settingsPath: readonly string[], credentialRef?: string,
  ): Promise<RemoteLlmProviderMutationResult> {
    const descriptor = settings.describe({ redactSecrets: true }).find(entry => entry.ns === request.settingsNs)
    if (descriptor === undefined) return fail('provider-registration-rejected', 'committed provider settings are unavailable')
