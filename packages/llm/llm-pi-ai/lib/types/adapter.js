...
import { verifyExactModel } from "./verification.js";
...
function requestHeaders(headers, credentialHeaders) {
...
export class PiAiAdapter extends LlmAdapter {
    ...
    profileOf(snapshot, provider) {
        ...
        if (profile.migrationRequired !== undefined)
            throw new LlmError('provider credential headers require migration to credential references', 'MISSING_CREDENTIAL');
        return profile;
    }
    ...
    async verifyProvider(provider, model, signal) {
        ...
        if (provider === 'bailian-cn' || provider === 'bailian-intl')
            return undefined;
        if (resolved.api !== 'openai-completions' && resolved.api !== 'openai-responses')
            return undefined;
        ...
        if (apiKey === undefined && Object.keys(credentials).length === 0)
            return undefined;
        const headers = requestHeaders(profile.headers, credentials);
        if (apiKey !== undefined && !Object.keys(headers).some(name => name.toLowerCase() === 'authorization'))
