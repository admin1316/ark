...
async function verifyMetadata(response, provider, model) {
    ...
    if (response.body === null)
        throw new LlmError('provider verification returned no metadata', 'VERIFICATION_FAILED');
    ...
    if (!('id' in data) || data.id !== model)
        throw new LlmError('provider verification returned a different model', 'INVALID_MODEL_INFO');
    if ('provider' in data && data.provider !== provider)
        throw new LlmError('provider verification returned a different provider', 'INVALID_MODEL_INFO');
}
/**
...
export async function verifyExactModel(request) {
    ...
    const aborted = () => { if (signal.aborted)
        throw new LlmError('provider metadata verification aborted', 'ABORTED'); };
    aborted();
    let response;
    try {
        response = await fetch(url, { method: 'GET', headers: request.headers, signal, redirect: 'manual' });
    }
    catch {
        aborted();
    ...
    let challenge;
    try {
        challenge = await fetch(url, { method: 'GET', headers, signal, redirect: 'manual' });
    }
    catch {
        aborted();
