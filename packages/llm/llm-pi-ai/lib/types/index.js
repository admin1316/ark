...
import { assertWritableConfig, Config, ProviderProfileSchema, resolveProfiles } from "./config.js";
...
function directoryEntries(profiles, base) {
    ...
    for (const [provider, profile] of profiles)
        declare(provider, profile.displayName, profile);
...
export function apply(ctx, config) {
    ...
    const resolveCredentialHeaders = async (_provider, profile) => {
        ...
        for (const [header, reference] of Object.entries(profile.credentialHeaders ?? {})) {
            ...
            if (value === undefined || value.length === 0)
                throw new LlmError('credential-backed provider header is not configured', 'MISSING_CREDENTIAL');
            try {
    ...
    ctx.llm.registerModelDiscovery(NS, (request, signal) => discoverModels({ ...request, ...signal === undefined ? {} : { signal } }));
