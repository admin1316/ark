...
const THINKING_BUDGET_FIELD_GATE = {
...
export const THINKING_BUDGET_FIELDS = Object.keys(THINKING_BUDGET_FIELD_GATE);
...
function catalogProviders() {
    ...
    if (qwen === undefined)
        throw new Error('Bailian presets require the installed Qwen protocol owner');
    ...
    if (models.length !== modelIds.size)
        throw new Error('Bailian preset model capabilities are missing');
    for (const [id, name, baseUrl] of BAILIAN_PRESETS) {
        const presetModels = models.map(model => ({ ...model, provider: id, baseUrl,
            compat: { ...model.compat, maxTokensField: 'max_tokens' } }));
        providers.set(id, {
            id, name, baseUrl,
            auth: { apiKey: {
                    name,
...
export function catalogModels(provider) {
    ...
    if (owner === undefined)
        return new Map();
    const models = BAILIAN_PRESETS.some(([id]) => id === provider)
        ? owner.getModels()
...
function resolveModelCompat(provider, entry, route, base, api) {
    ...
    const compat = { ...inherited, ...configured };
