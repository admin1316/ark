...
async function discoverModels(request) {
	if (request.provider !== void 0 && (request.baseURL ?? "").length === 0) {
	...
	if (request.apiKey !== void 0 && request.credentialEndpointFingerprint !== modelDiscoveryEndpointFingerprint(request.baseURL, api)) throw new LlmError("model discovery credential is not bound to this endpoint", "INVALID_DISCOVERY_CREDENTIAL_SCOPE");
	const apiKey = request.apiKey === void 0 ? void 0 : usableProbeKey(request.apiKey);
	let response;
	try {
		response = await fetch(url, {
			method: "GET",
