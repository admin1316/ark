...
describe('syncTools', () => {
  ...
  it.each([['page-1', 'page-1'], ['page-1', 'page-2', 'page-1']])
  ('rejects repeated discovery cursors while preserving callable tools and allowing recovery: %j', async (...cursors) => {
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair()
    let phase: 'initial' | 'cycle' | 'recovered' = 'initial'
    let pages = 0
    serverTransport.onmessage = (message) => {
      ...
      if (message.method === 'initialize') {
        ...
        if (phase === 'cycle') {
          const nextCursor = cursors[pages++]
          if (nextCursor === undefined) {
            void serverTransport.send({ jsonrpc: '2.0', id: message.id, error: { code: -32603, message: 'pagination exceeded script' } })
            return
          }
          result = { tools: [], nextCursor }
        } else result = { tools: [{ name: phase, inputSchema: { type: 'object' } }] }
    ...
    const client = new Client({ name: 'pagination-consumer', version: '1' })
    await client.connect(clientTransport)
    try {
      const previous = await syncTools(client, ctx, defaultOpts, new Map())
      const original = ctx.tools.get('mcp__srv__initial')
      phase = 'cycle'
      await expect(syncTools(client, ctx, defaultOpts, previous)).rejects.toThrow(/repeated.*cursor/)
      expect(pages).toBe(cursors.length)
      expect(ctx.tools.get('mcp__srv__initial')).toBe(original)
      const result = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('pagination-retained'),
        name: 'mcp__srv__initial', arguments: {} })
      expect(result.error).toBeUndefined()
      expect(result.content).toEqual([{ type: 'text', text: 'ok' }])
      phase = 'recovered'
      const recovered = await syncTools(client, ctx, defaultOpts, previous)
      expect([...recovered.keys()]).toEqual(['mcp__srv__recovered'])
      expect(ctx.tools.get('mcp__srv__initial')).toBeUndefined()
      const retained = ctx.tools.get('mcp__srv__recovered')
      await expect(syncTools(client, ctx, { ...defaultOpts, registrationFailure: 'throw' }, new Map())).rejects.toThrow()
      expect(ctx.tools.get('mcp__srv__recovered')).toBe(retained)
