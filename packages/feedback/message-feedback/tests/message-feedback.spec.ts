...
describe('MessageFeedbackService durability ordering', () => {
  it('purges the durable sidecar after an admitted mutation and cannot be resurrected late', async () => {
    const { ctx, persistence } = await harness()
    const fixture = messageFixture('permanent-delete-sidecar')
    persistence.persist(fixture.session)
    const messageId = fixture.assistantMessageIds[0]
    const first = expectItem(await ctx.messageFeedback.put({
      sessionId: fixture.session.id,
    ...
    const internals = ctx.messageFeedback as unknown as {
      table: KvTable<SessionId, MessageFeedbackRow>
    }
    expect(internals.table.get(fixture.session.id)?.items).toHaveLength(1)

    const writeStarted = Promise.withResolvers<undefined>()
    const releaseWrite = Promise.withResolvers<undefined>()
    const originalPut = internals.table.put.bind(internals.table)
    const putSpy = vi.spyOn(internals.table, 'put').mockImplementation(async (id, row) => {
      writeStarted.resolve(undefined)
      await releaseWrite.promise
      await originalPut(id, row)
    })
    const lateMutation = ctx.messageFeedback.put({
      sessionId: fixture.session.id,
      messageId,
      rating: 'negative',
      ifVersion: first.version,
    ...
    const cleanup = ctx.parallel('session-persistence/deleted', fixture.session.id)
    releaseWrite.resolve(undefined)

    expectItem(await lateMutation)
    await cleanup
    putSpy.mockRestore()
    expect(internals.table.get(fixture.session.id)).toBeUndefined()
  })
