...
export class MessageFeedbackService extends TypertRemoteService {
  ...
  protected async [Service.init](): Promise<void> {
    ...
    // Persistence owns the Session lifecycle. Queue deletion behind every
    // already-admitted mutation for this id so a late sidecar put cannot
    // recreate bytes after permanent deletion reports success.
    this.ctx.on('session-persistence/deleted', (sessionId: SessionId) =>
      this.purgeDeletedSession(sessionId))
  }
  ...
  /**
   * Delete one Session's durable feedback sidecar behind every previously
   * admitted mutation. The persistence coordinator awaits this hook and
   * re-emits it on idempotent deletion retries.
   */
  private purgeDeletedSession(sessionId: SessionId): Promise<void> {
