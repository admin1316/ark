...
const name = "tool-knowledge-wiki";
...
function wikiRootRelativePath(input) {
	if (input === "" || input.startsWith("/") || input.startsWith("wiki/") || input.includes("\\") || input.includes("\0")) throw new Error("path must be relative to the Wiki root (for example entities/name.md)");
	if (input.split("/").some((part) => part === "" || part === "." || part === "..")) throw new Error("path must not contain traversal segments");
	return input;
}
/**
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: { query: {
			type: "string",
			required: true,
			description: "The search query (keywords or a question)."
		} },
		output: {
			schema: {
				type: "object",
		...
		async execute(args) {
			...
			if (service === void 0) throw new Error("knowledgeWiki service unavailable");
			return { hits: await service.search({
				query: args.query,
				topK: SEARCH_MAX_RESULTS
			}) };
		}
	}));
	ctx.tools.register(defineTool({
		...
		async execute(_args) {
			...
			return {
				files: entries.slice(0, FILES_MAX).map((entry) => entry.path),
	...
	ctx.tools.register(defineTool({
		...
		parameters: { path: {
			type: "string",
			required: true,
			description: "Project-relative page path."
		} },
		output: {
			schema: {
				type: "object",
		...
		async execute(args) {
			...
			return {
				path: page.path,
				content: page.content.slice(0, READ_MAX_CHARS),
				truncated
			};
		}
	}));
	ctx.tools.register(defineTool({
		...
		parameters: {
			query: {
				type: "string",
				description: "Filter nodes whose label or id contains this text."
			},
			nodeType: {
				type: "string",
				description: "Filter by node type (entity/concept/source/finding/…)."
			},
			limit: {
				type: "number",
		...
		async execute(args) {
			...
			const nodes = graph.nodes.filter((node) => {
				if (t !== void 0 && node.type !== t) return false;
				if (q !== void 0 && q !== "" && !node.label.toLowerCase().includes(q) && !node.id.toLowerCase().includes(q)) return false;
				return true;
			}).slice(0, limit);
			for (const node of nodes) matchedIds.add(node.id);
			return {
				nodes,
				edges: graph.edges.filter((edge) => matchedIds.has(edge.source) && matchedIds.has(edge.target))
			};
		}
	}));
	ctx.tools.register(defineTool({
		...
		async execute(_args) {
			const service = wikiService(ctx);
			if (service === void 0) throw new Error("knowledgeWiki service unavailable");
			return { reviews: (await service.reviews({
				status: "unresolved",
				limit: REVIEW_MAX
			})).map((review) => ({
				id: review.id,
				title: review.title,
				type: review.type,
				...review.description === void 0 ? {} : { description: review.description }
			})) };
		}
	}));
	ctx.tools.register(defineTool({
		...
		parameters: { input: {
			type: "string",
			required: true,
			description: "Project-relative source path or http(s) URL."
		} },
		output: {
			schema: {
				type: "object",
			...
			render: (_args, value) => {
				...
				return textBlock(tasks.length === 0 ? "No task was queued." : tasks.map((task) => `- #${task.id} ${task.status}: ${task.input}`).join("\n"));
			}
		},
		async execute(args) {
			const service = wikiService(ctx);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			reviewId: {
				type: "string",
				required: true,
				description: "Candidate review id returned by wiki_reviews."
			},
			action: {
				type: "string",
				required: true,
		...
		output: {
			...
			render: (_args, value) => textBlock(value.ok ? `Candidate verified by the trusted owner (${value.receiptId}).` : `Candidate verification failed${value.errorCode ? `: ${value.errorCode}` : "."}`)
		},
		async execute(args, exec) {
			const service = wikiService(ctx);
			if (service === void 0) throw new Error("knowledgeWiki service unavailable");
			return service.verifyCandidate({
