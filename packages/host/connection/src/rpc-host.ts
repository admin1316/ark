...
import { API_PATH, RESPOND_PATH } from './api-path.ts'
import type {
...
interface ConnectionEventRegistration {
  readonly source: ConnectionEventSource
  readonly active: Set<AbortController>
}

interface ConnectionResponseRegistration {
  readonly handler: ConnectionResponseHandler
  readonly active: Set<AbortController>
}

interface ConnectionDownloadRegistration {
  readonly handler: ConnectionDownloadHandler
  readonly options: ConnectionRpcHandlerOptions
  readonly active: Set<DownloadLifetime>
}
...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  private readonly eventSources = new Map<ConnectionEventChannel, ConnectionEventRegistration>()
  private readonly downloadHandlers = new Map<string, ConnectionDownloadRegistration>()
  private responseHandler: ConnectionResponseRegistration | undefined
  ...
  get downloads(): HostConnectionHandle['downloads'] {
    ...
    return {
      handle: (path, handler, options) =>
        this.registerDownloadHandler(owner, path, handler, options),
    }
  }
  ...
  async * openEventStream(
    ...
    signal: AbortSignal,
  ): AsyncIterable<ConnectionEventFrame> {
    ...
    const abort = (): void => { lifetime.abort(signal.reason) }
    ...
    return {
      fetch: (request) => {
        ...
        if (pathname === RESPOND_PATH) {
          ...
          if (registration === undefined) return Promise.resolve(new Response('not found', { status: 404 }))
  ...
  private registerEventSource(
    ...
    const registration: ConnectionEventRegistration = { source, active: new Set() }
    return owner.effect(() => {
      ...
      return () => {
        ...
        for (const lifetime of registration.active) lifetime.abort(
          new Error(`host-connection: ${channel} event source was disposed`),
        )
  ...
  private registerResponseHandler(
    ...
    const registration: ConnectionResponseRegistration = {
      handler,
      active: new Set(),
    }
    return owner.effect(() => {
      ...
      return () => {
        if (this.responseHandler === registration) this.responseHandler = undefined
        for (const lifetime of [...registration.active]) lifetime.abort(
          new Error('host-connection: /api/respond handler was disposed'),
        )
        registration.active.clear()
      }
  ...
  private registerDownloadHandler(
    owner: Context,
    path: string,
    handler: ConnectionDownloadHandler,
    options: ConnectionRpcHandlerOptions,
  ): () => Promise<void> {
    assertDownloadPath(path)
    const registration: ConnectionDownloadRegistration = {
      handler,
      options,
      active: new Set(),
    }
  ...
  private register(
    ...
    return owner.effect(
      ...
      `host-connection: ${channel} rpc channel`,
    )
  }
...
async function downloadFetch(
  request: Request,
  registration: ConnectionDownloadRegistration,
): Promise<Response> {
...
async function responseFetch(
  request: Request,
  registration: ConnectionResponseRegistration,
): Promise<Response> {
  ...
  return withRequestLifetime(request, registration.active, async (signal) => {
    try {
      ...
      const receipt: ConnectionResponseReceipt = await registration.handler(body, signal)
      signal.throwIfAborted()
...
async function withRequestLifetime(
  request: Request,
  active: Set<AbortController>,
  use: (signal: AbortSignal) => Promise<Response>,
): Promise<Response> {
  const lifetime = new AbortController()
  const abort = (): void => { lifetime.abort(request.signal.reason) }
...
/** One download retained by its registration until all body work becomes quiescent. */
interface DownloadLifetime {
  readonly signal: AbortSignal
  abort(reason: unknown): Promise<void>
  finish(reason?: unknown): boolean
  fail(error: unknown): boolean
}

/** Track request cancellation without releasing owner activity before the body settles. */
function createDownloadLifetime(request: Request, active: Set<DownloadLifetime>): DownloadLifetime {
  const controller = new AbortController()
  let finished = false
  let failed = false
  let failure: unknown
  let resolveFinished!: () => void
  const settled = new Promise<void>((resolve) => { resolveFinished = resolve })
  const settle = (reason: unknown, error?: { readonly value: unknown }): boolean => {
  ...
  const lifetime: DownloadLifetime = {
  ...
  const abortFromRequest = (): void => { void lifetime.abort(request.signal.reason).catch(() => {}) }
  active.add(lifetime)
...
async function streamDownloadResponse(
  ...
  let terminal = false
  const beginTerminal = (): boolean => {
    if (terminal) return false
    terminal = true
    lifetime.signal.removeEventListener('abort', abort)
    return true
  }
  const releaseReader = (): void => { reader.releaseLock() }
...
function rpcFetchHandler(
  ...
  return {
    async fetch(request: Request): Promise<Response> {
      ...
      if (message === undefined) return invalidEnvelopeResponse(body)
...
function invalidEnvelopeResponse(body: unknown): Response {
...
function parseClientRequest(value: unknown): ConnectionClientRequest | undefined {
  ...
  if (candidate.type !== 'client-request'
    ...
    || !Object.hasOwn(candidate, 'payload')) return undefined
  return {
    type: 'client-request',
...
function errorResponse(rpcId: string, error: ConnectionRpcError): Response {
  return fullResponse(rpcId, { ok: false, error })
}

function fullResponse(rpcId: string, result: ConnectionRpcResult<unknown>): Response {
...
function assertDownloadPath(path: string): void {
