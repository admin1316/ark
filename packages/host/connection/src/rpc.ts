...
export type ConnectionRpcResult<T> =
  ...
  | { readonly ok: false; readonly error: ConnectionRpcError }
...
export type ConnectionResponseHandler = (
  message: unknown,
  signal: AbortSignal,
) => ConnectionResponseReceipt | Promise<ConnectionResponseReceipt>

/** Exact non-RPC download or export handler mounted below `/api`. */
export type ConnectionDownloadHandler = (
  request: Request,
  signal: AbortSignal,
) => Promise<Response>
