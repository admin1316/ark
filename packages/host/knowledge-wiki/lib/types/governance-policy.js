...
import { mergeCandidateIntoCanonical } from "./canonical-merge.js";
import { readRegularFileBounded } from "./filesystem.js";
 ...
 */
export function resolveGovernedWikiPath(root, input, allowMissing) {
 ...
 */
export function decideCandidateGovernance(wikiRoot, candidatePath, content, suggestedTarget) {
...
function scoreQuality(candidatePath, title, body, content) {
    ...
    if (shortCommand)
        reasons.push('title is a conversational command');
    if (pastedPlaceholder)
        reasons.push('pasted-image placeholder');
    if (incidentCandidate)
        reasons.push('incident history belongs in Evidence or Archive, not Canonical');
    if (reflectionCandidate && !reflectionComplete)
        reasons.push('reflection is missing failure, cause, counterfactual, prevention, or applicability');
    if (reflectionCandidate && epistemicStatus !== 'verified')
        reasons.push('reflection remains a hypothesis until independently verified');
    if (reflectionCandidate && independentSourceCount < 2)
        reasons.push('reflection has fewer than two independent sources');
    if (compact.length < 120)
        reasons.push('knowledge body is too short');
    if (!hasSource)
        reasons.push('candidate has no source evidence');
    if (!hasIndependentSource)
        reasons.push(hasSource ? 'session or workspace context is not independent evidence' : 'candidate has no independent source evidence');
    if (sessionQuestion && !durableSignal)
        reasons.push('generic session Q&A without durable project knowledge');
    if (transcriptPollution)
        reasons.push('conversation transcript markers remain in the body');
    if (processMatches >= 3)
        reasons.push('tool and progress log pollution');
    if (unstableHistory)
        reasons.push('local path, build artifact, or revision detail is not stable knowledge');
    if (assistantClaim && !verification)
        reasons.push('assistant completion claim has no independent verification');
    ...
    if (hardReject)
        return { score: 0, reasons, hardReject, autoEligible: false };
    let score = 0;
    if (compact.length >= 180 && compact.length <= 12_000)
        score += 1;
    if (hasIndependentSource)
        score += 2;
    if (durableSignal)
        score += 2;
    if (/^#{2,3}\s+|^\s*[-*]\s+|^\s*\d+[.]\s+/mu.test(body))
        score += 1;
    if (title.length >= 4 && title.length <= 48 && !sessionQuestion)
        score += 1;
    if (verification)
        score += 1;
    ...
    if (!autoEligible)
        reasons.push('candidate remains in review because one or more canonical admission gates failed');
...
function markdownFiles(root) {
    ...
    for (const entry of readdirSync(root, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            output.push(...markdownFiles(full));
...
function ngrams(value, size) {
    ...
    for (let i = 0; i <= value.length - size; i += 1)
        output.add(value.slice(i, i + size));
    return output;
}
...
//# sourceMappingURL=governance-policy.js.map