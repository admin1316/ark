...
import { sourceIdentityForPath, sourceSummaryFileNameFromIdentity } from "./source-slug.js";
import { stampSourcesField } from "./frontmatter-utils.js";
import { sanitizeIngestedFileContent, stampGeneratedFrontmatterDates, stampGeneratedLogDate } from "./sanitize.js";
import { buildFallbackSourceSummaryPage, fallbackSummaryRelPath } from "./fallback-summary.js";
import { atomicWriteFile, isMissingPathError } from "./filesystem.js";
import { mergePageContent } from "./merge-page.js";
import { appendCandidateReviews, appendReviews, parseReviewBlocks } from "./reviews.js";
...
function languageRule(sourceContent) {
    ...
    if (creative && !engineering)
        return 'MANDATORY OUTPUT LANGUAGE: Chinese（简体中文）';
    if (engineering && !creative)
...
function candidateIngestRel(rel) {
    if (rel === CANDIDATE_LOG_REL || rel.startsWith(`${CANDIDATE_INGEST_PREFIX}/`))
        return rel;
    if (!rel.startsWith('wiki/'))
        throw new Error(`ingest: FILE path must start with wiki/: ${rel}`);
    if (rel === 'wiki/log.md')
        return CANDIDATE_LOG_REL;
...
function stampCandidateFrontmatter(content) {
    if (!content.startsWith('---\n'))
        return content;
    let stamped = content;
    if (/^status:\s*/mu.test(stamped))
        stamped = stamped.replace(/^status:\s*.*$/mu, 'status: candidate');
    else
        stamped = stamped.replace(/^---\n/u, '---\nstatus: candidate\n');
    if (/^origin:\s*/mu.test(stamped))
        stamped = stamped.replace(/^origin:\s*.*$/mu, 'origin: ingest');
    else
        stamped = stamped.replace(/^---\n/u, '---\norigin: ingest\n');
    return stamped;
}
 ...
 */
export async function completeText(llm, provider, model, prompt, operation = 'ingest', signal) {
 ...
 */
export async function ingestSource(executor, provider, model, projectPath, sourceRel, signal) {
    signal.throwIfAborted();
    const warnings = [];
    ...
    if (analysis.text === null)
        throw new Error('ingest analysis returned no text');
    ...
    if (generated.text === null)
        throw new Error('ingest generation returned no text');
    const written = [];
