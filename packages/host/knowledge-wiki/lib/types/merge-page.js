 ...
 */
import { canonicalizeSourcesField, parseFrontmatterArray, parseFrontmatterBlock, parseFrontmatterField, renderCanonicalFrontmatterField, renderPreservedFrontmatterField, } from "./frontmatter-utils.js";
/**
 ...
 */
export function isOwnedOnlyBySource(existingContent, identity) {
 ...
 */
export function mergePageContent(existingContent, newContent, identity, today) {
    if (isOwnedOnlyBySource(existingContent, identity))
        return newContent;
    ...
    const stamped = payloadLines
        ...
        if (field?.key === 'sources')
            return renderCanonicalFrontmatterField(field, serialized);
...
//# sourceMappingURL=merge-page.js.map