...
import { atomicWriteFile, isMissingPathError, readRegularFileBounded } from "./filesystem.js";
 ...
 */
export function updateWikiIndexDeterministically(indexPath, wikiRelativePaths) {
...
function readIndexLines(indexPath) {
    try {
...
function appendNewSection(indexPath, lines, wikiRelativePaths) {
...
function trimToRecentCap(lines, headingIndex) {
    let entries = 0;
    const kept = [];
    for (const [index, line] of lines.entries()) {
        if (index > headingIndex && indexEntryTarget(line) !== null) {
            entries += 1;
            if (entries > MAX_RECENT_ENTRIES)
                continue;
        }
        kept.push(line);
    }
...
function pageTitle(indexPath, wikiRelativePath) {
