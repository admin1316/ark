 ...
 */
export function normalizeConfinedRelativePath(input) {
 ...
 */
export function resolveConfinedPath(root, input, allowMissingLeaf) {
    ...
    if (!rootStat.isDirectory() || rootStat.isSymbolicLink())
        throw new Error('configured root is not an ordinary directory');
    ...
    for (const [index, part] of rel.split('/').entries()) {
        ...
        try {
            ...
            if (stat.isSymbolicLink())
                throw new Error(`symbolic link is not allowed: ${cursor}`);
            if (index < rel.split('/').length - 1 && !stat.isDirectory()) {
                throw new Error(`non-directory path ancestor: ${cursor}`);
            }
        }
        catch (error) {
            if (!isMissingPathError(error) || !allowMissingLeaf)
                throw error;
            break;
        }
    }
 ...
 */
export function ensureConfinedDirectory(root, input) {
    const rel = normalizeConfinedRelativePath(input);
    const base = resolve(root);
    const rootStat = lstatSync(base);
    if (!rootStat.isDirectory() || rootStat.isSymbolicLink())
    ...
    for (const part of rel.split('/')) {
        ...
        try {
            ...
            if (!stat.isDirectory() || stat.isSymbolicLink())
                throw new Error(`unsafe directory path: ${cursor}`);
        }
        catch (error) {
            ...
            if (!created.isDirectory() || created.isSymbolicLink())
                throw new Error(`unsafe created directory: ${cursor}`);
        }
    }
    return cursor;
}
function ensureAbsoluteDirectory(path) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink())
        throw new Error(`unsafe directory path: ${absolute}`);
    return absolute;
}
 ...
 */
export function readConfinedText(root, input, maxBytes = MAX_WIKI_PAGE_BYTES) {
 ...
 */
export function createPrivateFileIfMissing(path, content) {
    const parent = ensureAbsoluteDirectory(dirname(path));
    assertOrdinaryDestination(path);
    let descriptor;
    try {
 ...
 */
export function durableUnlinkFile(path) {
    let stat;
    try {
        stat = lstatSync(path);
    }
 ...
 */
export function readOptionalText(path, maxBytes = MAX_WIKI_PAGE_BYTES) {
    try {
        return readRegularFileBounded(path, maxBytes).toString('utf8');
    }
    catch (error) {
 ...
 */
export function inodeIdentity(path) {
    const stat = lstatSync(path);
 ...
 */
export function assertAbsolutePathInside(root, child) {
