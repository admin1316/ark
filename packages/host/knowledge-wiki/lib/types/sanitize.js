 ...
 */
import { parseFrontmatterBlock, parseFrontmatterField, renderCanonicalFrontmatterField, renderPreservedFrontmatterField, } from "./frontmatter-utils.js";
 ...
 */
export function sanitizeIngestedFileContent(content) {
    let cleaned = content;
...
function addMissingOpeningFrontmatterFence(content) {
    ...
    if (!FRONTMATTER_FIELD_RE.test(first))
        return content;
...
function repairWikilinkListsInFrontmatter(content) {
    ...
    const repaired = block.body
        ...
        if (field === null || !/^\[\[[^\]]+\]\](?:\s*,\s*\[\[[^\]]+\]\])+$/u.test(field.value))
            return line;
 ...
 */
export function stampGeneratedFrontmatterDates(content, today) {
    const block = parseFrontmatterBlock(content);
    if (block === null)
        return content;
    const stamped = block.body
        ...
        .map((line) => {
        const field = parseFrontmatterField(line);
        if (field === null || (field.key !== 'created' && field.key !== 'updated'))
            return line;
 ...
 */
export function stampGeneratedLogDate(entry, today) {
