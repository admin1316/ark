...
function dedupeMessageParts(parts: string[]): string[] {
  const out: string[] = []
 ...
 */
export function extractTurnPair(events: unknown[], sessionId: string, turn: number): TurnPair | null {
  ...
  for (const ev of events) {
    ...
    if (type === 'user/message') {
      ...
      if (t.trim()) userParts.push(t.trim())
      ...
      if (t.trim()) outputParts.push(t.trim())
  ...
  if (!input || !output || output.length < MIN_BODY) return null
 ...
 */
export function titleFrom(input: string, sessionTitle: string | undefined): string {
...
const PROBLEM_SIGNS =
  /修复|解决|根因|排查|故障|报错|错误|原因|导致|验证|确认|坑|异常|失败|回退|回滚|变回|恢复|生效|失效|重启|定位|原因在于|没反应|打不开|显示空白|不生效/u
/** 缺陷抱怨词（口语化，与上拆行避免 max-len）。 */
const DEFECT_SIGNS =
  /太[^，。\s！!]{0,4}|不对|错了|问题|不正常|奇怪|卡住|不见了|没有颜色|不显示|显示不出/u
 ...
 */
export function classifyKind(input: string, output: string): 'concept' | 'problem-solving' {
 ...
 */
export function buildPage(title: string, pair: TurnPair, today: string, workspaceName: string, kind: 'concept' | 'problem-solving' = 'concept'): string {
 ...
 */
export function slugFrom(title: string, sessionId: string, turn: number): string {
...
export async function classifyWithLlm(
  ...
  input: string,
  output: string,
): Promise<{ kind: 'concept' | 'problem-solving'; title: string | null } | null> {
  const text = await complete(
    KIND_CLASSIFY_PROMPT + `${input.slice(0, 500)}\n助手：${output.slice(0, 1500)}`,
  )
  const m = text.match(/\{[\s\S]*\}/u)
  if (!m) return null
  let decoded: unknown
  try { decoded = JSON.parse(m[0]) } catch { return null }
 ...
 */
export function pageExists(wikiRoot: string, rel: string): boolean {
 ...
 */
export function readPageContent(wikiRoot: string, rel: string): string {
 ...
 */
export function extractConversationText(events: unknown[]): string {
  const parts: string[] = []
  let currentTurn = 0
 ...
 */
export function sessionSummarySlug(title: string, sessionId: string): string {
  const base = title.replace(/[\\/:*?"<>|\s]+/gu, '-').replace(/-+/gu, '-').replace(/^-|-$/gu, '').slice(0, 30)
