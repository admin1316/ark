...
interface WikiPage {
  /** Project-relative path, e.g. wiki/concepts/工具调用规范.md. */
  readonly path: string
  /** Display title from frontmatter, else the file stem. */
  readonly title: string
  /** Frontmatter type (entity/concept/source/finding/...), else other. */
  readonly nodeType: string
  /** Outgoing wikilink targets (raw text inside [[...]]). */
  readonly links: string[]
  /** `related` frontmatter entries. */
  readonly related: string[]
  /** Page body text (frontmatter stripped), for search. */
...
function parsePageContent(raw: string): {
  ...
  if (block === null) return { type: undefined, title: undefined, related: [], body: raw }
 ...
 */
export function visitWikiTree(
  wikiRoot: string,
  visitor: {
    readonly onDirectory?: (entry: { name: string; path: string }) => void
    readonly onMarkdown?: (entry: { name: string; path: string; fullPath: string; size: number }) => void
  },
): void {
  let rootStat
  try {
  ...
  const walk = (dir: string, relPrefix: string): void => {
    ...
    for (const name of entries) {
      ...
      if (st.isDirectory()) {
        ...
        if (st.size > MAX_WIKI_PAGE_BYTES) throw new Error(`Wiki page exceeds 5 MiB: ${path}`)
...
interface GraphPageState {
  ...
  readonly neighbors: Map<GraphPageState, number>
  degree: number
  incoming: number
...
interface WeightedEdge {
  readonly source: GraphPageState
  readonly target: GraphPageState
  weight: number
}

interface TargetLookup {
  readonly byPath: Map<string, GraphPageState>
  readonly byTitle: Map<string, GraphPageState>
  readonly byStem: Map<string, GraphPageState>
}

/** Build first-match lookup tables matching the legacy traversal semantics. */
function buildTargetLookup(states: GraphPageState[]): TargetLookup {
  const byPath = new Map<string, GraphPageState>()
  const byTitle = new Map<string, GraphPageState>()
  const byStem = new Map<string, GraphPageState>()
...
function resolveTarget(target: string, lookup: TargetLookup): GraphPageState | undefined {
 ...
 */
function louvain(nodes: GraphPageState[]): void {
  const total = nodes.reduce((sum, node) => sum + node.degree, 0)
  if (total === 0) return

  const moveNode = (node: GraphPageState): boolean => {
    const current = node.community
    const k = node.degree
    current.degree = Math.max(0, current.degree - k)
    const gains = new Map<CommunityState, number>()
  ...
  for (let pass = 0; pass < 12; pass++) {
    ...
    for (const node of nodes) {
      if (moveNode(node)) moved = true
    }
    if (!moved) break
  }
  // Compact community ids.
  const compact = new Map<CommunityState, number>()
  for (const node of nodes) {
...
export function buildGraph(wikiRoot: string): WikiGraphResult {
  ...
  const rawEdges = new Map<string, WeightedEdge>()
  const addEdge = (source: GraphPageState, target: GraphPageState): void => {
    ...
    if (existing === undefined) rawEdges.set(key, { source: first, target: second, weight: 1 })
  ...
  for (const source of states) {
    ...
    for (const target of targets) {
      ...
      if (resolved === undefined || seen.has(resolved.page.path)) continue
  ...
  const nodes: GraphNode[] = states.map(state => ({
    id: state.page.path,
  ...
  const edges: GraphEdge[] = [...rawEdges.values()].map(edge => ({
  ...
  const communitiesInfo: CommunityInfo[] = [...communityNodes.entries()]
    .map(([id, members]) => ({
      id,
...
export function listPages(wikiRoot: string): Array<{
  ...
  const hasDescendant = (path: string): boolean =>
    out.some(entry => entry.path.startsWith(`${path}/`))
 ...
 */
export function readPage(wikiRoot: string, relPath: string): string {
