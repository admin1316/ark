...
function upgradeApiTokenAuthorized(req, apiToken) {
    if (apiToken === undefined || apiToken === '')
        return true;
    if (apiTokenAuthorized(req, apiToken))
        return true;
...
export class WebServer extends Service {
    ...
    async [Service.init]() {
        ...
        if (this.config.apiOnly !== true) {
            ...
            this.tapIndex((html, req) => {
                if (this.config.host === '0.0.0.0'
                    && (req === undefined || !indexTokenAuthorized(req, apiToken)))
                    return html;
        ...
        this.ctx.effect(() => async () => {
            const servers = [this.server, this.apiServer].filter((server) => server !== undefined);
            const serversClosed = servers.map(server => new Promise((resolve) => {
