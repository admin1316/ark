...
| `$DSH_HOME/.credentials.yaml` (`file`) or macOS login Keychain (`keychain`) | `file` or `keychain` | yes (`set`/`unset`) | over both `.env` layers |
...
| `mode` | `file` | Writable reference backend: `file`, or `keychain` on macOS. |
| `keychainService` | `dsh.credentials` | Generic-password service used by `keychain` mode. |

## The document

A versioned YAML document with one section per key space, and nothing else. In `keychain` mode, `refs` live in the login Keychain while structured `records` remain in this document:
...
In `file` mode the document is `0600` under a `0700` directory, which stops other OS users — **not** the model. Tool processes (bash, the filesystem tools) run as the same user, and the shipped `workspace-write` file policy confines mutations rather than reads, so they can read this file exactly like any other file the user owns; no sandbox mode singles it out. What the harness does hold to is narrower: it never hands the model a resolved path to the document, and never loads it into the process environment — unlike `$DSH_HOME/.env`, which is the user's ordinary environment layer (see [app-boot's Harness-home layers](../../boot/app-boot/README.md#profiles)) — so reaching the value takes a deliberate read of a path the agent was not given.

In `keychain` mode reference values never enter the YAML document, child-process argv, captured terminal output, or propagated diagnostics. The provider sends both confirmations to `/usr/bin/security` through a private pseudoterminal; structured records remain file-backed and therefore must never contain secret material. Keychain access follows the login Keychain's ACL and does not claim isolation from every process running as the same user.
