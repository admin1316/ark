 ...
 * Keychain), layered against the environment by how much each layer is
 * trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > managed store                    (.credentials.yaml, or Keychain in keychain mode)
 ...
 *
 * In `mode: 'keychain'` the reference managed store is the macOS login
 * Keychain (`/usr/bin/security` generic passwords, service
 * {@link DEFAULT_KEYCHAIN_SERVICE}) instead of the document; the file is not
 * read or watched for references, so secrets never sit in a home-directory
 * file. Structured records remain document-backed in that mode.
...
export interface Config {
  ...
  debounceMs?: number
  /**
   * Reference managed-store backend. `file` keeps `.credentials.yaml`;
   * `keychain` stores reference secrets in the macOS login Keychain (macOS
   * only; recommended for signed production builds, where the Keychain ACL
   * binds to the bundle's code signature instead of an ad-hoc rebuild).
   * Structured records remain document-backed either way.
   */
  mode?: 'file' | 'keychain'
  /** Keychain service name separating this deployment's credentials from other items. */
  keychainService?: string
}

/** Default Keychain service name; deployments may override it per environment. */
export const DEFAULT_KEYCHAIN_SERVICE = 'dsh.credentials'
...
/** The `code` carried by a failed subprocess or filesystem error. */
function exitCodeOf(error: unknown): string | number | undefined {
...
export class LocalCredentialProvider extends CredentialProvider {
  ...
  static Config: z<Config> = z.object({
    ...
    mode: z.union([z.const('file'), z.const('keychain')]).default('file'),
  ...
  private readonly mode: 'file' | 'keychain'
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.keychainService = config.keychainService ?? DEFAULT_KEYCHAIN_SERVICE
    if (this.mode === 'keychain' && process.platform !== 'darwin') {
      throw new Error('credentials-local: keychain mode requires macOS')
    }
  }

  /** Run one /usr/bin/security command; resolves its trimmed stdout. */
  private security(args: readonly string[]): Promise<string> {
    return new Promise((resolvePromise, reject) => {
      execFile('/usr/bin/security', args, (error, stdout) => {
        if (error !== null) {
          // Exit 44 is the tool's "item not found"; treat as absent.
          if (exitCodeOf(error) === 44) {
            resolvePromise('')
            return
          }
          reject(error instanceof Error ? error : new Error('security command failed'))
          return
        }
  ...
  private keychainGet(ref: CredentialRef): Promise<string> {
    return this.security(['find-generic-password', '-s', this.keychainService, '-a', ref, '-w'])
  }

  private async keychainSet(ref: CredentialRef, value: string): Promise<void> {
    // `security -w` reads two confirmations from a controlling terminal, not
    // stdin. node-pty supplies that terminal while keeping the secret out of
    // argv, files, captured output, and propagated diagnostics.
    const pty = await import('node-pty')
    return new Promise((resolvePromise, reject) => {
      const terminal = pty.spawn(
        '/usr/bin/security',
        ['add-generic-password', '-U', '-s', this.keychainService, '-a', ref, '-w'],
        {
          name: 'xterm',
          cols: 80,
          rows: 24,
      ...
      const prompts = ['password data for new item:', 'retype password for new item:']
      let promptIndex = 0
      let promptBuffer = ''
      let settled = false
      ...
      terminal.onData((data) => {
        if (settled) return
        const expectedPrompt = prompts[promptIndex]
        if (expectedPrompt === undefined) return
        promptBuffer = (promptBuffer + data).slice(-256)
        if (!promptBuffer.includes(expectedPrompt)) return
        promptBuffer = ''
        promptIndex++
      ...
      terminal.onExit(({ exitCode }) => {
        ...
        if (exitCode === 0 && promptIndex === prompts.length) {
          resolvePromise()
          return
        }
        reject(Object.assign(
          new Error(`security command failed (exit ${exitCode})`),
          { code: exitCode },
        ))
      })
    })
  ...
  private keychainUnset(ref: CredentialRef): Promise<void> {
    return this.security(['delete-generic-password', '-s', this.keychainService, '-a', ref])
      .then(() => undefined)
      .catch((error: unknown) => {
        /* v8 ignore next -- security() maps exit 44 to an empty resolve, so a 44-coded rejection cannot reach this catch */
        if (exitCodeOf(error) === 44) return
        throw error
      })
  }
  ...
  async* [Service.init](): AsyncGenerator<() => Promise<void> | void, void, void> {
    ...
    // Keychain mode keeps the reference store out of the document; nothing
    // external can hot-edit the Keychain, so there is no seam to watch. The
    // document still loads above, because structured records remain
    // document-backed in this mode.
    if (!this.spec.watch || this.mode === 'keychain') return
  ...
  override resolve(ref: CredentialRef): Promise<ResolvedCredential | undefined> {
    ...
    if (this.mode === 'keychain') {
      return this.keychainGet(ref).then(stored => stored === ''
        ? this.resolveDotenvFallback(ref)
        : { value: stored, source: 'keychain' })
    }
    const stored = this.values.get(ref)
    if (stored !== undefined) return Promise.resolve({ value: stored, source: 'file' })
    return this.resolveDotenvFallback(ref)
  }

  /** The `.env` fallback for a reference, or `undefined` when absent. */
  private resolveDotenvFallback(ref: CredentialRef): Promise<ResolvedCredential | undefined> {
  ...
  override describe(ref: CredentialRef): Promise<CredentialInfo> {
    ...
    if (this.mode === 'keychain') {
      return this.keychainGet(ref).then(stored => stored === ''
        ? this.describeDotenvFallback(ref)
    ...
    return this.describeDotenvFallback(ref)
  }

  /** The `.env` fallback description for a reference, or "not configured". */
  private describeDotenvFallback(ref: CredentialRef): Promise<CredentialInfo> {
    const fallback = this.dotenvFallback(ref)
