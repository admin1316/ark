...
|---|---|---|---|
| 继承的进程环境 | `env` | 否 | 始终优先 |
| `$DSH_HOME/.credentials.yaml`（`file`）或 macOS 登录钥匙串（`keychain`） | `file` 或 `keychain` | 是（`set`/`unset`） | 高于两个 `.env` 层 |
...
- **同一引用的无条件写入是后写胜出**——条件写入在文档锁内比较值摘要与可选来源。两种模式的引用写入和带检查的记录提交共享该锁及提供方队列。使用其他文档路径或直接修改钥匙串的写入方不参与该独占；摘要比较也无法识别期间改动后又变回相同值的情况。
