...
export class LocalCredentialProvider extends CredentialProvider {
    ...
    async keychainSet(ref, value) {
        ...
        return new Promise((resolvePromise, reject) => {
            const terminal = pty.spawn('/usr/bin/security', ['add-generic-password', '-U', '-s', this.keychainService, '-a', ref, '-w'], {
                name: 'xterm',
                cols: 80,
                rows: 24,
                cwd: '/',
            ...
            terminal.onData((data) => {
                ...
                if (!promptBuffer.includes(expectedPrompt))
                    return;
                promptBuffer = '';
                promptIndex++;
            ...
            terminal.onExit(({ exitCode }) => {
                ...
                reject(Object.assign(new Error(`security command failed (exit ${exitCode})`), { code: exitCode }));
            });
        });
    }
    keychainUnset(ref) {
    ...
    async *[Service.init]() {
        ...
        if (!this.spec.watch || this.mode === 'keychain')
            return;
        ...
        return this.enqueue(async () => {
            ...
            await withFileLock(this.spec.filename, async () => {
                ...
                if (condition !== undefined)
                    assertCredentialCondition(ref, await this.resolve(ref), condition);
                if (this.mode === 'keychain') {
                    if (value === undefined)
