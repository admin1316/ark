...
import type {
  ...
  CredentialRecord,
  CredentialRef,
  RemoteCredentialView,
  RemoteCredentialsDescription,
} from './types.ts'
...
export class CredentialConflictError extends Error {
  ...
  constructor(public readonly ref: CredentialRef) {
 ...
 */
export function credentialCondition(current: ResolvedCredential | undefined): CredentialCondition {
 ...
 */
export function assertCredentialCondition(
  ref: CredentialRef, current: ResolvedCredential | undefined, expected: CredentialCondition,
): void {
 ...
 * one lock).
 */
export abstract class CredentialProvider extends TypertRemoteService {
   ...
   */
  @Remote('set')
  async remoteSet(refName: string, value: string): Promise<Record<never, never>> {
    let ref: CredentialRef
    try {
   ...
   */
  @Remote('unset')
  async remoteUnset(refName: string): Promise<Record<never, never>> {
    let ref: CredentialRef
    try {
      ref = credentialRef(refName)
    } catch (error: unknown) {
...
function remoteCredentialInputFailure(error: Error, details: { ref: string }): never {
...
function remoteCredentialRejected(details: { ref: string }): never {
  throw new TypertLookupFailure({
