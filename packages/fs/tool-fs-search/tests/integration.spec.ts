...
if (processExitListenerBaseline.some(listener => Function.prototype.toString.call(listener).includes('terminateForHostExit'))) {
  throw new Error('fs-search worker started with a leaked LocalSubprocessRuntime exit listener')
}
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((_resolve, reject) => {
        timer = setTimeout(() => { reject(new Error('fs-search context cleanup timed out')) }, 5_000)
      }),
    ])
  } finally {
...
describe('search tools over the real subprocess service + the packaged rg', () => {
  ...
  afterEach(async () => {
    ...
    expect(process.rawListeners('exit')).toEqual(processExitListenerBaseline)
  })
