...
describe('strict image-modality gate', () => {
  ...
  it('re-checks the route before deferring, refusing a switch that landed mid-read', async () => {
    await writeFile(join(dir, 'red.png'), PNG_1X1)
    const ctx = await setup({ toolMode: 'code' })
    const runtime = ctx.codeRuntime as FakeRuntime
    // The first gate consult sees the vision route; by the second consult
    // (after the byte commit, before deferring) the route has become
    // text-only — the switch landed in the read/commit window.
    let gateConsults = 0
    const agent = {
      options: {},
      session: {
        ...
        requestHeader: () => {
          gateConsults += 1
          return { config: gateConsults === 1
            ? { provider: 'visual', model: 'vision-model' }
            : { provider: 'visual', model: 'text-model' } }
        },
        append: () => undefined,
      },
    }
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('does not declare image input')
    // No image block ever reached the outer context.
