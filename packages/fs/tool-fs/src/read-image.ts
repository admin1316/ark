...
export function applyReadImageTool(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // Re-check the route just before the image becomes a durable tool
      // result: the file read and attachment commit above take real time,
      // and a model switch committed in that window must not leave an image
      // block on a route that rejects image input.
      await assertImageCapableRoute(ctx, exec, args.file_path)
      return value
    },
