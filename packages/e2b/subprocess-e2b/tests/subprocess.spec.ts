...
describe('E2BSubprocessHandle', () => {
  ...
  it('rejects malformed SDK process ids and non-command settlement failures', async () => {
    ...
    // A failed command settles commandState to undefined; waitForExit observes
    // quiescence rather than hanging on the rejected readyState.
    await expect(crashed.waitForExit()).resolves.toBe(true)
  })
