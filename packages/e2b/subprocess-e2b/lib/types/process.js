...
class DeferredStdin extends Writable {
    ...
    _write(chunk, _encoding, callback) {
        void this.ready.then(handle => handle.sendStdin(chunk)).then(() => { callback(); }, (error) => { callback(asError(error)); });
    }
    _final(callback) {
        void this.ready.then(handle => handle.closeStdin()).then(() => { callback(); }, (error) => { callback(asError(error)); });
    }
}
...
function waitWithSignal(promise, signal) {
    if (signal === undefined)
        return promise;
    if (signal.aborted)
        return Promise.resolve(WAIT_ABORTED);
    return new Promise((resolve, reject) => {
        const onAbort = () => { cleanup(); resolve(WAIT_ABORTED); };
        ...
        (error) => {
            // The awaited operation failed: propagate the rejection (not the
            // WAIT_ABORTED marker) and remove the abort listener so it cannot
...
export class E2BSubprocessHandle {
    ...
    /** @inheritdoc */
    terminate() {
        if (this.quiescenceProven || this.terminationAttempt !== undefined)
            return;
    ...
    async waitForExit(signal) {
        ...
        while (await this.groupAlive(sandbox, processGroupId, signal)) {
            this.throwTerminationFailure();
            if (!await waitTick(this.pollMs, signal))
                return false;
        }
        this.throwTerminationFailure();
        if (signal?.aborted === true)
    ...
    async run() {
        ...
        try {
            ...
            if (this.outputTransportError !== undefined)
                throw this.outputTransportError;
            ...
            if (canceledPreparation && failure === error)
    ...
    async writeBatchStdin(handle) {
        if (typeof this.spec.stdio.stdin !== 'object')
            return;
        try {
    ...
    async writeOutput(pipe, inherited, data) {
        ...
        if (target === undefined || data.length === 0 || this.terminationController.signal.aborted)
            return;
        if (target.destroyed)
            throw new Error('subprocess output stream is closed');
        ...
        await new Promise((resolve, reject) => {
            ...
            if (this.terminationController.signal.aborted || this.outputReleased.signal.aborted)
                onRelease();
        });
    }
    async waitForProcessGroupId(sandbox, completion) {
        ...
        while (true) {
            ...
            if (settled)
                throw new Error('subprocess-e2b: remote command exited before publishing its process-group id');
        }
    }
    async waitForCommand(sandbox, handle, completion) {
        const settlement = completion.then(result => ({ kind: 'result', result }), (error) => ({ kind: 'error', error }));
        ...
        while (true) {
            ...
            if (rawStatus.length > 0) {
                ...
                if (completed !== undefined)
                    return this.commandOutcome(completed, exitCode);
                const drained = await withinMs(settlement, this.spec.graceMs);
                if (drained !== undefined)
                    return this.commandOutcome(drained, exitCode);
            ...
            if (completed !== undefined)
                return this.commandOutcome(completed);
    ...
    commandOutcome(settlement, publishedExitCode) {
        ...
        if (settlement.error instanceof CommandExitError) {
            if (publishedExitCode !== undefined)
                return { exitCode: publishedExitCode, signal: null };
    ...
    async rollbackPublishedFailure(error) {
        if (this.remotePid <= 0 || this.quiescenceProven)
            return error;
        this.terminate();
        try {
    ...
    async groupAlive(sandbox, pid, signal) {
        const result = await sandbox.commands.run(`set -o pipefail; ps -eo pgid=,stat= | awk '$1 == ${pid} && $2 !~ /^[ZXx]/ { live=1 } END { if (live) print "live" }'`, commandOpts(this.controlEnvs, signal)).catch((error) => {
            if (signal?.aborted === true)
                return undefined;
            if (error instanceof SandboxNotFoundError)
