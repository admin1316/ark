 ...
 */
export function parseNativeApiPort(value: string | undefined): number | undefined {
...
export function apply(ctx: Context): void {
  ...
  program.action(() => {
    ...
    try {
      port = parseNativeApiPort(program.opts<NativeApiOptions>().port)
    } catch (error) {
    ...
    ctx.provide(NATIVE_API_STARTUP_SERVICE, Object.freeze({
      ...(port === undefined ? {} : { port }),
    }) satisfies NativeApiStartupValues)
  })
