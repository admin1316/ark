---
description: "The shared dsh core as a profile bundle: cordis.patch.yml inserts every base plugin row — model adapters, the shared agent-default-model selection, tools, persistence, policy, settings/credentials, telemetry, and the core spawn/fork subagent providers — over the empty profile root, as the first layer of every profile's dsh.profile.bundles list."
kind: "package-bundle"
---
...
- **`workspace-write` is the shipped factory default, by design** — fresh sessions confine file effects to the workspace plus session temp grants and gate everything else behind per-action `ask` approvals (`DSH_PERMISSION_MODE` overrides; see `cordis.patch.yml`). `danger-full-access` maps the approval policy to `never`: unrestricted host filesystem/process access with no per-action prompt, so deploy it only on a trusted single-user host or approval-less CI, never pointed at sensitive data.
