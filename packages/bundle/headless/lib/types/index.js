 ...
 */
/**
 * Stream reasoning deltas for the exact direct-run agent to stderr.
 * Package-private; the public headless entry exports only the plugin contract.
 */
function streamReasoning(ctx, agent, stderr) {
    ...
    const dispose = ctx.on('session/event', (session, event) => {
        ...
        if (!started || event.type !== 'assistant/chunk')
            return;
