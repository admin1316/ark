  ...
  const definition = defineTool({
    ...
    async execute(args, exec): Promise<RunCodeOutput> {
      ...
      interface PendingDispatch {
        ...
        abandon(): void
        /**
         * Settle a call whose prepare stage failed before any dispatch: the
         * binding rejects (the worker turns it into ToolCallError), the
         * failure is logged like any dispatched error, and the head is marked
         * settled so the ordered lane still converges.
         */
        ...
        settled: boolean
        /** True once fail() settled this head; guards the one-settle-event contract. */
        failed: boolean
      ...
      /** The head being committed right now (already shifted out of commitQueue). */
      let committing: PendingDispatch | undefined
      let exclusiveActive = false
      let driving = false
      let driverRun: Promise<void> = Promise.resolve()
      /** A terminal lane failure (a commit-stage throw), rethrown by drainDispatches. */
      let laneFailure: Error | undefined
          ...
          try {
            for (;;) {
              ...
              if (commitHead !== undefined && commitHead.settled) {
                commitQueue.shift()
                // Cleared only after a SUCCESSFUL commit: a commit-stage
                // throw must leave the head visible to the lane catch so its
                // binding can be failed instead of hanging the run.
                committing = commitHead
              ...
              if (head !== undefined) {
                ...
                if (capacity) {
                  ...
                  try {
                    ...
                    // A prepare-stage rejection must not wedge the lane: fail
                    // the head (binding rejection + logged failure) and keep
                    // driving the remaining dispatches. Without this, the
                    // driverRun rejection strands every in-flight binding and
                    // the run hangs on its own teardown.
                    head.fail(error)
                  }
            ...
            // A lane defect (a commit-stage throw) must fail the run loudly
            // and settle every still-pending binding: the program may be
            // awaiting any of them, and a never-settling binding would hang
            // the run itself. drainDispatches rethrows the captured failure;
            // queued-unstarted heads are abandoned like the abort path,
            // started heads fail with the lane error. The barrier releases
            // too: a program that catches a failed binding must be able to
            // issue another call without wedging behind a stale exclusive.
            laneFailure = error instanceof Error ? error : new Error(String(error))
            exclusiveActive = false
            if (committing !== undefined) {
              committing.fail(error)
              committing = undefined
            }
            for (const head of pendingQueue) head.abandon()
            for (const head of commitQueue) head.fail(error)
      ...
      const drainDispatches = async (): Promise<void> => {
        ...
        try {
          ...
          driveError = error instanceof Error ? error : new Error(String(error))
        }
        // In-turn settlement holds even when the lane failed: the live pool
        // and the settle-event work must drain before the failure surfaces,
        // or a fire-and-forget sub-call returns while its event is missing.
        while (inFlight.size > 0) await Promise.allSettled([...inFlight])
        while (logWork.size > 0) await Promise.allSettled([...logWork])
        if (driveError !== undefined) throw driveError
        // A prior lane failure must surface here even though drive() would
        // start a fresh converging run: the run failed and its bindings were
        // settled, so it must not complete successfully.
      ...
      const binding = (name: string): CodeBindingFunction => async (rawArgs: unknown): Promise<JsonValue> => {
        if (laneFailure !== undefined) {
          // The lane is terminal: reject the call immediately instead of
          // queueing work a dead driver can never settle.
          throw laneFailure
        }
        if (runOver()) {
        ...
        const outcome = await new Promise<DispatchOutcome>((resolve, reject) => {
          ...
          let parked:
            ...
            | undefined
          // Whether THIS head already delivered its settle event. A later
          // terminal lane failure must not emit a second durable event for an
          // already-settled head (one-settle-event contract); it only marks
          // the head failed/settled so the binding stays rejected thereafter.
          let delivered = false
          ...
          pendingQueue.push({
            ...
            fail(error: unknown): void {
              // Idempotent: a head failed once (prepare-stage) must not be
              // failed AGAIN by a later terminal lane failure — that would
              // emit a second durable settle event, violating the
              // one-settle-event contract. The binding rejection itself is
              // naturally idempotent.
              if (this.failed) return
              this.failed = true
              this.settled = true
              reject(error instanceof Error ? error : new Error(String(error)))
              // A head that already delivered its settle event (commit
              // finalized the result, then the lane failed on a sibling's
              // logWork rejection) must not emit a second durable event.
              if (delivered) return
              const agent = exec.agent
              if (agent === undefined) return
              const message = error instanceof Error ? error.message : String(error)
              const task: Promise<void> = (async () => {
                // Record the failure like any dispatched error: the program
                // receives a ToolCallError it may surface, so the model-visible
                // text stays reconstructable from the session log.
                const logged = await shapeDispatchLog({
                  exec, agent, subCallId, name, isError: true,
            ...
            async start(): Promise<void> {
              ...
              if (prepared.kind === 'dispatch') {
                this.flight = scheduler.dispatch(prepared.exec).then(
                  ...
                  (error: unknown) => {
                    // Mirror the native scheduler's defensive rejection
                    // branch: a dispatch-stage rejection (tool body,
                    // around-dispatch waterfall) must still settle the lane —
                    // the driver only commits `settled` heads and the binding
                    // needs a terminal outcome, or the run hangs even after
                    // abort. The failure folds into an error result and is
                    // committed in submission order like any other outcome.
                    const message = error instanceof Error ? error.message : String(error)
                    const info = error instanceof HarnessError ? { name: error.name, code: error.code } : undefined
                    parked = {
                      kind: 'final-result',
                      exec: prepared.exec,
                      result: {
            ...
            async commit(): Promise<void> {
              // The fail path settles the head without parking a result; its
              // binding was already rejected and the failure logged, so the
              // cursor just shifts past it.
          ...
          wakeup()
          // The lane rejects only for its own defects (a commit-stage throw);
          // drainDispatches observes the same rejection at run settlement.
          // The void launch must not surface an unhandled rejection in the
          // window before then — fail-loud would exit the process.
          void drive().catch(() => { /* observed by drainDispatches at run settlement */ })
        })
