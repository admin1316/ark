...
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult, ToolRuntimeScheduler } from '@deepseek-ai/dsh-tools'
...
describe('mode-aware wire contribution', () => {
  ...
  it("mode 'ptc' exposes only run_code, enforces the same direct-call collapse, and layers its log hook", async () => {
    expect(isPtcPresentationMode('ptc')).toBe(true)
    expect(isPtcPresentationMode('code')).toBe(true)
    expect(isPtcPresentationMode('native')).toBe(false)
    expect(isPtcPresentationMode('both')).toBe(false)

    const { ctx, systemPrompt, runtime } = await setup({ mode: 'ptc' })
    registerEcho(ctx)
    const assembly = await systemPrompt.assemble()
    expect(assembly.tools.map(tool => tool.name)).toEqual([RUN_CODE_NAME])
    expect(assembly.sections.find(section => section.name === 'tools:ptc-only')?.text)
      .toContain(`\`${RUN_CODE_NAME}\` is the only tool you can call directly`)
    ...
    ctx.on('tools/ptc-dispatch-log', async (_dispatch, next) => {
      hooks.push('ptc')
      return [...await next(), { type: 'text', text: 'ptc log marker' }]
    })
    runtime.behavior = async request => ({
      logs: [],
      value: await request.bindings[0]!.functions['echo']!({ value: 'inside' }),
    })
    const nested = await runCode(ctx, 'return tools.echo({ value: "inside" })', { agent })
    expect(nested).toMatchObject({ isError: false })
    expect(nested.content).toEqual([{ type: 'text', text: 'echo:inside' }])
    expect(hooks).toEqual(['ptc', 'legacy'])
    const settle = events.find(event => event.type === 'tool/code-dispatch')
    expect(settle?.data).toMatchObject({ name: 'echo' })
    expect((settle?.data as { content: unknown[] }).content)
      .toContainEqual({ type: 'text', text: 'ptc log marker' })

    const direct = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('ptc-direct'),
      name: 'echo',
      arguments: { value: 'outside' },
      agent,
    })
...
describe('the run_code dispatch bridge', () => {
  ...
  it('a dispatch-stage rejection settles the lane and reaches the program as a tool error', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    const scheduler = (ctx.tools as unknown as { [K in typeof TOOL_RUNTIME_SCHEDULER]: ToolRuntimeScheduler })[TOOL_RUNTIME_SCHEDULER]
    const originalDispatch = scheduler.dispatch.bind(scheduler)
    const failures: unknown[] = [
      new Error('dispatch exploded'),
      'dispatch rejected with a string',
      new CodeRunFailedError('dispatch rejected with a HarnessError'),
    ]
    scheduler.dispatch = async () => { throw failures.shift() }
    try {
      registerEcho(ctx)
      const { agent, events } = fakeAgent()
      runtime.behavior = async (request) => {
        const messages: string[] = []
        for (const value of ['error', 'string', 'coded']) {
          try {
            await request.bindings[0]!.functions.echo!({ value })
            messages.push('unreachable')
          } catch (error: unknown) {
            messages.push(error instanceof Error ? error.message : String(error))
          }
        }
        return { logs: [], value: messages.join('|') }
      }
      // Without the rejection branch this run hangs: the binding never
      // settles and drainDispatches never converges (vitest timeout fails).
      const result = await runCode(ctx, 'program', { agent })
      expect(result.isError).toBe(false)
      if (result.isError) throw new Error('expected success')
      expect(result.value).toMatchObject({
        result: 'dispatch exploded|dispatch rejected with a string|dispatch rejected with a HarnessError',
      })
      const settles = events.filter(event => event.type === 'tool/code-dispatch')
      expect(settles).toHaveLength(3)
      expect(settles.every(event => (event.data as { isError?: boolean }).isError === true)).toBe(true)
    } finally {
      scheduler.dispatch = originalDispatch
    }
  })

  it('a commit-stage failure fails the run loudly instead of being swallowed by a lane restart', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    const scheduler = (ctx.tools as unknown as { [K in typeof TOOL_RUNTIME_SCHEDULER]: ToolRuntimeScheduler })[TOOL_RUNTIME_SCHEDULER]
    const originalFinalize = scheduler.finalize.bind(scheduler)
    // A process/wire seam may reject with a non-Error value. The lane must
    // normalize it both when capturing the terminal failure and when draining.
    scheduler.finalize = async () => { throw 'commit exploded' }
    try {
      registerEcho(ctx)
      runtime.behavior = async (request) => {
        try {
      ...
      // The commit throw must surface as a failed run_code call — not a
      // silently restarted lane that reports success while the binding never
      // settles.
      const result = await runCode(ctx, 'program')
      expect(result.isError).toBe(true)
      if (!result.isError) throw new Error('expected failure')
      expect(result.error.message).toContain('commit exploded')
    } finally {
      scheduler.finalize = originalFinalize
    }
  })

  it('a commit-stage failure is one-settle-event across already-failed heads', async () => {
    const { ctx, runtime } = await setup({ mode: 'code', maxParallelSubCalls: 3 })
    const scheduler = (ctx.tools as unknown as { [K in typeof TOOL_RUNTIME_SCHEDULER]: ToolRuntimeScheduler })[TOOL_RUNTIME_SCHEDULER]
    const originalPrepare = scheduler.prepare.bind(scheduler)
    const originalFinalize = scheduler.finalize.bind(scheduler)
    // The first echo fails at commit while `boom` is an already-failed commit
    // head, `slow` is still in flight, and a fourth call is still pending. One
    // terminal edge must settle all four without duplicating `boom`'s event.
    scheduler.prepare = async (input) => {
      if (input.name === 'boom') throw 'prepare exploded'
      return originalPrepare(input)
    }
    scheduler.finalize = async () => { throw new Error('commit exploded') }
    try {
      ctx.tools.register(defineTool({
        name: 'boom',
        description: 'A tool whose prepare stage explodes.',
        parameters: {},
        output: { schema: { type: 'string' }, render: () => [{ type: 'text', text: 'unreachable' }] },
        execute: () => Promise.resolve('unreachable'),
        isConcurrencySafe: () => true,
      }))
      const fast = Promise.withResolvers<undefined>()
      const slow = Promise.withResolvers<undefined>()
      const slowStarted = Promise.withResolvers<undefined>()
      ctx.tools.register(defineTool({
        name: 'echo',
        description: 'Parallel echo used to fail the first commit.',
        parameters: { value: { type: 'string', required: true } },
      ...
      ctx.tools.register(defineTool({
        name: 'slow',
        description: 'Parallel body that remains in flight across lane failure.',
        parameters: {},
        output: { schema: { type: 'string' }, render: (_args, value) => [{ type: 'text', text: value }] },
        async execute() {
      ...
      runtime.behavior = async (request) => {
        const outcomes = Promise.allSettled([
          request.bindings[0]!.functions.echo!({ value: 'x' }),
          request.bindings[0]!.functions.boom!({}),
          request.bindings[0]!.functions.slow!({}),
          request.bindings[0]!.functions.echo!({ value: 'pending' }),
        ])
        await Promise.resolve()
        fast.resolve(undefined)
        return { logs: [], value: (await outcomes).map(outcome => outcome.status).join('|') }
      }
      ...
      if (!result.isError) throw new Error('expected failure')
      expect(result.error.message).toContain('commit exploded')
      // One-settle-event contract: 'boom' (prepare-failed) and the first
      // 'echo' (commit-failed) each log exactly one failure. The in-flight
      // 'slow' head is failed and drained; the fourth pending call is abandoned.
      const failureEvents = events.filter(event => event.type === 'tool/code-dispatch' && (event.data as { isError?: boolean }).isError === true)
      expect(failureEvents).toHaveLength(3)
      const names = failureEvents.map(event => (event.data as { name?: string }).name).sort()
      expect(names).toEqual(['boom', 'echo', 'slow'])
    } finally {
      scheduler.prepare = originalPrepare
      scheduler.finalize = originalFinalize
    }
  })

  it('normalizes a non-Error lane defect outside commit and rejects later bindings immediately', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    registerEcho(ctx)
    vi.spyOn(ctx.tools, 'executionMode').mockImplementationOnce(() => {
      throw 'classification exploded'
    })
    runtime.behavior = async (request) => {
      const call = request.bindings[0]!.functions.echo!
      const first = await call({ value: 'first' })
        .then(() => 'resolved', (error: unknown) => error instanceof Error ? error.message : String(error))
      const second = await call({ value: 'second' })
        .then(() => 'resolved', (error: unknown) => error instanceof Error ? error.message : String(error))
      return { logs: [], value: `${first}|${second}` }
    }

    const result = await runCode(ctx, 'program')
    expect(result.isError).toBe(true)
    if (!result.isError) throw new Error('expected failure')
    expect(result.error.message).toContain('classification exploded')
  })

  it('drains an in-flight body after a fire-and-forget commit rejects with a non-Error value', async () => {
    const { ctx, runtime } = await setup({ mode: 'code', maxParallelSubCalls: 2 })
    const scheduler = (ctx.tools as unknown as {
      [K in typeof TOOL_RUNTIME_SCHEDULER]: ToolRuntimeScheduler
    })[TOOL_RUNTIME_SCHEDULER]
    const originalFinalize = scheduler.finalize.bind(scheduler)
    const commitStarted = Promise.withResolvers<undefined>()
    scheduler.finalize = async () => {
      commitStarted.resolve(undefined)
      throw 'fire-and-forget commit failed'
    }
    const fastStarted = Promise.withResolvers<undefined>()
    const slowStarted = Promise.withResolvers<undefined>()
    const releaseFast = Promise.withResolvers<undefined>()
    const releaseSlow = Promise.withResolvers<undefined>()
    try {
      for (const [name, started, release] of [
        ['fast', fastStarted, releaseFast],
        ['slow', slowStarted, releaseSlow],
      ] as const) {
        ctx.tools.register(defineTool({
          name,
          description: `${name} parallel probe`,
          parameters: {},
          output: {
            schema: { type: 'string' },
            render: (_args, value) => [{ type: 'text', text: value }],
      ...
      runtime.behavior = async (request) => {
        void request.bindings[0]!.functions.fast!({}).catch(() => undefined)
        void request.bindings[0]!.functions.slow!({}).catch(() => undefined)
        await Promise.all([fastStarted.promise, slowStarted.promise])
        return { logs: [] }
      }

      const running = runCode(ctx, 'program')
      await Promise.all([fastStarted.promise, slowStarted.promise])
      releaseFast.resolve(undefined)
      ...
      expect(result.isError).toBe(true)
      if (!result.isError) throw new Error('expected failure')
      expect(result.error.message).toContain('fire-and-forget commit failed')
    } finally {
      releaseFast.resolve(undefined)
      releaseSlow.resolve(undefined)
      scheduler.finalize = originalFinalize
  ...
  it('bounds pending settle logs and never duplicates a result delivered before append failure', async () => {
    const { ctx, runtime } = await setup({ mode: 'code', maxParallelSubCalls: 1 })
    registerEcho(ctx)
    const releaseFirstLog = Promise.withResolvers<undefined>()
    const firstAppendAttempted = Promise.withResolvers<undefined>()
    let logCalls = 0
    ctx.on('tools/code-dispatch-log', async (_dispatch, next) => {
      const content = await next()
      logCalls++
      if (logCalls === 1) {
        await releaseFirstLog.promise
      } else {
        releaseFirstLog.resolve(undefined)
        await firstAppendAttempted.promise
      }
      return content
    })
    let appendCalls = 0
    const agent = {
      session: {
        ...
        append(type: string): void {
          if (type !== 'tool/code-dispatch') return
          appendCalls++
          if (appendCalls === 1) {
            firstAppendAttempted.resolve(undefined)
            throw new Error('settle append failed')
          }
        },
      },
    } as unknown as Agent
    runtime.behavior = async request => ({
      ...
      value: (await Promise.allSettled([
        request.bindings[0]!.functions.echo!({ value: 'one' }),
        request.bindings[0]!.functions.echo!({ value: 'two' }),
      ])).map(outcome => outcome.status).join('|'),
    })

    const result = await runCode(ctx, 'program', { agent })
    expect(result.isError).toBe(true)
    if (!result.isError) throw new Error('expected failure')
    expect(result.error.message).toContain('settle append failed')
    expect(logCalls).toBe(2)
    expect(appendCalls).toBe(2)
  })

  it('a prepare-stage rejection fails the call without wedging later dispatches', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    const scheduler = (ctx.tools as unknown as { [K in typeof TOOL_RUNTIME_SCHEDULER]: ToolRuntimeScheduler })[TOOL_RUNTIME_SCHEDULER]
    const originalPrepare = scheduler.prepare.bind(scheduler)
    scheduler.prepare = async (input) => {
      if (input.name === 'boom') throw new Error('prepare exploded')
      return originalPrepare(input)
    }
    try {
      registerEcho(ctx)
      ...
      const { agent, events } = fakeAgent()
      runtime.behavior = async (request) => {
        const first = await request.bindings[0]!.functions.boom!({})
          .then(() => 'resolved', (error: unknown) => error instanceof Error ? error.message : String(error))
        const second = await request.bindings[0]!.functions.echo!({ value: 'x' })
        return { logs: [], value: `${first}|${second as string}` }
      }
      const result = await runCode(ctx, 'program', { agent })
      expect(result.isError).toBe(false)
      if (result.isError) throw new Error('expected success')
      // The failed call rejects its binding; the lane keeps driving and the
      // later call still dispatches and settles.
      expect(result.value).toMatchObject({ result: 'prepare exploded|echo:x' })
      const settles = events.filter(event => event.type === 'tool/code-dispatch')
      expect(settles).toHaveLength(2)
      expect(settles[0]?.data).toMatchObject({ name: 'boom', isError: true })
      expect(settles[1]?.data).toMatchObject({ name: 'echo', isError: false })
    } finally {
      scheduler.prepare = originalPrepare
    }
  })
