		...
		async execute(args, exec) {
			...
			*/
			...
			const drainDispatches = async () => {
				...
				if (driveError !== void 0) throw driveError;
				if (laneFailure !== void 0) throw laneFailure;
			};
						...
						async start() {
							...
							if (prepared.kind === "dispatch") {
								this.flight = scheduler.dispatch(prepared.exec).then((dispatchOutcome) => {
									...
									const message = error instanceof Error ? error.message : String(error);
									const info = error instanceof HarnessError ? {
										name: error.name,
										code: error.code
									} : void 0;
									parked = {
...
var ToolExecutor = class {
	...
	*/
	...
	async completeScheduledExecution(runtime, ctx, prepared) {
		switch (prepared.kind) {
			case "dispatch": {
				...
				return dispatched.kind === "post-result" ? await this.finalizeScheduledExecution(runtime, ctx, prepared.exec, dispatched.result) : this.finishScheduledExecution(runtime, ctx, prepared.exec, dispatched.result);
			}
			case "post-result": return await this.finalizeScheduledExecution(runtime, ctx, prepared.exec, prepared.result);
			case "final-result": return this.finishScheduledExecution(runtime, ctx, prepared.exec, prepared.result);
	...
	createExecution(exec) {
		...
		const collapsed = visible !== void 0 && this.registry.collapses(name, agent, parent !== void 0);
	...
	*/
	...
	async prepareExecution(runtime, ctx, input, next) {
		...
		try {
			...
			const askResolution = gate.kind === "ask" ? await this.serviceAsk(ctx, exec, gate) : {
				decision: gate,
			...
			const denialReason = decision.kind === "allow" ? this.registry.guardReason(exec) : decision.reason;
	...
	*/
	async dispatchToolBody(exec) {
		...
		exec.signal = signal;
		try {
			const tool = this.registry.resolveExecution(exec.name, exec.agent, exec.parent !== void 0);
	...
	*/
	async finalizeScheduledExecution(runtime, ctx, exec, result) {
		try {
			...
			return this.finishScheduledExecution(runtime, ctx, exec, this.callerCancelled(exec) && !postResult.isError ? this.cancellationResult(exec, postResult) : postResult);
		} catch (error) {
	...
	*/
	...
	notifyResult(runtime, ctx, exec, result) {
		...
		const callbacks = ctx.events.dispatch("emit", [
			scopeTarget(runtime, exec.agent),
			"tools/result",
			exec,
			result
		]);
...
function isPtcPresentationMode$1(mode) {
...
var ToolRegistry = class {
	...
	*/
	codeTransport;
	constructor(runtime, ctx, config = {}) {
	...
	*/
	collapseSection(mode = this.defaultMode) {
		return {
			name: mode === "ptc" ? "tools:ptc-only" : "tools:code-only",
			order: FIRST_PARTY_SECTION_ORDER.PTC_ONLY,
			text: (context) => isPtcPresentationMode$1(this.modeFor(context.scope)) ? CODE_ONLY_INSTRUCTION : ""
		};
	}
	/**
	...
	*/
	wireSchemas(scope) {
		...
		if (isPtcPresentationMode$1(mode) && mode !== "both") return {
	...
	*/
	restrict(ctx, filter) {
		...
		return this.layers.effect(ctx, (layer) => layer.restrictions.append(compiled), { label: "tools.restrict()" });
	}
	/**
	...
	*/
	guard(ctx, guard) {
		return this.layers.effect(ctx, (layer) => layer.guards.append(guard), {
			label: "tools.guard()",
			notify: false
		});
	}
	...
	*/
	async shapeDispatchLog(dispatch) {
		try {
			const legacy = () => this.ctx.waterfall(scopeTarget(this.runtime, dispatch.agent), "tools/code-dispatch-log", dispatch, () => Promise.resolve(dispatch.content));
			return this.modeFor(dispatch.agent) === "ptc" ? await this.ctx.waterfall(scopeTarget(this.runtime, dispatch.agent), "tools/ptc-dispatch-log", dispatch, legacy) : await legacy();
		} catch (error) {
	...
	*/
	collapses(name, scope, nested) {
		return !nested && isPtcPresentationMode$1(this.modeFor(scope)) && name !== "run_code";
	}
};
//#endregion
...
*/
function isPtcPresentationMode(mode) {
	return mode === "ptc" || mode === "code";
}
/**
