...
export class ToolRegistry {
     ...
     */
    guard(ctx, guard) {
        return this.layers.effect(ctx, layer => layer.guards.append(guard), { label: 'tools.guard()', notify: false });
    }
    /**
    ...
    async shapeDispatchLog(dispatch) {
        try {
            ...
            return this.modeFor(dispatch.agent) === 'ptc'
                ? await this.ctx.waterfall(scopeTarget(this.runtime, dispatch.agent), 'tools/ptc-dispatch-log', dispatch, legacy)
                : await legacy();
        }
        catch (error) {
