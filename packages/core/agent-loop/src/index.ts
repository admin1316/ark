...
import { DEFAULT_AGENT_TURN_BUDGET, DEFAULT_MAX_PARALLEL_TOOL_CALLS } from './constants.ts'
...
function resolveTurnBudget(value: Partial<AgentTurnBudgetLimits> | undefined): AgentTurnBudgetLimits {
  const budget = { ...DEFAULT_AGENT_TURN_BUDGET, ...value }
  for (const [key, limit] of Object.entries(budget)) {
    if (!Number.isSafeInteger(limit) || limit <= 0) {
      throw new Error(`turnBudget.${key} must be a positive safe integer`)
    }
  }
...
interface PreparedAgent {
  ...
  dispose(options?: CancelOptions): Promise<void>
}
...
export { AgentQuiescenceTimeoutError } from './agent.ts'
export type {
  AgentOperationStage,
  AgentQuiescenceResidual,
  AgentTurnBudgetDimension,
  AgentTurnBudgetExhaustedReason,
  AgentTurnBudgetUsage,
} from './agent.ts'
export { DEFAULT_AGENT_TURN_BUDGET, DEFAULT_MAX_PARALLEL_TOOL_CALLS }
export type { AgentTurnBudgetLimits } from './constants.ts'
...
type ResolvedConfig = Omit<Config, 'turnBudget'> & {
  maxParallelToolCalls: number
  turnBudget: AgentTurnBudgetLimits
}
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  private prepare(ownerCtx: Context, id: SessionId, options: AgentOptions, session: Session, callerSignal?: AbortSignal): PreparedAgent {
    ...
    // Reverse teardown is shared while in flight and memoized after success.
    // A bounded quiescence failure deliberately keeps the Agent, Session, and
    // Scope registered because same-process code is still running; a later
    // retry may finish teardown after that exact operation really settles.
    const dispose = (
      ...
      const operation = (async () => {
        ...
        // sent after this point remains parked because the lifecycle is closing.
        if (machine === undefined) await machineReady.promise
        if (machine !== undefined) {
          machine.cancel({ kind: 'disposed' }, options)
          await machine.whenIdle()
        }
        // From this point the driver really is quiescent. Scope unwind may
        // fail, but registry detachment is now safe and remains best effort.
        try {
    ...
    try {
      ...
      return {
        ...
        publish: (source) => {
          ...
          assertLive()
          return { agent, dispose: options => dispose(false, options) }
        },
