...
describe('AgentLoop initiator scope', () => {
  it('a wake racing the initiator-scope close converges instead of wedging the agent', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(SessionStore)
    await ctx.plugin(SystemPrompt)
    ...
    const adapter = new MockAdapter([textResponse('recovered'), textResponse('again'), textResponse('third')])
    ctx.llm.registerAdapter(['mock'], adapter)

    const agent = ctx.agentLoop.create(SessionId('wake-close-race'), { provider: 'mock', model: 'mock' })
    // Simulate the fiber-unload window: the initiator scope is closing while
    // the service is still resolvable — the exact race that left the driver
    // promise pending and the phase stuck in `running`, wedging whenIdle()
    // and every disposal that awaits it.
    const internals = ctx.agents as unknown as { initiatorState: 'active' | 'closing' | 'disposed' }
    internals.initiatorState = 'closing'
    try {
      send(agent, 'wake-during-close')
      // Without the catch the driver promise stays pending and whenIdle()
      // never settles (vitest timeout). With it, the phase converges back to
      // idle and the activity promise resolves.
      await agent.whenIdle()
      expect(adapter.requests).toHaveLength(0)
    } finally {
      internals.initiatorState = 'active'
    }

    // The driver machinery must be fully functional afterwards: drain the
    // close-race message through a real turn, then verify a fresh wake starts
    // exactly one more model turn.
    send(agent, 'drain-stale')
    await agent.whenIdle()
    const before = adapter.requests.length
    send(agent, 'after-close-race')
    await agent.whenIdle()
