...
describe('the session-persistence Agent Note: AgentLoop factory create/resume', () => {
  it('preserves queued input when the owning product disposes for reconnect', async () => {
    const sessionId = SessionId('reconnect-inbox')
    const first = await persistentHarness(new MockAdapter(['hang']))
    ...
    firstHandle.agent.followup(createUserMessage({
      content: [{ type: 'text', text: 'queued after reconnect' }],
      source: { kind: 'user' },
    }))
    expect(firstHandle.agent.inbox.nextTurn).toHaveLength(1)

    await firstHandle.dispose({ keepInbox: true })
    await first.ctx.fiber.dispose()

    const second = await mountPersistentHarness(first.root, new MockAdapter([textResponse('next')]))
    const secondHandle = await second.agents.resume({
      resumeSessionId: sessionId,
      agentOptions: { provider: 'mock', model: 'mock' },
    })
    expect(secondHandle.agent.inbox.nextTurn).toHaveLength(1)
    expect(JSON.stringify(secondHandle.agent.inbox.nextTurn)).toContain('queued after reconnect')
