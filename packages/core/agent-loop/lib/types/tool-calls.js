 ...
 */
export async function executeToolCalls(ctx, turn, step, toolCalls, signal, acceptContext, awaitOperation) {
    ...
    while (next < planned.length) {
        ...
        const outcome = await runGroup(ctx, turn, step, group, mode, signal, acceptContext, awaitOperation);
    ...
    const commitReady = async () => {
        while (committed < group.length) {
            ...
            const result = slot.needsPost
                ? await awaitOperation('tool-finalize', () => ctx.tools[TOOL_RUNTIME_SCHEDULER].finalize(slot.exec, slot.result))
