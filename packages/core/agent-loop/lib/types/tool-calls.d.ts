...
import type { AgentOperationStage } from './agent.ts';
import { type ToolCallBlock } from '@deepseek-ai/dsh-llm';
import type { UserMessage } from '@deepseek-ai/dsh-session';
/** Agent-owned monitor that never abandons an in-process operation on abort. */
type AwaitOperation = <T>(stage: AgentOperationStage, operation: () => PromiseLike<T> | T) => Promise<T>;
/**
 ...
 */
export declare function executeToolCalls(ctx: Context, turn: number, step: number, toolCalls: ToolCallBlock[], signal: AbortSignal, acceptContext: (context: UserMessage) => void, awaitOperation: AwaitOperation): Promise<{
    concluded: boolean;
}>;
export {};
