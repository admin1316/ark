...
*/
async function executeToolCalls(ctx, turn, step, toolCalls, signal, acceptContext, awaitOperation) {
	...
	while (next < planned.length) {
		...
		const outcome = await runGroup(ctx, turn, step, mode === "parallel" ? planned.slice(next) : [first], mode, signal, acceptContext, awaitOperation);
...
*/
async function runGroup(ctx, turn, step, group, mode, signal, acceptContext, awaitOperation) {
	...
	const commitReady = async () => {
		while (committed < group.length) {
			...
			const result = slot.needsPost ? await awaitOperation("tool-finalize", () => ctx.tools[TOOL_RUNTIME_SCHEDULER].finalize(slot.exec, slot.result)) : ctx.tools[TOOL_RUNTIME_SCHEDULER].finish(slot.exec, slot.result);
	...
	const startCall = async (index) => {
		...
		started++;
		const prepared = await awaitOperation("tool-policy", () => ctx.tools[TOOL_RUNTIME_SCHEDULER].prepare(call.exec));
		throwSchedulerFailure();
		switch (prepared.kind) {
			case "dispatch": {
				const promise = awaitOperation("tool-body", () => ctx.tools[TOOL_RUNTIME_SCHEDULER].dispatch(prepared.exec)).then((outcome) => {
					slots[index] = {
						exec: prepared.exec,
...
*/
const DEFAULT_AGENT_TURN_BUDGET = Object.freeze({
	maxElapsedMs: 720 * 60 * 1e3,
	maxSteps: 4096,
	maxModelAttempts: 8192,
	maxToolCalls: 65536,
	cancellationGraceMs: 1e4
});
//#endregion
...
var TurnBudgetExhaustedError = class extends Error {
	reason;
	constructor(reason) {
...
*/
var AgentQuiescenceTimeoutError = class extends Error {
	residual;
...
var TurnBudget = class {
	...
	constructor(limits, abort, reportResidual) {
		...
		this.elapsedTimer = setTimeout(() => {
			if (!this.abort.signal.aborted) this.markExhausted("elapsed-ms", this.limits.maxElapsedMs, this.usage.elapsedMs);
	...
	consume(dimension, amount) {
		...
		this.checkElapsed();
		const current = dimension === "steps" ? this.steps : dimension === "model-attempts" ? this.modelAttempts : this.toolCalls;
		const limit = dimension === "steps" ? this.limits.maxSteps : dimension === "model-attempts" ? this.limits.maxModelAttempts : this.limits.maxToolCalls;
		const observed = current + amount;
		if (observed > limit) throw this.markExhausted(dimension, limit, observed);
	...
	checkElapsed() {
		...
		if (elapsed >= this.limits.maxElapsedMs) throw this.markExhausted("elapsed-ms", this.limits.maxElapsedMs, elapsed);
	}
	markExhausted(dimension, limit, observed) {
		if (this._exhaustion !== void 0) return new TurnBudgetExhaustedError(this._exhaustion);
		const reason = {
	...
	armResidualDeadline() {
		if (this.closed || this.reportedResidual || this.graceTimer !== void 0) return;
		const elapsed = this.abortStartedAt === void 0 ? 0 : Math.max(0, Date.now() - this.abortStartedAt);
		...
		this.graceTimer = setTimeout(() => {
			...
			const abortKind = abortReason instanceof TurnBudgetExhaustedError ? "budget-exhausted" : typeof abortReason === "object" && abortReason !== null && "kind" in abortReason && (abortReason.kind === "user" || abortReason.kind === "parent" || abortReason.kind === "hook" || abortReason.kind === "disposed") ? abortReason.kind : "unknown";
			this.reportResidual({
				code: "AGENT_OPERATION_UNRESPONSIVE",
				abortKind,
				graceMs: this.limits.cancellationGraceMs,
				operations: [...counts].map(([stage, count]) => ({
					stage,
					count
				})),
				budget: this.usage
...
var ReactLoopAgent = class {
	...
	*/
	...
	async whenIdle() {
		...
		do {
			...
			const outcome = await Promise.race([activity.done.then(() => ({ kind: "done" })), activity.residual.then((residual) => ({
				kind: "residual",
				residual
			}))]);
	...
	async preStep(target, position, budget) {
		...
		const decision = await budget.monitor("pre-step", () => this.dispatch.waterfall("agent/pre-step", {
			messages: claimed,
			...position,
			signal
		}, () => Promise.resolve({
		...
		try {
			while (true) {
				...
				if (turnEnds && this.inbox.nextStep.length === 0) {
					await budget.monitor("turn-stopping", () => this.dispatch.serial("agent/turn-stopping", {
						turn,
						signal
					}));
					signal.throwIfAborted();
			...
			if (budget.exhaustion !== void 0) {
				turnEnds = budget.exhaustion;
				if (this.inbox.nextStep.length > 0) this.inbox.splice("next-step", 0, this.inbox.nextStep.length, []);
	...
	async step(assembly, budget) {
		...
		while (true) {
			...
			let sawFinish = false;
			try {
				const iterator = (preparedCall?.stream(request) ?? this.loopCtx.llm.stream(request))[Symbol.asyncIterator]();
				signal.throwIfAborted();
				let completed = false;
				try {
					while (true) {
					...
					if (!completed && iterator.return !== void 0) await budget.monitor("provider-close", async () => {
			...
			const finish = sawFinish ? assembler.finish : {
				kind: "error",
				failure: {
	...
	*/
	async buildRequest(turn, step, tools, system, boundaryMessages, signal, budget) {
		...
		const proposedConfig = await budget.monitor("request-config", () => this.dispatch.waterfall("agent/request", {
			turn,
			step,
			signal
...
/** Resolve one immutable finite budget shared by every turn this factory creates. */
function resolveTurnBudget(value) {
	const budget = {
		...DEFAULT_AGENT_TURN_BUDGET,
		...value
	};
	for (const [key, limit] of Object.entries(budget)) if (!Number.isSafeInteger(limit) || limit <= 0) throw new Error(`turnBudget.${key} must be a positive safe integer`);
...
var AgentLoop = class extends Service {
	...
	static Config = z.object({
		...
		turnBudget: z.object({
			maxElapsedMs: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER),
			maxSteps: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER),
			maxModelAttempts: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER),
			maxToolCalls: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER),
			cancellationGraceMs: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER)
		}),
	...
	constructor(ctx, config) {
		...
		this.config = {
			...
			turnBudget: resolveTurnBudget(config.turnBudget),
	...
	*/
	prepare(ownerCtx, id, options, session, callerSignal) {
		...
		const dispose = (ownerTriggered = false, options = {}) => {
			...
			const operation = (async () => {
				...
				if (machine !== void 0) {
					machine.cancel({ kind: "disposed" }, options);
			...
			disposing = operation;
			operation.catch(() => {
				if (disposing === operation) disposing = void 0;
			});
			return operation;
		};
		...
		try {
			const agent = machine = new ReactLoopAgent(loopCtx, id, options, session, this.config.turnBudget);
			...
			return {
				...
				publish: (source) => {
					...
					return {
						agent,
						dispose: (options) => dispose(false, options)
					};
				},
				dispose: (options) => dispose(false, options)
			};
...
//#endregion
export { AGENT_LOOP_SETTINGS_NAMESPACE, AGENT_LOOP_SETTINGS_SCHEMA, AgentLoop, AgentLoop as default, AgentQuiescenceTimeoutError, CONFIGURED_AGENT_IDENTITIES_KEY, DEFAULT_AGENT_TURN_BUDGET, DEFAULT_MAX_PARALLEL_TOOL_CALLS };
