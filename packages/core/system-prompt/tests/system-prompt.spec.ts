...
describe('SystemPrompt', () => {
  ...
  it('orders equal-priority sections by code-unit name in either comparator direction', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    ctx.systemPrompt.section({ name: 'middle', order: 10, text: 'middle' })
    ctx.systemPrompt.section({ name: 'zulu', order: 10, text: 'zulu' })
    ctx.systemPrompt.section({ name: 'alpha', order: 10, text: 'alpha' })
