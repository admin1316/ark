...
import {
  validateRestoredSessionHeader,
  snapshotSessionHeader,
  freezeRestoredObject,
  assertSessionEventEnvelope,
  assertSupportedRequestHeader,
} from './validation.ts'
import { forkSeed, resolveForkSource, SessionForkError, SessionForkSource } from './fork.ts'
import { deriveEventMessage, SurfaceManager } from './surface.ts'
import type { SessionSurface } from './surface.ts'
import { foldRequestHeader } from './request-header.ts'
import {
  sessionRemoteCancelled,
  sessionRemoteUnavailable,
} from './remote.ts'
import type {
  SessionRemoteAcceptedValue,
  SessionRemoteAttachmentRequest,
  SessionRemoteAttachmentValue,
  SessionRemoteCancelRequest,
  SessionRemoteCreateRequest,
  SessionRemoteCreateValue,
  SessionRemoteForkRequest,
  SessionRemoteForkValue,
  SessionRemoteHistoryRequest,
  SessionRemoteHistoryValue,
  SessionRemoteListRequest,
  SessionRemoteListValue,
  SessionRemoteModels,
  SessionRemoteModelsRequest,
  SessionRemoteOperations,
  SessionRemotePromptRequest,
  SessionRemotePromptValue,
  SessionRemoteRenameRequest,
  SessionRemoteRenameValue,
  SessionRemoteResult,
  SessionRemoteSearchRequest,
  SessionRemoteSearchValue,
  SessionRemoteSelectModelRequest,
  SessionRemoteSelectModelValue,
  SessionRemoteUpdateQueueRequest,
} from './remote.ts'
...
export { SessionForkError } from './fork.ts'
export type { SessionForkErrorCode, SessionForkSource } from './fork.ts'
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    ...
    /** Host-composed implementation of the generated `session/*` user-flow port. */
    sessionRemoteOperations?: SessionRemoteOperations
  }
 ...
 */
export class SessionStore extends TypertRemoteService {
   ...
   */
  @Remote('list')
  remoteExportList(request: SessionRemoteListRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteListValue>> {
   ...
   */
  @Remote('search')
  remoteExportSearch(request: SessionRemoteSearchRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteSearchValue>> {
   ...
   */
  @Remote('create')
  remoteExportCreate(request: SessionRemoteCreateRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteCreateValue>> {
   ...
   */
  @Remote('history')
  remoteExportHistory(request: SessionRemoteHistoryRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteHistoryValue>> {
   ...
   */
  @Remote('models')
  remoteExportModels(request: SessionRemoteModelsRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteModels>> {
  ...
  */
   ...
   */
  @Remote('rename')
  remoteExportRename(request: SessionRemoteRenameRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteRenameValue>> {
   ...
   */
  @Remote('fork')
  remoteExportFork(request: SessionRemoteForkRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteForkValue>> {
   ...
   */
  @Remote('prompt')
  remoteExportPrompt(request: SessionRemotePromptRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemotePromptValue>> {
  ...
  */
   ...
   */
  @Remote('cancel')
  remoteExportCancel(request: SessionRemoteCancelRequest, signal: AbortSignal): Promise<SessionRemoteResult<SessionRemoteAcceptedValue>> {
  ...
  private async invokeRemote<Result extends SessionRemoteResult<unknown>>(
    invoke: (operations: SessionRemoteOperations) => Promise<Result>,
    signal: AbortSignal,
  ): Promise<Result> {
    if (signal.aborted) return sessionRemoteCancelled() as Result
    const operations = this.ctx.get('sessionRemoteOperations')
    if (operations === undefined) return sessionRemoteUnavailable() as Result
...
function sessionRemoteResultAfterOperation<Result extends SessionRemoteResult<unknown>>(
  signal: AbortSignal,
  result: Result,
): Result {
  return signal.aborted ? sessionRemoteCancelled() as Result : result
}
