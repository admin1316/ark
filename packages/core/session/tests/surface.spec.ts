...
describe('findToolCallArguments', () => {
  it('finds the latest durable call and fails closed on invalid JSON', () => {
    const session = Session.create(SessionId('tool-call-lookup'))
    ...
    expect(findToolCallArguments(session.events, 'valid')).toEqual({
      name: 'bash',
      args: { command: 'pwd' },
    })
    expect(findToolCallArguments(session.events, 'invalid')).toBeUndefined()
    expect(findToolCallArguments(session.events, 'missing')).toBeUndefined()
  })
})
