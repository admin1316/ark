...
import { decodeSeqRanges, decodeStorageRecord, encodeSeqRanges, packChunkRuns } from '@deepseek-ai/dsh-session'
...
describe('source sequence range codec', () => {
  it('packs only profitable increasing runs and round-trips them', () => {
    expect(encodeSeqRanges([1, 2, 3, 5, 6])).toEqual([[1, 3], 5, 6])
    expect(decodeSeqRanges([[1, 3], 5, 6])).toEqual([1, 2, 3, 5, 6])
    expect(encodeSeqRanges([2, 2, 1])).toEqual([2, 2, 1])
  })

  it.each([
    ['a non-array', null, undefined],
    ['a malformed entry', [{}], undefined],
    ['a malformed pair arity', [[1, 2, 3]], undefined],
    ['a descending range', [[3, 2]], undefined],
    ['an oversized range', [[1, 3]], 2],
    ['non-increasing expanded values', [[2, 3], 1], undefined],
    ['a negative sequence', [-1], undefined],
    ['a fractional sequence', [1.5], undefined],
    ['an unsafe sequence', [Number.MAX_SAFE_INTEGER + 1], undefined],
    ['too many scalar entries', [1, 2], 1],
  ] as ReadonlyArray<readonly [string, unknown, number | undefined]>)('rejects %s', (_label, value, maxEntries) => {
    expect(() => decodeSeqRanges(value, maxEntries)).toThrow(/sourceEventSeqs/)
  })
})
