...
describe('Session.requestHeader', () => {
  it('returns undefined before a snapshot and reuses the frozen fold without new events', () => {
    const session = Session.create(SessionId('request-header-empty'))
    expect(session.requestHeader()).toBeUndefined()
    session.append('request/header', { header: { config: CONFIG, system: 'first' }, reason: 'initial' })

    const first = session.requestHeader()
    expect(first).toEqual({ config: CONFIG, system: 'first' })
    expect(Object.isFrozen(first)).toBe(true)
    expect(session.requestHeader()).toBe(first)
  })

  it('folds a seeded log and advances incrementally to the newest snapshot', () => {
    const session = Session.create(SessionId('request-header-seeded'), [
      { type: 'turn/start', seq: 0, time: 1, data: { turn: 1 } },
      { type: 'request/header', seq: 1, time: 2, data: { header: { config: CONFIG }, reason: 'initial' } },
    ])
    expect(session.requestHeader()).toEqual({ config: CONFIG })
    session.append('todo/write', { todos: [] })
    session.append('request/header', {
      header: { config: { provider: 'mock', model: 'next' }, tools: [] },
      reason: 'change',
    })
    expect(session.requestHeader()).toEqual({ config: { provider: 'mock', model: 'next' } })
  })
})
