...
describe('SessionStore', () => {
  ...
  it('prepares persistence-owned restored values through the strict restore path', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const id = SessionId('restored-preparation')
    ...
    const session = ctx.sessions.prepare(id, { seed, meta, seedSource: 'persistence' })

    expect(session.header).toBe(meta)
    expect(session.events.slice(0, seed.length)).toEqual(seed)
