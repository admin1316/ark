 ...
 */
export function createProcessShutdown(dispose, forceExit = (code) => { process.exit(code); }, complete = (code) => { process.exitCode = code; }, timeoutMs = PROCESS_SHUTDOWN_TIMEOUT_MS, reportFailure = (error) => {
    ...
    const start = () => {
        ...
        pending = Promise.resolve().then(dispose).then(() => {
            if (forceAfterDispose)
                ...
                completeOnce(requestedCode);
        }, (error) => { failOnce(error); });
        return pending;
    };
    return {
        shutdown(code) {
