...
import { boot, composeEntries, healProfilesModuleFallback, installFailLoud, isSnapshotServedDirectory, loadOptionalPatches, loadOverlayPatches, loadProfile, PROFILE_PATCH_FILENAME, watchUserPatches, } from '@deepseek-ai/dsh-app-boot';
 ...
 */
export function prepareProfile(name, installAnchor, userLayer = true) {
...
function composeProfile(name, installAnchor, patchFiles, additionalSystemPresetRoots, profilePatchMode, homePatchMode) {
    ...
    const profilePatches = profilePatchMode === 'managed'
        ? loadOverlayPatches(NAME, profile.patchPath)
        : profile.patches;
    const homePatches = homePatchMode === 'user'
    ...
    if (rows.has('agent-presets')) {
        const systemPresetRoots = [...new Set([
                SHIPPED_PRESET_ROOT,
                ...additionalSystemPresetRoots,
            ])];
        composedOverlays.push({
            id: 'agent-presets',
            config: {
                ...(rows.get('agent-presets')?.config ?? {}),
                roots: systemPresetRoots.map(path => ({ path, trust: 'system' })),
            },
        });
    }
    ...
    return { profile, bundlePatches, profilePatches, homePatches, overlays: composedOverlays, rows };
}
/**
...
export async function runProfile(options) {
    ...
    installFailLoud(NAME, process, async () => {
        // Fatal loader rejection, signals, appExit, and natural completion all
        // join the same disposal promise. installFailLoud still owns the final
        // fatal process.exit(1), but it can no longer enter the Cordis disposer a
        // second time with a shorter competing deadline.
    ...
    const composeLive = () => structuredClone([
        ...composed.bundlePatches,
        ...((options.profilePatchMode ?? 'user') === 'user'
            ? loadOptionalPatches(NAME, composed.profile.patchPath) ?? []
            : composed.profilePatches),
        ...((options.homePatchMode ?? 'user') === 'user'
            ? loadOptionalPatches(NAME, homePatchPath()) ?? []
            : []),
        ...composed.overlays,
    ]);
    ...
    if (options.watchLiveConfig !== false
        ...
        try {
            ...
            for (const filename of watchedPatchPaths) {
