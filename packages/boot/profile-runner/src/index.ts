...
interface ComposedProfile {
  ...
  /** Profile patch bytes classified by the caller as user-managed, application-managed, or absent. */
  profilePatches: PatchOptions[]
  /** The home-level user layer (`$DSH_HOME/cordis.patch.yml`), when enabled. */
  ...
  if (rows.has('agent-presets')) {
    ...
    composedOverlays.push({
      ...
      config: {
        ...
        roots: systemPresetRoots.map(path => ({ path, trust: 'system' as const })),
      },
    })
  }
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  const composed = composeProfile(
    ...
    options.patchFiles,
    options.additionalSystemPresetRoots ?? [],
    options.profilePatchMode ?? 'user',
    options.homePatchMode ?? 'user',
  )
