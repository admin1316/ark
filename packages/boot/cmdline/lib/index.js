...
* @param label - The label input.
*/
function exitOnStdinEnd(ctx, label) {
	const exit = ctx.get("appExit");
	if (exit === void 0) throw new Error("stdio app: the launcher must provide ctx.appExit before the tree mounts");
...
function parseCmdline(ctx, program) {
	...
	configureExitAndOutput(program, process.stdout, process.stderr);
	try {
...
*/
function configureExitAndOutput(command, stdout, stderr) {
	command.exitOverride().configureOutput({
		writeOut: (text) => void stdout.write(text),
		writeErr: (text) => void stderr.write(text)
	});
	for (const child of command.commands) configureExitAndOutput(child, stdout, stderr);
}
/**
...
//#endregion
export { exitOnStdinEnd, parseCmdline, provideCmdline };
