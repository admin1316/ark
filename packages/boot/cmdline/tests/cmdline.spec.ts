...
import { parseCmdline, provideCmdline } from '../src/index.ts'
...
function fakeStdin(readableEnded = false): {
  input: StdinBoundary
  end(): void
  removed(): number
} {
...
describe('provideCmdline', () => {
  ...
  it('routes a pre-registered subcommand rejection through the launcher exit request', () => {
    ...
    vi.spyOn(process, 'stderr', 'get').mockReturnValue({
      write: (chunk: string) => { err += chunk; return true },
...
describe('exitOnStdinEnd', () => {
  it('requests bounded completion exactly once on EOF and removes its listener when the tree disposes', async () => {
    const ctx = new Context()
    const exits: number[] = []
    const input = fakeStdin()
    mockProcessStdin(input.input)
    ctx.provide('appExit', (code) => { exits.push(code) })

    const { exitOnStdinEnd } = await import('../src/index.ts')
    ...
    input.end()
    expect(exits).toEqual([0])
    expect(input.removed()).toBe(1)
  })

  it('settles an already-ended stdin on the next microtask', async () => {
    const ctx = new Context()
    const exits: number[] = []
    const input = fakeStdin(true)
    mockProcessStdin(input.input)
    ctx.provide('appExit', (code) => { exits.push(code) })

    const { exitOnStdinEnd } = await import('../src/index.ts')
    exitOnStdinEnd(ctx, 'test.ended-stdin')
  ...
  it('fails loud before mounting when the launcher did not provide an exit request', async () => {
    const ctx = new Context()
    mockProcessStdin(fakeStdin().input)
    const { exitOnStdinEnd } = await import('../src/index.ts')
    expect(() => { exitOnStdinEnd(ctx, 'test.missing-exit') })
      .toThrow('stdio app: the launcher must provide ctx.appExit')
    await ctx.fiber.dispose()
  })
})
