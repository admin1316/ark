...
describe('healProfilesModuleFallback', () => {
  ...
  it('never lets a concurrent resolver observe a missing entry while re-pointing links', async () => {
    // The restart shape: a previous runtime is still resolving the fallback
    // (roster, preset mount) while this heal re-points every link. A reader in
    // another process polls all entries for the whole heal; one missing
    // observation is exactly the failure that marks a preset broken.
    const names = Array.from({ length: 160 }, (_, index) => `pkg-${String(index)}`)
    const anchor = stageInstallation(Object.fromEntries(names.map(name => [name, {}])))
    const home = tmp()
    const fallback = join(home, 'profiles', 'node_modules')
    const stale = tmp()
    mkdirSync(fallback, { recursive: true })
    for (const name of names) {
      mkdirSync(join(stale, name), { recursive: true })
      writeFileSync(join(stale, name, 'package.json'), JSON.stringify({ name, version: '0.0.0' }))
      symlinkSync(join(stale, name), join(fallback, name), 'junction')
    }
    ...
    writeFileSync(reader, [
      ...
      "  for (const name of names) if (!existsSync(join(fallback, name, 'package.json'))) misses += 1",
      '}',
      'process.stdout.write(String(misses))',
    ].join('\n'))
    const { spawn } = await import('node:child_process')
    const child = spawn(process.execPath, [reader, fallback, ...names], { stdio: ['ignore', 'pipe', 'inherit'] })
    let output = ''
    ...
    expect(readlinkSync(join(fallback, 'pkg-0'))).toContain(join('app', 'node_modules'))
  }, 20_000)
})
