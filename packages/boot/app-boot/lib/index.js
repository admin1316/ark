...
function healProfilesModuleFallback(installAnchor, home = resolveDshHome()) {
	if (isSnapshotServedDirectory(dirname(installAnchor))) return;
...
function loadProfile(binName, name, installAnchor, home = resolveDshHome(), options = {}) {
	...
	if (patchReload !== "live" && patchReload !== "startup") throw new Error(`${binName}: profile ${JSON.stringify(name)} has invalid dsh.profile.patchReload`);
	const layers = (manifest.dsh?.profile?.bundles ?? []).map((packageName) => {
