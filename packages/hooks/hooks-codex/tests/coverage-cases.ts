...
if (processExitListenerBaseline.some(listener => Function.prototype.toString.call(listener).includes('terminateForHostExit'))) {
  throw new Error('codex coverage worker started with a leaked LocalSubprocessRuntime exit listener')
}
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((_resolve, reject) => {
        timer = setTimeout(() => { reject(new Error('codex coverage context cleanup timed out')) }, 5_000)
      }),
    ])
  } finally {
...
afterEach(async () => {
  ...
  if (currentExitListeners.length !== processExitListenerBaseline.length
    ...
    failures.push(new Error('codex coverage did not restore the exact process exit listener baseline'))
  }
  if (failures.length === 1) throw failures[0]
  if (failures.length > 1) throw new AggregateError(failures, 'codex coverage teardown failures')
})
