...
if (processExitListenerBaseline.some(listener => Function.prototype.toString.call(listener).includes('terminateForHostExit'))) {
  throw new Error('codex hooks worker started with a leaked LocalSubprocessRuntime exit listener')
}
...
afterEach(async () => {
  ...
  for (const ctx of contexts.splice(0).reverse()) {
    try {
      await withTeardownDeadline('codex hooks context cleanup', async () => { await ctx.fiber.dispose() })
    } catch (error: unknown) {
      failures.push(error)
    }
  }
  ...
  if (currentExitListeners.length !== processExitListenerBaseline.length
    ...
    failures.push(new Error(
      'codex hooks did not restore the exact process exit listener baseline',
    ))
  }
  if (failures.length === 1) throw failures[0]
  if (failures.length > 1) throw new AggregateError(failures, 'codex hooks teardown failures')
})
