...
const processExitListenerBaseline = process.rawListeners('exit')
if (processExitListenerBaseline.some(listener => Function.prototype.toString.call(listener).includes('terminateForHostExit'))) {
  throw new Error('claude hooks worker started with a leaked LocalSubprocessRuntime exit listener')
}
...
async function withTeardownDeadline(label: string, operation: () => Promise<void>): Promise<void> {
...
afterEach(async () => {
  ...
  for (const ctx of contexts.splice(0).reverse()) {
    try {
      await withTeardownDeadline('claude hooks context cleanup', async () => { await ctx.fiber.dispose() })
    } catch (error: unknown) {
      failures.push(error)
    }
  }
  for (const d of dirs.splice(0)) rmSync(d, { recursive: true, force: true })
  const currentExitListeners = process.rawListeners('exit')
  if (currentExitListeners.length !== processExitListenerBaseline.length
    || currentExitListeners.some((listener, index) => listener !== processExitListenerBaseline[index])) {
    failures.push(new Error(
      'claude hooks did not restore the exact process exit listener baseline',
    ))
  }
  if (failures.length === 1) throw failures[0]
  if (failures.length > 1) throw new AggregateError(failures, 'claude hooks teardown failures')
})
...
function processIsAlive(pid: number): boolean {
  if (!Number.isSafeInteger(pid) || pid <= 1) throw new Error(`hook PID file contained unsafe pid: ${String(pid)}`)
  try {
    process.kill(pid, 0)
    return true
  } catch (error: unknown) {
