...
if (processExitListenerBaseline.some(listener => Function.prototype.toString.call(listener).includes('terminateForHostExit'))) {
  throw new Error('claude coverage worker started with a leaked LocalSubprocessRuntime exit listener')
}
...
async function withTeardownDeadline(operation: () => Promise<void>): Promise<void> {
  let timer: ReturnType<typeof setTimeout> | undefined
  try {
    await Promise.race([
      operation(),
      new Promise<void>((_resolve, reject) => {
        timer = setTimeout(() => { reject(new Error('claude coverage context cleanup timed out')) }, 5_000)
      }),
    ])
  } finally {
...
afterEach(async () => {
  ...
  for (const ctx of contexts.splice(0).reverse()) {
    try {
      await withTeardownDeadline(async () => { await ctx.fiber.dispose() })
    } catch (error: unknown) {
      failures.push(error)
    }
  }
  ...
  if (currentExitListeners.length !== processExitListenerBaseline.length
    ...
    failures.push(new Error('claude coverage did not restore the exact process exit listener baseline'))
  }
  if (failures.length === 1) throw failures[0]
  if (failures.length > 1) throw new AggregateError(failures, 'claude coverage teardown failures')
})
