...
* @param routeOrAgent - The route or agent input.
*/
async function admitAcpPrompt(ctx, routeOrAgent, prompt, imageEnabled, signal) {
	const route = routeOrAgent === void 0 ? void 0 : "options" in routeOrAgent ? (() => {
		const config = routeOrAgent.session.requestHeader()?.config;
		const provider = config?.provider ?? routeOrAgent.options.provider;
		const model = config?.model ?? routeOrAgent.options.model;
...
const DEFAULT_MCP_TOOL_TIMEOUT_MS = 6e4;
...
*/
function createOwnedStdioStream(input, output) {
	let controller;
	...
	const finishInput = (error) => {
		...
		detachInput();
		input.pause();
		if (error === void 0) controller.close();
		else controller.error(error);
	};
	const onData = (chunk) => {
		controller.enqueue(typeof chunk === "string" ? Buffer$1.from(chunk) : Uint8Array.from(chunk));
	};
	const onEnd = () => {
		finishInput();
	};
	...
	const inputBytes = new ReadableStream({ start(next) {
		controller = next;
		...
		stream: ndJsonStream(new WritableStream({ write(chunk) {
			return new Promise((resolveWrite, rejectWrite) => {
				output.write(chunk, (error) => {
					if (error === null || error === void 0) resolveWrite();
...
* @param allowStaleImageReference - The allow stale image reference input.
*/
async function assistantUpdates(ctx, session, event, allowStaleImageReference = false) {
	const updates = [];
	for (const block of event.data.message.content) {
		...
		let content;
		try {
			content = await assistantBlockToAcp(ctx, block);
		} catch (error) {
			if (block.type !== "image" || !allowStaleImageReference) throw error;
			ctx.logger.warn(`acp: assistant output conversion failed; assistant image unavailable, delivering an attachment reference: ${String(block.attachment.attachmentId)}`);
			content = {
				type: "text",
...
var AcpSession = class AcpSession {
	...
	*/
	static async create(ctx, options) {
		const llm = ctx.get("llm");
		if (llm === void 0) throw internalError$1("ACP session requires an LLM service");
		const modelControl = new AcpModelControl(llm, options.fallbackSelection);
		...
		try {
			if (event.type === "assistant/message") {
				...
				const delivery = this.outputTail.then(async () => {
					for (const update of await assistantUpdates(this.ctx, session, event, inflight !== void 0)) await this.notify({
						sessionId: this.agent.session.id,
						update
					});
				});
				...
				this.outputTail = previous.then(() => this.notify({
					...
					if (inflight !== void 0) inflight.outputError ??= error;
	...
	close(detail) {
		...
		this.closing = (async () => {
			...
			try {
				...
				if (sessions !== void 0) await sessions.flush(this.agent.session);
			} catch (error) {
...
const name = "acp";
/** Agent creation is the legacy minimum; persistence is checked by resume/list paths. */
const inject = ["agents"];
...
function apply(ctx, config) {
	...
	const implementation = {
		async initialize(_params) {
			...
			return {
				...
				agentCapabilities: {
					...
					...ctx.get("sessionPersistence") === void 0 ? {} : {
		...
		async newSession(params, signal) {
			...
			try {
				...
				assertOpen();
				if (persistence !== void 0) await persistence.ensureMaterialized(record.agent.session);
				assertOpen();
				return {
					sessionId,
					configOptions
		...
		async resumeSession(params, signal) {
			assertOpen();
			if (persistence === void 0) throw internalError("session/resume requires a session persistence backend");
			validateWorkspaceParams(params);
			const sessionId = SessionId(params.sessionId);
			if (sessions.has(sessionId) || activating.has(sessionId) || ctx.get("sessions")?.get(sessionId) !== void 0) throw invalidParams(`session is already active: ${sessionId}`);
		...
		async listSessions(params, signal) {
			assertOpen();
			if (persistence === void 0) throw internalError("session/list requires a session persistence backend");
			...
			const entries = (await Promise.all(listed.map(async (header) => {
				if (sessions.has(header.id) || activating.has(header.id) || ctx.get("sessions")?.get(header.id) !== void 0 || header.origin === "subagent" || header.parentSession !== void 0 || header.cwd === void 0 || !isAbsolute(header.cwd)) return void 0;
	...
	if (config.stream === void 0) {
		ownedStdio = createOwnedStdioStream(process.stdin, process.stdout);
		stream = ownedStdio.stream;
	} else stream = config.stream;
	const conn = new AgentSideConnection((connection) => ({
		initialize: (params) => implementation.initialize(params),
		authenticate: async (params) => {
			await implementation.authenticate(params);
			return {};
		},
		newSession: (params) => implementation.newSession(params, connection.signal),
		listSessions: (params) => implementation.listSessions(params, connection.signal),
		resumeSession: (params) => implementation.resumeSession(params, connection.signal),
		closeSession: (params) => implementation.closeSession(params),
		setSessionConfigOption: (params) => implementation.setSessionConfigOption(params, connection.signal),
		prompt: (params) => implementation.prompt(params, connection.signal),
		cancel: (params) => implementation.cancel(params)
	}), stream);
	let quiescing;
	const quiesce = () => {
...
function agentOptions(config) {
	return {
		...
		...config.reasoningEffort === void 0 ? {} : { reasoningEffort: ReasoningEffortId(config.reasoningEffort) },
