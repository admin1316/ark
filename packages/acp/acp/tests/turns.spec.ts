...
describe('ACP prompt lifecycle', () => {
  ...
  it('ignores an agent/error for a different turn than the inflight prompt', async () => {
    harness = await makeBridgeHarness({ script: [textResponse('answer')] })
    const sessionId = await newSession(harness)
    const session = harness.ctx.sessions.get(SessionId(sessionId))!
    // A late error for ANOTHER turn must not reject the healthy inflight
    // prompt; the prompt settles on its own turn's idle.
    void harness.ctx.parallel('agent/error', {
      agent: harness.ctx.agents.get(SessionId(sessionId))!,
      turn: 99,
      step: 1,
      error: new Error('unrelated turn failed'),
    })
