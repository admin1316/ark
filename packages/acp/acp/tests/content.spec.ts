...
function admissionFixture(options: {
  ...
  header?: { provider?: string; model?: string; reasoningEffort?: ReturnType<typeof ReasoningEffortId> }
} = {}): AdmissionFixture {
...
describe('ACP rich content codec', () => {
  ...
  it('reconstructs image-only and baseline prompts without empty text blocks', async () => {
    const fixture = admissionFixture()
    await expect(admitAcpPrompt(fixture.ctx, undefined, [
      { type: 'text', text: 'route-free text' },
    ...
    const reasoned = admissionFixture({
      header: { provider: 'mock', model: 'vision', reasoningEffort: ReasoningEffortId('high') },
    })
    await expect(admitAcpPrompt(reasoned.ctx, reasoned.agent, [
      { type: 'image', data: 'AQ==', mimeType: 'image/png' },
