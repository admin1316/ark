...
describe('ACP connection ownership', () => {
  ...
  it('repeated default-stdio mounts restore the exact process stream listeners', async () => {
    const inputEvents = ['data', 'end', 'close', 'error'] as const
    const outputEvents = ['end', 'error', 'close', 'finish', 'drain'] as const
    const inputBaseline = new Map(inputEvents.map(event => [event, process.stdin.rawListeners(event)]))
    const outputBaseline = new Map(outputEvents.map(event => [event, process.stdout.rawListeners(event)]))

    for (let iteration = 0; iteration < 12; iteration += 1) {
      const ctx = new Context()
      ctx.provide('agents', {} as never)
      const fiber = await ctx.plugin(AcpPlugin, {})
      await fiber.dispose()
      await ctx.fiber.dispose()
      for (const event of inputEvents) {
        expect(process.stdin.rawListeners(event)).toEqual(inputBaseline.get(event))
      }
      for (const event of outputEvents) {
        expect(process.stdout.rawListeners(event)).toEqual(outputBaseline.get(event))
      }
    }
  })
