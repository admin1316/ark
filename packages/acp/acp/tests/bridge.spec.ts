...
describe('automation-only ACP bridge', () => {
  ...
  it('passes configured reasoning and output-token settings to the exact ACP route', async () => {
    harness = await makeBridgeHarness({
      script: [textResponse('configured')],
      config: { provider: 'mock', model: 'reasoner', reasoningEffort: 'high', maxTokens: 321 },
      models: [{ id: 'reasoner', reasoningEfforts: ['low', 'high'] }],
    })
    await harness.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })
    const { sessionId, configOptions } = await harness.client.newSession({ cwd: process.cwd(), mcpServers: [] })
    expect(configOptions?.find(option => option.id === 'reasoning_effort')).toMatchObject({
      currentValue: 'high',
    })
  ...
  it('clears the previous reasoning effort when ACP changes to a route without it', async () => {
    harness = await makeBridgeHarness({
      script: [textResponse('plain')],
      config: { provider: 'mock', model: 'reasoner', reasoningEffort: 'high' },
      models: [
        { id: 'reasoner', reasoningEfforts: ['high'] },
        { id: 'plain' },
      ],
    })
    await harness.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })
    ...
    const switched = await harness.client.setSessionConfigOption({
      sessionId,
      configId: 'model',
      value: JSON.stringify(['mock', 'plain']),
    })
    expect(switched.configOptions?.some(option => option.id === 'reasoning_effort')).toBe(false)
