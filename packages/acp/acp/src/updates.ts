...
export async function assistantUpdates(
  ...
  allowStaleImageReference = false,
  ...
  for (const block of event.data.message.content) {
    ...
    let content: Awaited<ReturnType<typeof assistantBlockToAcp>>
    try {
      content = await assistantBlockToAcp(ctx, block)
    } catch (error: unknown) {
      // A stale assistant image must remain visible as a recoverable reference
      // instead of converting an otherwise committed answer into a failed ACP
      // turn. Direct content conversion remains strict for callers that need
      // to distinguish storage corruption.
      ...
      content = { type: 'text' as const, text: `[image attachment ${String(block.attachment.attachmentId)}]` }
    }
