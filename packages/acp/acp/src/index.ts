...
import { createOwnedStdioStream, type OwnedStdioStream } from './stdio.ts'
...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  const implementation = {
    ...
    async newSession(params: NewSessionRequest, signal: AbortSignal): Promise<NewSessionResponse> {
      ...
      try {
        ...
        assertOpen()
        if (persistence !== undefined) await persistence.ensureMaterialized(record.agent.session)
        assertOpen()
    ...
    async resumeSession(params: ResumeSessionRequest, signal: AbortSignal): Promise<ResumeSessionResponse> {
      assertOpen()
      if (persistence === undefined) throw internalError('session/resume requires a session persistence backend')
      validateWorkspaceParams(params)
      const sessionId = SessionId(params.sessionId)
      if (sessions.has(sessionId) || activating.has(sessionId) || ctx.get('sessions')?.get(sessionId) !== undefined) {
    ...
    async listSessions(params: ListSessionsRequest, signal: AbortSignal): Promise<ListSessionsResponse> {
      assertOpen()
      if (persistence === undefined) throw internalError('session/list requires a session persistence backend')
      ...
      const filtered = await Promise.all(listed.map(async (header) => {
        if (
          sessions.has(header.id)
            ...
            || ctx.get('sessions')?.get(header.id) !== undefined
  ...
  let ownedStdio: OwnedStdioStream | undefined
  let stream: Stream
  ...
  const conn = new AgentSideConnection(connection => ({
    ...
    authenticate: async (params) => { await implementation.authenticate(params); return {} },
 ...
 */
function agentOptions(config: AcpConfig): AgentOptions {
  return {
