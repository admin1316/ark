...
export class AcpSession {
  ...
  static async create(ctx: Context, options: CreateAcpSessionOptions): Promise<AcpSession> {
    ...
    if (llm === undefined) throw internalError('ACP session requires an LLM service')
    ...
    try {
      if (event.type === 'assistant/message') {
        ...
        const delivery = previous.then(async () => {
          for (const update of await assistantUpdates(this.ctx, session, event, inflight !== undefined)) {
        ...
        this.outputTail = previous
          ...
          .catch((error: unknown) => {
            if (inflight !== undefined) inflight.outputError ??= error as Error
