...
function canPassThroughNormalization(detected, bytes, policy) {
	const maxPixels = policy.maxPixels ?? Number.MAX_SAFE_INTEGER;
	return detected.mediaType !== "image/gif" && !detected.animated && !detected.carriesMetadata && detected.depth === "uchar" && detected.space === "srgb" && bytes <= policy.maxBytes && detected.width * detected.height <= maxPixels && Math.max(detected.width, detected.height) <= policy.maxDimension;
}
...
function initialDimensions(detected, policy) {
	const budgeted = requestImageDimensions(detected.width, detected.height, policy.maxPixels ?? Number.MAX_SAFE_INTEGER);
...
/**
* Derive the immutable normalized-object path without reading the object.
* @param root - The root input.
* @param ref - The ref input.
* @returns The value produced by normalized image path.
*/
