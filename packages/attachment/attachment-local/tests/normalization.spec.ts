...
import * as ImageModule from '../src/image.ts'
...
describe('canPassThroughNormalization', () => {
  it('accepts an in-budget clean PNG/JPEG/WebP and refuses GIF, animation, metadata, oversized edges, and oversized bytes', () => {
    ...
    expect(canPassThroughNormalization(
      { mediaType: 'image/png', width: 4, height: 4, ...clean },
      100,
      { maxDimension: 8192, maxBytes: POLICY.maxBytes },
    )).toBe(true)
  })
})
...
describe('normalizeImage', () => {
  ...
  it('uses the unlimited pixel default while still enforcing the long-edge cap', async () => {
    const data = await flatImage(10, 6, 'png')
    const normalized = await normalizeImage(data, await detectImage(data), {
      maxDimension: 5,
      maxBytes: POLICY.maxBytes,
  ...
  it('fails closed when the encoded image does not verify against its declared facts', async () => {
    const data = await flatImage(6, 4, 'png')
    const detected = await detectImage(data)
    const spy = vi.spyOn(ImageModule, 'detectImage').mockResolvedValueOnce({
      ...detected,
      width: detected.width + 1,
    })
    await expect(normalizeImage(data, { ...detected, carriesMetadata: true }, POLICY))
      .rejects.toMatchObject({
        code: 'ATTACHMENT_WRITE_FAILED',
