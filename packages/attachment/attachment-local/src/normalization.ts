...
export interface NormalizationPolicy {
  /** Total-pixel budget; larger sources are downscaled proportionally. Omission preserves the pre-budget API. */
  maxPixels?: number
...
function initialDimensions(detected: DetectedImage, policy: NormalizationPolicy): { width: number; height: number } {
  const budgeted = requestImageDimensions(
    detected.width,
    detected.height,
    policy.maxPixels ?? Number.MAX_SAFE_INTEGER,
  )
