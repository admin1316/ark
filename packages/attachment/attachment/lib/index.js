...
function requestImageDimensions(width, height, maxPixels) {
	if (!Number.isSafeInteger(width) || width <= 0 || !Number.isSafeInteger(height) || height <= 0 || !Number.isSafeInteger(maxPixels) || maxPixels <= 0) throw new RangeError("request image dimensions require positive safe integers");
...
var AttachmentStore = class extends Service {
	...
	*/
	...
	/**
	* Locate the normalized object in the harness host filesystem, when this
	* backend is file-backed. Native execution-world adapters may map this path
	* into their own readable namespace; non-file stores return undefined.
	* @param _ref - normalized attachment reference to locate.
	* @returns the absolute host path, or `undefined` for a non-file store.
	*/
	imageHostPath(_ref) {}
	/**
