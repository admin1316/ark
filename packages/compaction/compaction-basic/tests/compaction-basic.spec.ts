...
describe('compact configuration and defaults', () => {
  ...
  it('resolves threshold and retention overrides independently', () => {
    const thresholdOnly = resolveCompactionConfig({
      thresholdRatio: 0.5,
    })
    ...
    const retentionOnly = resolveCompactionConfig({
      retainTokens: 70,
    })
  ...
  it('merges exact provider/model policy overrides and scales ratios per model', () => {
    ...
    const ratioOverride = resolveTargetPolicy(resolveCompactionConfig({
      retainTokens: 200,
      modelPolicies: [{
  ...
  it('validates common values and pressure-policy invariants', () => {
    ...
    for (const [config, pattern] of bad) {
      expect(() => resolveCompactionConfig(config as BasicCompactionConfig)).toThrow(pattern)
    }

    const invalidPressure = resolveTargetPolicy(resolveCompactionConfig({
      thresholdRatio: 0.5,
      retainTokens: 500,
    }), { provider: MODEL, model: MODEL })
