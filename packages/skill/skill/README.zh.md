...
`skill/list` Remote 接收网关解析后的 Agent，只返回其已保存 cwd 与作用域内用户可调用的元数据。它不加载正文，也不接收原始宿主路径。读取取消时返回 `cancelled`；会话缺少 cwd 时返回 `session-unavailable`；已经分类的 Remote 失败保留原有载荷。
