...
let SkillRegistry = (() => {
	...
	return class SkillRegistry extends _classSuper {
		static {
		...
		collectCacheMaxEntries = __runInitializers(this, _instanceExtraInitializers);
		...
		*/
		async remoteList(agent, signal) {
			...
			if (cwd === void 0) remoteSkillFailure("session-unavailable", `session "${agent.id}" has no project cwd`, { sessionId: String(agent.id) });
