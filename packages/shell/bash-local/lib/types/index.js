 ...
 */
export function resolveBashExecutable(platform = process.platform) {
    if (platform !== 'win32')
        return 'bash';
...
function scanWindowsBashExecutable() {
    ...
    const systemDirectories = new Set([systemRoot, join(systemRoot, 'System32'), join(systemRoot, 'SysWOW64')].map(directory => directory.toLowerCase()));
    for (const directory of (process.env.PATH ?? '').split(delimiter)) {
        if (directory.trim() === '' || systemDirectories.has(directory.toLowerCase()))
            continue;
    ...
    throw new Error('bash-local: no bash.exe found on PATH outside the Windows system directories — '
