...
- **Agent preset**：Ark 默认使用随产品交付的 `standard` preset，并通过原生设置提供 preset 选择与编写。受管 profile 禁用预设服务的通用内置根，让启动器的原生根拥有预设列表。每个会话获得其所选 preset 拥有的工具与 prompt；通用 CLI 预设仍可用于其自身部署。
...
内置提供方保留目录中的端点、协议和逐模型能力。原生展示按品牌组织变体，但不合并路由身份或凭据。模型选择器支持搜索与显式选择，空模型列表会恢复内置目录。Logo 是经过哈希校验的本地资源，缺少素材时回退为文字。OAuth 和多字段账户设置仍需补齐原生授权连接；目录中存在条目，并不证明相应流程或真实账号已通过验收。
