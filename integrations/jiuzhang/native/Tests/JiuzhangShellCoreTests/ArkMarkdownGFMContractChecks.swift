...
func runArkMarkdownGFMContractChecks() {
  ...
  let sourceAID = NativeAssistantMarkdownSourceID(messageID: 42, sourceSlot: 0)
  let sourceBID = NativeAssistantMarkdownSourceID(messageID: 42, sourceSlot: 3)
  ...
  let mixedRows = NativeAssistantMarkdownRowProjection.rows(
    ...
    blocksBySourceID: [
      sourceAID: [.paragraph([.text("a0")]), .paragraph([.text("a1")])],
      sourceBID: [.paragraph([.text("b0")])],
    ]
  )
  let mixedOrder = mixedRows.map { row -> String in
    switch row {
    case .pending(let source): return "pending-\(source.sourceSlot)"
    case .markdown(let block): return "markdown-\(block.sourceID.sourceSlot)-\(block.blockIndex)"
  ...
  let pendingRows = NativeAssistantMarkdownRowProjection.rows(
    message: mixedMessage,
    sources: [sourceA, sourceB],
    blocksBySourceID: [sourceAID: [.paragraph([.text("a0")])]]
  )
  check(
  ...
  let hugeRows = NativeAssistantMarkdownRowProjection.rows(
    message: hugeMessage,
    sources: [hugeSource],
    blocksBySourceID: [hugeSourceID: largeFinalBlocks]
  )
  let changedSource = NativeAssistantMarkdownSource(
    id: hugeSourceID,
  ...
  let flatRows = NativeAssistantMarkdownFlatProjection.rows(
    messageID: hugeMessage.id,
    hasPrefix: true,
    bodyRows: hugeRows
  )
  check(
...
@MainActor
@ViewBuilder
private func markdownHistoryProbeRow(
  _ row: NativeAssistantMarkdownFlatRow
) -> some View {
  switch row {
  case .prefix(let messageID):
    Text("History prefix \(messageID)")
  case .body(let bodyRow):
    ...
    case .pending(let sourceID):
      Text("Pending \(sourceID.messageID)-\(sourceID.sourceSlot)")
    case .companion(let messageID, let blockIndex, _):
      Text("Companion \(messageID)-\(blockIndex)")
    }
  case .suffix(let messageID):
    Text("History suffix \(messageID)")
  }
}
...
private func runLargeFinalMarkdownHostingProbe() {
  ...
  var historyRows: [NativeAssistantMarkdownFlatRow] = []
  for messageOffset in 0..<messageCount {
    let start = blockCount * messageOffset / messageCount
    let end = blockCount * (messageOffset + 1) / messageCount
    let messageID = 9_000 + messageOffset
    let sourceID = NativeAssistantMarkdownSourceID(messageID: messageID, sourceSlot: 0)
    let bodyRows: [NativeAssistantProjectedBodyRow] = (start..<end).enumerated().map {
      localIndex, parsedIndex in
      .markdown(NativeAssistantMarkdownBlockRow(
        sourceID: sourceID,
        blockIndex: localIndex,
        block: parsed[parsedIndex]
      ))
    }
    historyRows.append(contentsOf: NativeAssistantMarkdownFlatProjection.rows(
      messageID: messageID,
      hasPrefix: messageOffset == 0 || messageOffset == messageCount - 1,
      bodyRows: bodyRows
    ))
  }
  guard Set(historyRows.map(\.id)).count == historyRows.count else {
    check(false, "native Markdown history probe has unique source-free flattened row identities")
    return
  }
  ...
  print(
    "R20 Markdown history probe: messages=\(messageCount) blocks=\(blockCount) "
      + "rows=\(historyRows.count) ax=deferred-live elapsed=\(elapsedLabel)s "
...
private func runLargeFinalMarkdownHostingProbeProcess() {
  ...
  do {
    ...
    if timedOut || !childExited || !childPassed || launcher.terminationStatus != 0 {
      keepFailedApp = true
      print(rendered)
      print("R20 Markdown probe App retained: \(app.path)")
    }
  } catch {
