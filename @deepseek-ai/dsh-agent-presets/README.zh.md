...
“仅限尚未产出任何内容的 agent”是一条产品规则而非机制约束：在对话进行中调换工具，会留下新组装无法执行的、已被记录的工具调用。Preset 服务的严格 `agentPreset/select` Remote 在同时持有实时 Agent 与 Session 历史的位置执行该规则，并在不改变组装的情况下返回 `agent-preset-locked`。
