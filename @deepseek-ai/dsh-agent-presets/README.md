...
The restriction to a produced-nothing agent is a product rule, not a mechanical one: swapping tools mid-conversation would leave logged tool calls the new composition cannot make. The preset service's strict `agentPreset/select` Remote enforces it while holding the live Agent and Session history, returning `agent-preset-locked` without changing composition.
