...
function apply(ctx, config = {}) {
	...
	const belongsToComposition = (agent) => compositionScope === void 0 || scopeChainOf(scopeOf(agent.ctx)).includes(compositionScope);
	const uninstall = (agent) => {
		installed.get(agent)?.();
		installed.delete(agent);
	};
	const maybeInstall = (agent) => {
		if (!belongsToComposition(agent) || installed.has(agent) || ctx.agentTeams.tryMembership(agent) === void 0) return;
