...
- `settle(ns, revision)` 等待该精确版本的 owner 回调，即使在文档通知中同步调用也如此。回调失败时返回 false，但不会回滚持久化；版本被取代或 namespace 被释放时则拒绝。相同文档的重复写入保留既有结算，不会把等待中或失败的结果替换为成功。
...
两种通知都能读取已提交的值和已安装的结算；文档通知先于解析值通知，但本身不表示激活成功。如果同步监听器发布了较新版本或值，后续监听器不会收到已被取代的通知。每个已提交值的 watcher 回调仍按序执行。[观察者生命周期决策](../../../.agents/notes/implemented/architecture/2026-07-30-settings-write-path-integrity.zh.md)解释了这一区别。
...
- **`schema.toJSON()` 仍会把 secret 字段的 `.default(...)` 带给每个客户端** — 脱敏遍历器本身现已 fail closed：只能经由 union、intersect 或 transform 节点抵达的 `role('secret')` 会拒绝整条描述，而不是原样返回机密。序列化 schema 封装仍未被证明安全：无论是否传入 `describe({ redactSecrets: true })`，secret 的 `.default()` 都会随 `schema.toJSON()` 输出传输；净化该封装暂缓。
