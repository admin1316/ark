...
- `settle(ns, revision)` waits for that exact revision's owner callbacks, including when called synchronously from its document notification. It returns false on callback failure without rolling back persistence; a superseded revision or disposed namespace rejects. An identical document write retains the existing settlement rather than replacing a pending or failed result with success.
...
Both notifications see the committed value and installed settlement; the document notification precedes the resolved-value notification and does not imply activation. If a synchronous listener publishes a newer revision or value, later listeners do not receive the superseded notification. Watcher callbacks remain ordered for every committed value. The [observer lifecycle decision](../../../.agents/notes/implemented/architecture/2026-07-30-settings-write-path-integrity.md) explains this distinction.
...
- **`schema.toJSON()` still carries a secret field's `.default(...)` to every client** — the redaction walker itself now fails closed: a `role('secret')` reachable only through a union, intersect, or transform node refuses the whole descriptor instead of returning the secret verbatim. The serialized schema envelope remains unproven: a secret-bearing `.default()` travels in `schema.toJSON()` output regardless of `describe({ redactSecrets: true })`, and sanitizing that envelope is deferred.
