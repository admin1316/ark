...
| `startContinuable(spec)` | 建立一个持久化的可继续子 agent，并投递其初始提示词。只有 inbox 插入越过已配置的 Session flush 屏障后，调用才兑现为 `{ childId, messageId, durable: true }`；它不等待轮次结束。接受前发生的失败会完全回滚该子 agent。如果在线注册表或已配置的持久化已经占用调用方预留的 `childId`，则拒绝该身份。要求 `ctx.agents`、会话持久化以及具备 `prepareContinuable` 能力的提供方。 |
...
面向模型的工具默认同步收集：先等待子 agent 结果，再 dispose 运行，然后才返回。一次性后台委派会在工具中注册普通 Task，其通用状态、收集和取消工具负责后续交互，并将模型提供的 `description` 持久化为可选显示标签。可继续后台委派会调用 `ctx.subagents.startContinuable()`，并且只在初始 inbox 插入持久化后返回；之后由子 agent 拥有自己的轮次，因此没有 Task，也没有结果 promise。调用方使用 `send_message` 发送后续工作；`interrupt()` 只停止当前轮次而不 dispose 子 agent；持久化的子 agent Session 仍是详细输出的来源。原生后续消息携带调用方稳定的 `invocationId`；完全相同的重试会在同一子 Session 的 inbox／事件日志中找到并返回原消息 id，而同一 invocation 携带不同正文会失败。`listChildren()` 枚举在线优先的 Session 语料，并仅对每个 child 自己的后缀（`events.slice(seedLength)`）应用 `foldSubagentDescriptor()`，据此分类在线和冷 child。只有一个描述符才有效；重复、格式错误或仅从种子继承的描述符会成为受控的 `corrupt` 行。投影状态始终只是派生显示／缓存数据，不能覆盖这项身份权威。冷记录使用有界并发的持久化检查，读取失败仍是下一次列表会重试的瞬时 `unavailable`。列表在每次持久化读取前后转发取消，并且仍要求 Session 与投影注册表完成生命周期和外来投影校验。
