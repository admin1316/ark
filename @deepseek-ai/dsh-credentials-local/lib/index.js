...
const DEFAULT_KEYCHAIN_SERVICE = "dsh.credentials";
/**
...
var LocalCredentialProvider = class extends CredentialProvider {
	...
	*/
	...
	constructor(ctx, config) {
		...
		this.keychainService = config.keychainService ?? "dsh.credentials";
		if (this.mode === "keychain" && process.platform !== "darwin") throw new Error("credentials-local: keychain mode requires macOS");
	}
		...
		return new Promise((resolvePromise, reject) => {
			execFile("/usr/bin/security", args, (error, stdout) => {
				if (error !== null) {
					...
					reject(error instanceof Error ? error : /* @__PURE__ */ new Error("security command failed"));
					return;
				}
	...
	async keychainSet(ref, value) {
		...
		return new Promise((resolvePromise, reject) => {
			const terminal = pty.spawn("/usr/bin/security", [
			...
			const timeout = setTimeout(() => {
				...
				settled = true;
				terminal.kill();
				reject(/* @__PURE__ */ new Error("security command timed out"));
			}, 3e4);
			terminal.onData((data) => {
				if (settled) return;
				const expectedPrompt = prompts[promptIndex];
				if (expectedPrompt === void 0) return;
			...
			terminal.onExit(({ exitCode }) => {
				...
				reject(Object.assign(/* @__PURE__ */ new Error(`security command failed (exit ${exitCode})`), { code: exitCode }));
			});
		});
	}
	keychainUnset(ref) {
	...
	*/
	...
	resolve(ref) {
		...
		if (this.mode === "keychain") return this.keychainGet(ref).then((stored) => stored === "" ? this.resolveDotenvFallback(ref) : {
			value: stored,
			source: "keychain"
		});
	...
	describe(ref) {
		...
		if (this.mode === "keychain") return this.keychainGet(ref).then((stored) => stored === "" ? this.describeDotenvFallback(ref) : {
			configured: true,
			source: "keychain",
			writable: true
		});
...
//#endregion
export { CREDENTIALS_FILENAME, DEFAULT_KEYCHAIN_SERVICE, DOCUMENT_VERSION, LocalCredentialProvider, LocalCredentialProvider as default, parseCredentialsDocument, renderFlatLayoutMigration, resolveSpec };
