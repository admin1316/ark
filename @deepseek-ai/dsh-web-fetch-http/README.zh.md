...
- 每个初始地址和重定向地址都会在连接前解析；只要完整解析结果中的任一 IPv4、IPv6、IPv4-mapped 或已识别 NAT64 目标不是公网全局单播地址，就拒绝整组结果；空结果和格式错误的 DNS 结果同样拒绝。
- 将已经验证的地址集合固定到请求专用的 Undici dispatcher，同时保留 URL 主机名供 HTTP `Host` 与 TLS SNI 使用，防止连接阶段再次解析并通过 DNS rebinding 转向私有目标。
- 强制执行 URL 最大长度、响应字节上限（`WEB_FETCH_TOO_LARGE`）、解码主体字符上限、超时（`WEB_FETCH_TIMEOUT`）和重定向跳数上限。
- 把调用方的中止信号（`WEB_ABORTED`）传播到网络请求与流式读取。
- 只跟随**同源**重定向，并在每一跳连接前重新执行公网地址验证和固定；跨源重定向以 `WEB_REDIRECT_BLOCKED` 失败，要求发起新的工具调用（沿用 Claude Code 的 WebFetch 模式）。
...
- **仅允许公网目标**：没有私有网络 allowlist 或单次请求绕过入口。公网与私网混合的解析结果会整组拒绝；IPv6／NAT64 的保守验证无法完成时，请求也会失败，不会退回未固定的连接。
