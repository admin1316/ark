...
`llm/mutateProvider` 与 `llm/resumeProvider` 使用同一个事务 owner。共享提供方日志的调用按先入先出顺序执行，保护范围涵盖状态读取、校验、凭据写入、设置持久化和 owner 结算。共享的设置命名空间与凭据引用在同一区间内保持受保护；无关资源可以独立推进。

`expectedRevision` 仍用于检查冲突，不是后写覆盖的提示。新请求在排队期间修订号变为 陈旧 时，会在修改凭据之前记录冲突。对当前已完成收据的精确回放只读取结果，不会重复移除凭据，也不要求旧引用仍存在于配置中。

持久化认领前的调用方取消以 `cancelled` 拒绝，不修改凭据或配置。认领一旦开始，事务就保持保护，直到常规提交或失败／恢复结果结算；较晚的取消不是回滚。RPC 取消不能证明没有持久化：应查询 `llm/providerTransaction`。不可取消的提供方 I/O 必须结算后才能释放所有权。

事务回调内嵌套的修改或恢复调用以 `provider-transaction-reentrant` 拒绝，避免等待自身和锁顺序反转。执行租约局限于同一 Host 共享的服务身份，不是跨进程存储锁；不同 Host 需要隔离存储，本 owner 之外的特权直接写入不受其串行化约束。[执行所有权决策](../../../.agents/notes/implemented/bug-fix/2026-09-06-provider-transaction-execution-ownership.zh.md) 记录了资源顺序与理由。
