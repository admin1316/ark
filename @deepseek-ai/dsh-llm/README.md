...
`llm/mutateProvider` and `llm/resumeProvider` use one transaction owner. Calls sharing a provider journal execute FIFO through state reads, validation, credential writes, settings persistence and owner settlement. Shared settings namespaces and credential references remain protected for the same interval; unrelated resources can progress independently.

`expectedRevision` remains a conflict check, not a last-write-wins hint. A new request whose revision became stale while queued records the conflict before changing credentials. An exact replay of the current completed receipt reads its result without reapplying an unset or requiring the old reference to remain in the profile.

Caller cancellation before durable claim rejects with `cancelled` without credential or profile mutation. Once claim starts, the transaction keeps protection until its normal commit or failure/recovery result settles; late cancellation is not rollback. An RPC cancellation does not prove that persistence was absent: inspect `llm/providerTransaction`. Non-cancellable provider I/O must settle before ownership is released.

Mutation or resume calls nested inside transaction callbacks reject with `provider-transaction-reentrant`, avoiding self-wait and lock inversion. Execution leases are local to a Host's shared service identities, not a cross-process storage lock; separate Hosts need isolated stores, and privileged direct writes outside this owner are not serialized by it. The [execution-ownership decision](../../../.agents/notes/implemented/bug-fix/2026-09-06-provider-transaction-execution-ownership.md) records the resource ordering and rationale.
