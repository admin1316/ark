...
Its two consumers are the host-side native integrations: the [`directory-picker-native`](../../host/directory-picker-native/README.md) backend's OS chooser commands and [`dsh-settings`](../../settings/settings/README.md), whose strict `settings/openDocument` Remote opens only the provider-owned configuration document. The `NativeCommandRunner` type is their injectable command boundary.
