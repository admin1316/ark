...
- **以适合平台的方式发送信号的 detached 进程树**：POSIX 子进程使用 `detached` spawn（拥有独立进程组），信号以负 pgid 发送并以直接子进程作为回退；Windows 通过 `taskkill /PID <pid> /T /F` 终止进程树。`terminate()`（句柄唯一的终止操作）先发送 SIGTERM，经过 spec 的宽限期后再发送 SIGKILL（沿用 OpenCode 的升级策略；流水线与子 shell 会随父进程一起结束），进程树消亡后为空操作；`waitForExit()` 轮询整棵进程树的存活状态，使消费方的拆卸能确认真正的完全停稳，并受两个宽限期的观察期限约束（卡在不可杀状态的树会如实返回 `false`，而不是让拆卸永久挂起）。组长进程退出后，仍然打开的管道也只获得同样有界的排空宽限期，因此存活的后代进程无法无限期地拖住结果不结算。系统会容忍 ESRCH；重新指定父进程并脱离该组的 daemon 仍可能存活。
...
- **不可杀的进程树会如实上报而不是挂起**：处于不可中断状态、扛过 SIGKILL 的后代会使 `waitForExit()` 在两个宽限期的观察期限放弃并返回 `false`；句柄仍保持被持有，后续 dispose 仍可升级，拆卸会记录警告而不是永久阻塞 fiber。
