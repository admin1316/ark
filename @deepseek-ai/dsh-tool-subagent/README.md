...
Each plugin instance binds one subagent transport `provider` to one `toolName`; load another distinctly named instance to expose another transport. When `modelSelectionSettings` is enabled, the model may select an authorized child LLM `provider`/`model`, `reasoning_effort`, and positive `max_tokens` budget independently of that transport. The tool registers only while its transport provider exists, avoiding sibling load-order and provider-reload dependencies. Its description follows `provider.inheritsParentContext`: fresh children require standalone prompts, while forked children already see completed parent turns.
...
`backgroundMode` selects both the background route and the omitted `run_in_background` default. `one-shot` waits in the foreground by default; an explicit `true` registers a plain parent-owned Task and returns canonical `{ kind: 'background', jobId }`, rendered as `started background subagent job <id>`, even when the provider supports continuable children. Generic task tools own its later status, collection, cancellation, and notices; a failed Task keeps the stop reason and the same optional provider diagnostic in its detail. `continuable` runs in the background when the argument is omitted or `true`; an explicit `false` waits for the result in the foreground. Its background route requires a provider with the `prepareContinuable` capability, calls `ctx.subagents.startContinuable()`, and returns `{ kind: 'continuable', subagentId }`, rendered as `started subagent <childId>`. The route resolves after durable inbox acceptance: the child owns its turns from there, so this call never collects a result. The child's transcript by that id remains the source of detailed output, and the optional global `send_message` tool sends it more work. The continuation service emits one durable outcome notice whenever the child's Activation ends and avoids repeating a byte-identical explicit report. Starting continuable work does not require `send_message` to be loaded. See the [background subagent Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-background-subagent-tasks.md), the [continuable subagents Agent Note](../../../.agents/notes/implemented/feature/2026-07-28-continuable-subagent-conversations.md), and the [background-first delegation Agent Note](../../../.agents/notes/implemented/feature/2026-08-11-background-first-continuable-delegation.md).
...
| `modelSelectionSettings` | Exposes the session-captured child LLM allowlist and the `provider`, `model`, `reasoning_effort`, and `max_tokens` request fields. A provider without the `agentOptions` capability fails the mount instead of ignoring them. |
...
- **Non-model child policy is fixed per instance** — persona, tool filter, and depth cap require another distinctly named tool. Model selection is fixed too unless `modelSelectionSettings` exposes a session-captured allowlist; an enabled session may choose an allowed route, reasoning effort, and output budget per call.
