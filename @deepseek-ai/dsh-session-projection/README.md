...
Session-projection Service Definition and drive registry. It owns `ctx.sessionProjections`, the registry that drives every registered projection unit over committed session events and serves finished whole values to current consumers: `session-remote-operations` reads history and baselines, while `NativeEvents` projects the `session/projection` frame on its mux. A domain registers pure mathematics; the framework owns the drive. The [session-projection RFC](../../../.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.md) records the design rationale.
...
- **No wire vocabulary here.** The registry exposes only the change feed and the snapshot read face; `NativeEvents` projects the change feed on its mux, while `session-remote-operations` reads snapshots for session Remote responses.
...
This package owns the Service Definition and drive roles of the capability seam: domain host plugins (e.g. `dsh-tool-todo`) contribute units, `dsh-host-native-events` consumes the change feed, `dsh-host-session-remote-operations` consumes current baselines, and none knows the others' implementation.
