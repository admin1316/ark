...
临时纯文本 Markdown 首帧现在同时保留有界的开头与结尾。原来对已经截取的流式尾部再次取前缀，会在解析尚未完成时隐藏最终结束标记。组合预览仍受原有字符预算约束，仅额外包含明确的省略分隔符，并保留字素边界；规范正文不变。AppKit 滚动协调器也会处理程序性贴底之后稍后到达的正文高度变化，不再丢弃下一次尺寸通知。原有无动作判断与重入保护避免重复滚动；历史阅读锚点仍优先于自动贴底。
