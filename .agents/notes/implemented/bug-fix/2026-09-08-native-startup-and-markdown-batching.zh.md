...
流式轨迹构建器直接向自身持有的事件与文本缓冲区追加。此前每次追加前复制构建器，会共享这些缓冲区并反复复制不断增长的前缀；远端 2 万事件契约耗时 12.99 秒。改为原地追加后，同一契约及不变的 5 秒预算在本机以 0.048 秒通过（此前本机为 3.65 秒），全部事件及最终回答逐字保留。远端仍需重新验证。
