# Agent Note: profile 的有界信号关闭和重复信号强制退出
...
共享的 `dsh-profile-runner` 持有 `createProcessShutdown`，它是围绕根级 dispose 建立的进程级控制器。CLI 和 Native profile 启动共用这一实现；已退役的 Web 启动器仍保留在上述事故历史中：

- 多次正常关闭调用会汇合到同一次 dispose；非零结果会升级先前的零退出码，而不启动另一个 disposer。这些调用不会相互触发强制退出。dispose 成功后，控制器通过 `process.exitCode` 记录退出码，让 Node 自然排空剩余句柄；dispose 失败或超时时只报告一次诊断并以非零状态强制退出，因为启动器不能假定失败的插件树已经完全停稳。
- 第一个信号启动或加入同一次优雅 dispose，并受其保持引用的 5 秒退出兜底约束。dispose 成功或失败都只退出一次，均不能取消进程退出。
- 第二个信号立即强制退出。正常 dispose 期间收到的第一个信号加入该次排空，并要求结算后退出；dispose 完成后，如剩余句柄使 Node 继续运行，信号会强制退出。
...
profile 启动器为监督进程的 SIGTERM 请求退出码 0，为用户 SIGINT 请求退出码 130。disposer 失败或超时不能报告成功。headless 回合结果仍由其应用插件负责。
...
健康的正常退出流程仍会对整棵 Cordis 插件树执行 dispose，随后等待 Node 事件循环自然排空。已知的遥测等待最多会在 3 秒后解除；其他退出流程卡死时，如无进一步输入，最多等待 5 秒；第二个信号会立即结束待结算的排空，dispose 完成后的信号会立即结束残留句柄排空。强制退出或受截止时间限制的退出可能中断遥测导出或尚未完成的清理工作；只有优雅关闭约定已经失败，或用户明确要求强制退出时，才会有意接受这一结果。

该控制器属于启动器基础设施，而不是 Cordis 插件：它不会声称 dispose 已经完成，也不会削弱普通 disposer 必须达到完全停稳状态的生命周期规则。

## 测试

`packages/boot/profile-runner/tests/process-shutdown.spec.ts` 固定了自然完成、非零失败与超时诊断、5 秒退出兜底、正常与致命错误调用汇合、首个信号加入正常 dispose、dispose 后句柄排空，以及第二个信号强制退出的行为。`packages/boot/profile-runner/tests/telemetry-switch.spec.ts` 保留遥测关闭开关边界；CLI 不再持有独立实现。

`apps/cli/tests/headless-shutdown.e2e.ts` 在 PTY 中启动真实交付的 headless Loader 插件树，并挂载一个仅用于测试的插件；该插件的 disposer 会声明已经进入清理流程，但永不结算。测试在观察地址出现后发送 SIGINT，等待 dispose 已启动的证据，再次发送 SIGINT，并要求进程以 130 退出。源码／产物启动解析器使两个执行平面都覆盖同一项回归。该 PTY 用例覆盖用户可见的进程状态；模型输出快照没有变化。
