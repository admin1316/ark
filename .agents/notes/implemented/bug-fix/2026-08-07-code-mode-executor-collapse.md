...
- The prompt STATES the collapse, in the `tools:code-only` section ordered ahead of per-tool guidance by `FIRST_PARTY_SECTION_ORDER.PTC_ONLY`; the generated SDK follows that guidance at `FIRST_PARTY_SECTION_ORDER.TOOLS_SDK`. Those sections name their tool without qualifying how it is reached, so a model that read only them emitted a native call, received `UNKNOWN_TOOL` for a tool the same prompt declared, and concluded the deployment was inconsistent rather than correcting itself. The denial carries the route for the same reason. `both` renders the rule empty: its native calls do execute, so stating it there would be false — which is why `both-mode-turn` no longer shares `code-mode-turn`'s expected prompt.
