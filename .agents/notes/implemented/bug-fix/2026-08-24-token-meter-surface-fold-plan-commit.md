...
`_sync` captures `endSeqExclusive` once and reads accepted immutable events through `Session.eventAt(seq)`. Session projections advance to the fixed watermark, including late creation and hydration, without copying the full log prefix. Host history and query observation reuse the cached frozen `Session.events` snapshot; later appends cannot mutate a captured cut. Session may still allocate one snapshot after mutation, `measure()` still clones and freezes its result, and O(content) pricing remains. `SurfaceTokenFold` (the old detached-result type) is gone; `surface-fold.ts` is package-internal, so no external consumer changes. The [composer context-meter note](../feature/2026-08-05-composer-context-meter-breakdown.md) records the projection design around this fold.
