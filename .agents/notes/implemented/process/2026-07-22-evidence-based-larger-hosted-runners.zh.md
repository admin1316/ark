...
**CI 的固定沙盒工具是一条溯源链，而不是图省事的下载。** `scripts/prepare-ci-bubblewrap.sh` 最初只取一个 URL（`pool/main/b/bubblewrap/bubblewrap_0.9.0-1ubuntu0.1_amd64.deb`），并且在证明它能运行之前就把它写进 `PATH`。该修订后来离开 pool：2026-09-18 该 URL 在 archive.ubuntu.com、security.ubuntu.com、ports.ubuntu.com 均返回 HTTP 404（curl 退出码 22），使 coverage、snapshot 与 API e2e 在准备步骤内、任何测试运行之前就失败——而 sandbox 工作流的 bwrap leg 仍然通过，因为那条 leg 用 `apt-get` 安装发行版包，从不调用这个脚本。现在脚本固定 runner 发行版当前的补丁修订（`0.9.0-1ubuntu0.3`，amd64），摘要取自存档自身签名索引 `noble-updates` 与 `noble-security` 的 `Packages`——索引值，而不是对下载内容重新计算的哈希。它从一份已审的、全部属于同一 pool 文件的存档镜像列表下载，要求每个来源交付完全相同的字节；每个来源最多重试两次，404 一律不重试；在解压到本轮独占目录之前校验大小、摘要、控制身份（`Package`/`Version`/`Architecture`）与 payload 内容。只有版本串与真实隔离探针（新建命名空间、只读根绑定、必须被拒绝的写入）都成功之后才写入 `GITHUB_PATH`。安全状态是记录而不是假定：USN-8779-1 发布了 `0.9.0-1ubuntu0.2`，USN-8779-2 回退了该 CVE-2026-87766 修复，CVE 页面仍将 24.04 标为受影响，上游只在 bubblewrap 0.12.0 修复，因此本 CI 工具固定当前官方修订且不声称已修复；本轮拒绝在 runner 上源码构建 0.12.0，因为其摘要没有可公布的溯源，而且会给每个消费 job 增加一套构建工具链。`scripts/prepare-ci-bubblewrap.spec.ts` 用命令替身驱动真实脚本覆盖已审阅的结果（备用镜像、有界重试、HTML 错误正文、摘要不符、身份不符、架构不符、缺少可执行文件、解压失败、探针失败、只读绑定可写、残留目录、非 Linux 宿主），`scripts/ci-workflow.spec.ts` 则钉死每个并行调用者都等待两个子进程并传播任一失败。
