...
**CI 的沙盒工具改为从固定且经核验的上游发布构建。** `scripts/prepare-ci-bubblewrap.sh` 最初只取一个 pool URL（`bubblewrap_0.9.0-1ubuntu0.1_amd64.deb`），并在证明它能运行之前就写进 `PATH`，此后再也没走到测试：2026-09-18 该精确文件名在 archive.ubuntu.com、security.ubuntu.com、ports.ubuntu.com 均返回 HTTP 404（281 字节 HTML、无重定向），使 coverage、snapshot 与 API e2e 在准备步骤内失败；而 sandbox 工作流的 bwrap leg 仍通过，因为那条 leg 用 `apt-get` 安装发行版包、从不调用这个脚本。发行版包本身也不是修复：USN-8779-1 在 `0.9.0-1ubuntu0.2` 修复 CVE-2026-87766，USN-8779-2 回退了该修复，CVE 页面仍将 24.04 LTS 标为受影响，上游只在 bubblewrap 0.12.0 修复（GHSA-pxhw-h44j-8pfx：绝对符号链接改为 `openat2(RESOLVE_IN_ROOT)` 解析）。因此 CI 在一次性托管 runner 上构建该发布：从固定的发布 URL 下载 tarball 与其校验文件，并按发布自身的 SHA-256 值校验（126 452 字节的 `bubblewrap-0.12.0.tar.xz` 为 `9760d007...3314`，91 字节的 `.sha256sum` 为 `6f1804eb...21fc`），两份资产必须互相一致，解包树必须报告 `version : '0.12.0'`；构建使用该发布真实要求的工具（meson、ninja、C 工具链、libcap；`-Dtests=false -Dman=disabled -Dselinux=disabled`），产物只放在 `RUNNER_TEMP`，非 setuid、不附加文件能力、绝不覆盖系统 `bwrap`。仅当 `RUNNER_ENVIRONMENT=github-hosted` 时才安装软件包；自托管或用户机器必须已自带工具链。只有 `bwrap --version` 报告 `bubblewrap 0.12.0`、命名空间探针真正执行命令、只读绑定拒绝写入本轮拥有且宿主可写的目录、且该公告对应的上游安全回归（`TestSandbox.test_proc_symlink_escape_blocked` 及其 fallback，从已解包的固定源码加载）经严格适配器执行、对 skip/失败/选中用例数不符一律判失败，并用负向对照证明该门禁同时拒绝“永远成功”和“从不运行”的工具之后，才写入 `GITHUB_PATH`。此前手写的探针已移除：它把自己的观察目录绑定进沙箱，在那里写入本就是合法的沙箱内写入，并非逃逸。所有消费者（ci.yml、ci-master.yml、e2e.yml 以及 sandbox.yml 的 bwrap leg）现在都经这条统一路径准备，不再有任何车道悄悄沿用未修复的工具。`scripts/prepare-ci-bubblewrap.spec.ts` 用命令替身驱动真实脚本覆盖已审阅结果，`scripts/ci-workflow.spec.ts` 钉死并行调用者等待两个子进程并传播任一失败。
