...
> Division of labor: this RFC fixes the framework and constraints (rarely changes); `docs/web-styling.md` is the living spec (authoritative token values, the coding-rule checklist, the deviation record — it evolves with the implementation). Token changes and new rules go there; only changes to the framework itself come back here (overturning it requires a new RFC).
