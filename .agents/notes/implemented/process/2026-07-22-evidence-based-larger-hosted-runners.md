...
**The pinned CI sandbox tool is a provenance chain, not a convenience download.** `scripts/prepare-ci-bubblewrap.sh` originally fetched one URL (`pool/main/b/bubblewrap/bubblewrap_0.9.0-1ubuntu0.1_amd64.deb`) and published it on `PATH` before proving it ran. That revision later left the pool: on 2026-09-18 the URL returned HTTP 404 (curl exit 22) on archive.ubuntu.com, security.ubuntu.com, and ports.ubuntu.com, which failed the coverage, snapshot, and API e2e jobs inside their preparation step before any test ran — while the sandbox workflow's bwrap leg stayed green because that leg installs the distribution package with `apt-get` and never calls this script. The script now pins the runner release's current security revision (`0.9.0-1ubuntu0.3`, amd64) with the SHA-256 recorded from the archive's signed `noble-updates` and `noble-security` `Packages` indexes — the index value, never a hash recomputed from a download. It downloads from an audited list of archive-owned copies that must all deliver those same bytes, retries each source at most twice and never retries a 404, and verifies size, digest, control identity (`Package`/`Version`/`Architecture`) and payload contents before extracting into a directory this run owns. `GITHUB_PATH` is written only after the version string and a real confinement probe (new namespace, read-only root bind, and a write that must be refused) succeed. Security is recorded rather than assumed: USN-8779-1 published `0.9.0-1ubuntu0.2`, USN-8779-2 reverted that CVE-2026-87766 fix, the CVE page still lists 24.04 as vulnerable, and upstream fixes it only in bubblewrap 0.12.0, so this CI tool pins the current official revision and claims no fix; a source build of 0.12.0 was rejected here because its digest would have no published provenance and it would add a build toolchain to every consuming job. `scripts/prepare-ci-bubblewrap.spec.ts` drives the real script through command doubles for the ten reviewed outcomes (mirror fallback, bounded retry, HTML error body, digest mismatch, wrong identity, wrong architecture, missing executable, extraction failure, probe failure, writable read-only bind, leftover tree, non-Linux host), and `scripts/ci-workflow.spec.ts` pins that every parallel caller waits for both children and propagates either failure.
