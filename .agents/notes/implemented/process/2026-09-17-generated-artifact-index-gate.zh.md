...
同一索引检查拒绝属于本机的分发输入：任意深度的真实 `.env` 变体及 `.credentials.yaml` 备份、`.sessions`、`.llm-wiki` 和 `wiki` 目录，以及根目录 profiles、logs、cache、提供商状态、profile 回滚、Harness、Knowledge、Default Workspace、Document References、Workbench Drafts、`.dsh`、会话、存储、附件、终端状态和设置文件。根目录身份标识、运行时补丁及生成的设置参考文件也属于本机；嵌套源码 profile 夹具和示例补丁仍允许跟踪。具名环境示例与录制的快照夹具仍属于有效输入。这是路径检查，不是基于内容的密钥检测。
