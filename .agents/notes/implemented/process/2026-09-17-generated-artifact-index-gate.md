...
The same index check rejects machine-owned distribution inputs: real `.env` variants and `.credentials.yaml` backups at any depth, `.sessions` and `.llm-wiki` directories, and root Harness, `.dsh`, sessions, storages, attachments, terminal state, and settings files. Named environment examples and recorded snapshot fixtures remain valid inputs. This is a path check, not content-based secret detection.

[Source privacy](../../../../.github/workflows/source-privacy.yml) scans fetched Git history with checksum-pinned Gitleaks on pull requests and main pushes. Its [configuration](../../../../.gitleaks.toml) retains the default rules, with narrow exceptions for Git blob metadata, exact synthetic test literals, and the historical upstream public ingestion identifier in its original commits. Output is redacted. The media skill requires an explicit telemetry ingestion key instead of shipping one; an unconfigured installation sends no media telemetry. Local user data and offline recovery archives are outside these source checks; CI neither erases them nor rewrites history.
