...
[Workflow contract tests](../../../../scripts/ci-workflow.spec.ts) continue to pin [issue-policy.yml](../../../../.github/workflows/issue-policy.yml): the trusted default-branch checkout, the pinned action revision, and the minimal permissions are untouched, which is why the trusted check still runs the previous implementation until this change reaches the default branch.
