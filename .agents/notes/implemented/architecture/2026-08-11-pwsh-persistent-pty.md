...
**The injected bootstrap waits out the console's startup negotiation.** On hosts without stdin-wait evidence (Windows and macOS) readiness depends on the visible prompt marker, so a bootstrap written before the console is ready would leave the prompt function uninstalled and guarantee a startup timeout. Waiting for console quiet makes the first line land after the cursor-position negotiation; the previously timing-out macOS pwsh suites now pass, and Linux keeps the same ordering with its banner traffic already emitted before the wait expires.
