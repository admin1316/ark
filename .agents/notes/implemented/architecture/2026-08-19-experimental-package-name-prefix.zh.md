...
Agent Teams 曾使用 `@deepseek-ai/dsh-experimental-agent-team` 与 `@deepseek-ai/dsh-experimental-tool-agent-team` 身份。已晋级的包现在位于 `packages/subagent/agent-team` 与 `packages/subagent/tool-agent-team`，名称分别为 `@deepseek-ai/dsh-agent-team` 与 `@deepseek-ai/dsh-tool-agent-team`。旧名称仅是历史身份；没有兼容别名或仍在使用的退休包路径。
...
promotion 会明确重命名 import、配置、生成引用与元数据。不会有兼容包保留原实验名称。
