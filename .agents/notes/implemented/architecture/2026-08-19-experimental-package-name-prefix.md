...
Agent Teams formerly used the `@deepseek-ai/dsh-experimental-agent-team` and `@deepseek-ai/dsh-experimental-tool-agent-team` identities. The promoted packages now live at `packages/subagent/agent-team` and `packages/subagent/tool-agent-team` as `@deepseek-ai/dsh-agent-team` and `@deepseek-ai/dsh-tool-agent-team`. The former names are historical identities only; no compatibility alias or retired package path remains active.
...
Promotion deliberately renamed imports, configuration, generated references, and metadata. No compatibility package preserves the former experimental name.
