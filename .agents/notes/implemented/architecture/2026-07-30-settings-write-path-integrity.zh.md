...
[Settings 所有者](../../../../packages/settings/settings/src/index.ts)在发布 `settings/document-updated` 前安装已提交的解析值、串行 watcher 链尾与精确版本结算。写入和提供方发布共享这条提交路径。文档事件仍先于 `settings/updated`，表示持久化而非成功激活。监听器可以立即调用 `settle(ns, revision)`；它会等待真实回调。重复写入未改变的原始分节会保留既有结算，而没有解析值变化的新原始版本不产生回调，结算为 true。

同步监听器可以重入发布。较新提交保持权威，其 watcher 按序接在较旧 watcher 之后；旧事件扇出会在向后续监听器投递已被取代的版本或解析值之前停止。这些事件通知当前状态，而非每个中间状态的持久历史。已隔离的监听器失败和同步不变式重抛仍表示失败，不代表回滚或成功激活。[包约定](../../../../packages/settings/settings/README.zh.md)说明调用方可见的结果。

行为测试覆盖写入与提供方两条路径、延迟拒绝的回调、相同文档写入、仅原始值变化的版本、嵌套发布、通知中排队的写入、版本冲突及 owner 释放。测试通过共享基类上的真实内存提供方执行；它们不能证明安装态 Ark 故障或 GUI 恢复。
