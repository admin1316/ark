...
Project and custom skill roots go through `ctx.fs` when a filesystem service is loaded: project-root lookup probes `.git` with `resolve` and `stat`, root discovery uses `listDir`, and skill reads use `readText`. Operator-owned user roots and configured bundled roots use abortable host Node I/O so a session's workspace filesystem cannot hide deployment configuration; each winning locator retains that trust classification so catalog discovery and later body loading use the same authority. Missing roots and malformed or non-text files degrade to warn-and-skip. An unexpected root or entry failure marks the catalog observation incomplete while retaining readable sibling candidates, so consumers preserve their last-good catalog instead of publishing a misleading deletion.
