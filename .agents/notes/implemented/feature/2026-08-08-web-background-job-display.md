...
At the time of this design, `JobView` was browser-safe and lived at the now-retired path `packages/host/apiproxy/src/api/jobs.ts`, alongside the other domain contracts, with its wire schema beside it in `jobs.schema.ts`. The current Native owner is [`packages/host/native-events/src/index.ts`](../../../../packages/host/native-events/src/index.ts):
...
The frame carries a whole snapshot rather than a delta for the reason the retired `packages/host/apiproxy/src/api/events.ts` `session/queue` contract stated for itself: start, kill, settlement, reconnect, and a second browser tab all converge through one authoritative value. The current whole-snapshot producer is [`packages/host/native-events/src/index.ts`](../../../../packages/host/native-events/src/index.ts). A session's task set is single-digit; the frame is small.
...
The `web e2e scenario` is the end-to-end proof and runs keyless: a real `run_in_background` bash call registers with `ctx.jobs`, the header count and row appear with no user interaction, and killing the task through the registry flips the open list to its producer detail. It asserts the whole delivery path rather than any single layer.

Below it, [`jobs-local`](../../../../packages/jobs/jobs-local/tests/jobs.spec.ts) pins the change feed at all four commit points, its containment of a throwing observer, and its removal on both explicit disposal and fiber teardown. The retired `packages/host/apiproxy/tests/api-proxy-jobs.spec.ts` originally pinned the carrier rules; their current owner-side coverage is [`native-events.spec.ts`](../../../../packages/host/native-events/tests/native-events.spec.ts). The client suites pin the last-wins fold, the absent-key representation, both clears, and the component's ordering, duration, and dismissal behavior.
