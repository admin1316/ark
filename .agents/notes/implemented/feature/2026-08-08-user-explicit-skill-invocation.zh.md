...
- 客户端沿用[纯文本引用决策](../architecture/2026-07-25-web-input-machine-and-slash-pipeline.zh.md)：菜单 pick 落下字面文本 `/name `，该文本随提示词原样提交；ui-skill 不实现任何裁决钩子，也没有引用 codec。`skill.list`（该领域唯一的 RPC）提供每一个用户可调用的 skill 并携带 `modelInvocable`，供菜单标出仅限用户的条目。Session 提示词适配器首先让命令注册表解析以斜杠开头的行；未命中时，它查询当前 Agent 的 scoped skill 注册表，仅允许名称精确匹配且用户可调用的 skill，并保持文本不变，交给 pre-step 监听器。与宿主命令同名的名称仍解析为命令，其他未匹配的斜杠行仍返回 `unknown-command`。
