...
本设计形成时，`JobView` 是浏览器安全类型，位于现已退役的 `packages/host/apiproxy/src/api/jobs.ts`，与其他领域约定并列，线路 schema 则在旁边的 `jobs.schema.ts`。当前原生 owner 是 [`packages/host/native-events/src/index.ts`](../../../../packages/host/native-events/src/index.ts)：
...
这一帧带整份快照而非增量，理由就是已退役的 `packages/host/apiproxy/src/api/events.ts` 中 `session/queue` 约定写下的那条：启动、中断、结算、重连，以及第二个浏览器标签页，全都通过同一个权威值收敛。当前整份快照的 producer 是 [`packages/host/native-events/src/index.ts`](../../../../packages/host/native-events/src/index.ts)。一个会话的任务集是个位数，帧很小。
...
`web e2e 场景`是端到端的证据，且无需密钥：一次真实的 `run_in_background` bash 调用注册进 `ctx.jobs`，header 的计数与行在没有任何用户操作的情况下出现，通过注册表杀掉该任务后打开着的列表翻到生产者给出的 detail。它断言的是整条投递链路，而不是其中某一层。

在它之下，[`jobs-local`](../../../../packages/jobs/jobs-local/tests/jobs.spec.ts) 钉住变更订阅的全部四个提交点、对抛错观察者的包容，以及显式销毁与 fiber 拆除两条路径上的注销。已退役的 `packages/host/apiproxy/tests/api-proxy-jobs.spec.ts` 原先钉住载体规则；当前 owner 侧覆盖位于 [`native-events.spec.ts`](../../../../packages/host/native-events/tests/native-events.spec.ts)。客户端各套件钉住 last-wins 折叠、缺失键表示、两处清理，以及组件的排序、时长与关闭行为。
