...
test("imagegen is a RAM-graduated ladder; agent picks the best that fits", () => {
  ...
  assert.ok(ladder.length >= 2, "imagegen offers multiple RAM tiers");
  assert.ok(
...
test("imagegen on a high-memory Mac steps up to the higher-quality FLUX tier", () => {
  const pick = selectModel("imagegen", {
    ramMB: 96000,
    availableRamMB: 80000,
    gpu: { present: true, vramMB: 96000 },
  });
  assert.equal(pick.tier, "large", "80GB free selects the higher-quality FLUX model");
});
