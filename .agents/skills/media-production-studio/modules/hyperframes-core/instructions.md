...
## 生产流水线速查（依赖链，详版见 references/production-loop.md）

Blocks&assets → Audio（可与 Frames 并行）→ Frames → Duration sync → Assembly → Transitions → Captions → Verify → Deliver。
依赖链内顺序固定；独立阶段可并行（音频后台渲染是标准做法）。阶段需求缺失则跳过；编辑请求从所触达的工件重新进入并重跑 Verify。
